 ...
 * */
class AdminSocialNetworking extends \Controller
{
    ...
    public function serviceEdit($service_id = null)
    {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Social_networking_services_model->validateService($post_data);
            if (!empty($validate_data["errors"])) {
                $this->system_messages->addMessage(View::MSG_ERROR, $validate_data["errors"]);
                $data = array_merge($data, $validate_data["data"]);
            } else {
                $data = $validate_data["data"];
                $result = $this->Social_networking_services_model->save_service($service_id, $data);
    ...
    public function serviceActive($service_id = null, $status = '1')
    {
        ...
        $url = site_url('admin/social_networking/services');
        if ($service_id) {
            $data = $this->Social_networking_services_model->getServiceById($service_id);
        }
        if (!empty($data)) {
            $data['status'] = floor($status);
            $this->Social_networking_services_model->saveService($service_id, $data);
        ...
        if (!empty($data)) {
            if ($status == 1) {
                ...
                if ($data['oauth_version'] == 2) {
                    ...
                    $result = $this->Social_networking_connections_model->checkOauth2Connection($data,
                        site_url('admin/social_networking/oauth_active/' . $service_id . '/1/'));
                } else {
                    $result = $this->Social_networking_connections_model->checkOauthConnection($data,
                        site_url('admin/social_networking/oauth_active/' . $service_id . '/1/'));
                }
                // Вывод ошибок
            ...
            if ($this->input->post('btn_save')) {
                ...
                if (!empty($validate_data["errors"])) {
                    ...
                    $this->system_messages->addMessage(View::MSG_SUCCESS,
                        l('success_updated_application', 'social_networking'));
                    $url = site_url('admin/social_networking/services/');
                    redirect($url);
                }
            }
            $this->view->setHeader($data['name']);
            $data['oauth_uri'] = site_url('users_connections/oauth_login/' . $service_id . '/');
            $data['admin_oauth_uri'] = site_url('admin/social_networking/oauth_active/' . $service_id . '/1/');
            $this->view->assign('data', $data);
        ...
        if ($data) {
            if ($this->input->post('btn_save')) {
                ...
                if (!empty($validate_data["errors"])) {
                    ...
                    $this->system_messages->addMessage(View::MSG_SUCCESS,
                        l('success_updated_widgets', 'social_networking'));
