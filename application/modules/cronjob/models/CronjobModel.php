...
class CronjobModel extends \Model
{
    ...
    {
        $this->ci->db->select(implode(", ", $this->attrs))->from(CRONJOB_TABLE)->where("id", $id_cron);
    ...
    public function getCrons($params = array())
    {
        $data = array();
    ...
    public function getCronsCount($params = array())
    {
    ...
    public function deleteCronByParam($params = array())
    {
        if (isset($params["where"]) && is_array($params["where"]) && count($params["where"])) {
            foreach ($params["where"] as $field => $value) {
                $this->ci->db->where($field, $value);
    ...
    public function validateCron($id, $data)
    {
        ...
        if (empty($id) && (empty($data["module"]) || empty($data["model"]) || empty($data["method"]))) {
            ...
            if (isset($data["module"])) {
                if ($this->ci->pg_module->is_module_installed($data["module"]) && 
    ...
    public function run($id, $data = [])
    {
        ...
        if (!$this->isMethodCallable($data["module"], $data["model"], $data["method"])) {
            ...
            if ($data['is_kill'] == 1) {
                ...
                $this->saveCron($id, array("date_execute" => date("Y-m-d H:i:s"), "in_process" => 0));
            }
        }
    ...
    public function getLogCount($params = array())
    {
        $this->ci->db->select("COUNT(*) AS cnt");
        $this->ci->db->from(CRONJOB_LOG_TABLE);
        if (isset($params["where"]) && is_array($params["where"]) && count($params["where"])) {
    ...
    public function deleteLog($params = array())
    {
        if (isset($params["where"]) && is_array($params["where"]) && count($params["where"])) {
            foreach ($params["where"] as $field => $value) {
                $this->ci->db->where($field, $value);
    ...
    public function __call($name, $args)
    {
        ...
        if (!isset($methods[$name])) {
            throw new \Exception('Unknown method ' . $methods[$name]);
        }
