 ...
 **/
class AdminCronjob extends \Controller
{
    /**
     * Constructor
     */
    ...
    {
        parent::__construct();
        $this->load->model(['Cronjob_model', 'Menu_model']);
    ...
    {
        $filter_data = [
            "all" => $this->Cronjob_model->getCronsCount(),
            "not_active" => $this->Cronjob_model->getCronsCount(["where" => ["status" => 0]]),
            "active" => $this->Cronjob_model->getCronsCount(["where" => ["status" => 1]]),
        ];
    ...
    {
        $data = ($id) ? $this->Cronjob_model->getCronById($id) : ['cron_tab' => '0 0 * * *'];
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Cronjob_model->validateCron($id, $post_data);
            if (!empty($validate_data["errors"])) {
                ...
                $id = $this->Cronjob_model->saveCron($id, $save_data);
        ...
        $data = $this->Cronjob_model->formatCron($data);
    ...
    {
        if (!empty($id)) {
            $this->Cronjob_model->deleteCron($id);
            $this->system_messages->addMessage(View::MSG_SUCCESS, l('success_delete_cronjob', 'cronjob'));
        }
        redirect(site_url() . "admin/cronjob/index/" . $_SESSION["cronjob_list"]["filter"]);
    }
