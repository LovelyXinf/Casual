 ...
 */
class VideoUploadsModel extends \Model
{
    public $media_path = "";
    public $media_url = "";
    public $media_type = "";
    ...
    public function getMediaPath($config_gid, $postfix = '')
    {
        if (!empty($postfix)) {
            $postfix = $postfix . '/';
        }
    ...
    public function upload($config_gid, $postfix, $upload_gid, $object_id, $file_data, $name_format = 'format')
    {
        ...
        $type_check = $this->preCheckFileType($_FILES[$upload_gid]['name'], $_FILES[$upload_gid]['tmp_name'], $_FILES[$upload_gid]['type'], $config_data["file_formats"]);
    ...
    private function getValidateFileType($file, $post_file_type)
    {
        $file_type = $post_file_type;
        ///// следующим шагом пытаемся определить mime-Тип с поммощью fileinfo, если настройка включена
        $use_fileinfo = $this->ci->pg_module->get_module_config('video_uploads', 'use_fileinfo');
    ...
    private function preCheckFileType($file_name, $file, $post_file_type, $allowed_extensions)
    {
    ...
    public function uploadExists($config_gid, $postfix, $file_path, $object_id, $file_data)
    {
        $return["errors"] = array();
        $return["file"] = "";
     ...
     * @return array
     */
    public function formatUpload($config_gid, $postfix, $video, $image, $upload_type = '', $isHTML5 = false)
    {
        $upload = array();
        if (!$video) {
            return array();
        ...
        if ($upload_type == 'embed') {
            ...
            $upload = array(
                ...
                'width'     => $config_data['local_settings']['width'],
                'height'    => $config_data['local_settings']['height'],
                'embed'     => $this->ci->videoembed->get_embed_code($video),
            );
        }
    ...
    public function deleteUpload($config_gid, $postfix, $video, $image = '', $upload_type = '')
    {
        ...
        if ($config_data["upload_type"] == 'local') {
            ...
            if ($image) {
                $this->deleteThumbs($image, $path, $config_data["thumbs_settings"]);
            }
            $this->isEmptyPath($path);
        }
        if ($config_data["upload_type"] == 'youtube') {
            $this->ci->load->model('video_uploads/models/Video_uploads_youtube_model');
            $this->ci->Video_uploads_youtube_model->deleteMethod($video);
        }
    ...
    public function createThumbs($file_name, $file_path, $thumbs_data)
    {
    ...
    public function formatThumbs($file_name, $url, $path, $thumbs_data)
    {
        $upload = array();
    ...
    public function deleteThumbs($file_name, $file_path, $thumbs_data)
    {
    ...
    public function uploadEmbedVideoImage($video_config_gid, $data)
    {
    ...
    public function deleteEmbedVideoImage($config_gid, $postfix, $image)
    {
        $config_data = $this->get_config($config_gid);
        $path = $this->get_media_path($config_gid, $postfix);
    ...
    public function __call($name, $args)
    {
        $methods = [
            ...
            'delete_embed_video_image' => 'deleteEmbedVideoImage',
            'delete_thumbs' => 'deleteThumbs',
            'delete_upload' => 'deleteUpload',
            'format_filename' => 'formatFilename',
            'format_thumbs' => 'formatThumbs',
            ...
            'get_validate_file_type' => 'getValidateFileType',
            'upload_embed_video_image' => 'uploadEmbedVideoImage',
            'upload_exists' => 'uploadExists',
