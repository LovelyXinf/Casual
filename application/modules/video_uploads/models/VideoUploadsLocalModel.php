...
class VideoUploadsLocalModel extends \Model
{
        ...
        if ($this->ci->Video_uploads_settings_model->get_settings('use_local_converting_video')) {
            ...
            if (empty($return_video["errors"]) && !empty($return_video["data"]["video"])) {
                ...
                if ($this->ci->Video_uploads_settings_model->get_settings('use_local_converting_meta_data')) {
                    $this->flvtool2MetaTags($return["data"]["video"], $file_path, $config);
                }
     ...
     * @return array
     */
    protected function ffmpegConvert($file_name, $file_path, $config)
    {
        $return = array('errors' => array(), 'data' => array());
        $input_file = $file_path . $file_name;
    ...
    private function flvtool2MetaTags($file_name, $file_path, $config)
    {
        $return = array('errors' => array(), 'data' => array());
        $input_file = $file_path . $file_name;
    ...
    public function getDefaultEmbed($file_url, $width = 480, $height = 360, $isHTML5 = false)
    {
        if ($isHTML5) {
            ...
            //         <a href="' . $file_url . '" style="width:' . $width . 'px;height:' . $height . 'px;" id="default_player_' . $random . '" class="player-content"></a>
            //         <script>
