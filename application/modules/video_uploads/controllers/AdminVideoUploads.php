 ...
 **/
class AdminVideoUploads extends \Controller
{
    /**
    ...
    public function configEdit($config_id)
    {
        $this->load->model('video_uploads/models/Video_uploads_config_model');
        ...
        if ($this->input->post('btn_save')) {
            ...
            if (!empty($validate_data["errors"])) {
                ...
                $this->system_messages->addMessage(View::MSG_SUCCESS, l('success_updated_config', 'video_uploads'));
        ...
        $this->view->assign('formats', $this->Video_uploads_config_model->file_formats);
    ...
    public function systemSettings()
    {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $this->system_messages->addMessage(View::MSG_SUCCESS, l('success_update_system_settings', 'video_uploads'));
        ...
        $this->view->setHeader(l('admin_header_local_settings_list', 'video_uploads'));
        $this->view->render('system_settings');
    }

    public function systemSettingsReset()
    {
        $this->load->model('video_uploads/models/Video_uploads_settings_model');
        $this->Video_uploads_settings_model->reculc_settings();
        $this->Video_uploads_settings_model->write_settings();

        $this->system_messages->addMessage(View::MSG_SUCCESS, l('success_reset_system_settings', 'video_uploads'));
        redirect(site_url() . "admin/video_uploads/system_settings");
    }

    public function youtubeSettings()
    {
        if ($_ENV['YOUTUBE_SETTINGS']) {
            ...
            if ($this->input->post('btn_save')) {
                ...
                $this->system_messages->addMessage(View::MSG_SUCCESS, l('success_update_youtube_settings', 'video_uploads'));
            ...
            $this->view->setHeader(l('admin_header_youtube_settings_list', 'video_uploads'));
            $this->view->render('youtube_settings');
        } else {
            redirect(site_url() . "admin/video_uploads/index");
        }
    }

    public function youtubeSettingsCheck()
    {
        if ($_ENV['YOUTUBE_SETTINGS']) {
            $this->load->model('video_uploads/models/Video_uploads_youtube_model');
            ...
            if ($ret["error"]) {
                ...
                $this->system_messages->addMessage(View::MSG_SUCCESS, l('success_youtube_auth', 'video_uploads'));
            }
            redirect(site_url() . "admin/video_uploads/youtube_settings");
        } else {
            redirect(site_url() . "admin/video_uploads/index");
