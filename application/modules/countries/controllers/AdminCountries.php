...
class AdminCountries extends \Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(['Countries_model', 'Menu_model']);
    ...
    public function countryEdit($country_code = "")
    {
        $data = [];
        if ($country_code) {
            $data = $this->Countries_model->getCountry($country_code, $this->pg_language->current_lang_id, $this->pg_language->languages);
        }
        ...
        if ($this->input->post('btn_save')) {
            ...
            if (!empty($validate_data["errors"])) {
                ...
                $this->Countries_model->saveCountry($country_code, $save_data, $update_type);
    ...
    public function regionEdit($country_code, $id_region = '')
    {
        if (!$country_code) {
            redirect(site_url() . 'admin/countries');
        } else {
            $country_data = $this->Countries_model->getCountry($country_code, $this->pg_language->current_lang_id);
        ...
        $data = [];
        if ($id_region) {
            $data = $this->Countries_model->getRegion($id_region, $this->pg_language->current_lang_id, $this->pg_language->languages);
        }
        ...
        if ($this->input->post('btn_save')) {
            ...
            if (!empty($validate_data["errors"])) {
                ...
                $this->Countries_model->saveRegion($id_region, $save_data);
                ...
                $url = site_url() . "admin/countries/region_edit/" . $country_code . "/" . $id_region;
                redirect($url);
            }
        }
    ...
    public function cityEdit($country_code, $id_region, $id_city = '')
    {
        if (!$country_code) {
            redirect(site_url() . 'admin/countries');
        } else {
        ...
        if (!$id_region) {
            ...
            $region_data = $this->Countries_model->getRegion($id_region, $this->pg_language->current_lang_id);
        ...
        $data = [];
        if ($id_city) {
            $data = $this->Countries_model->getCity($id_city, $this->pg_language->current_lang_id, $this->pg_language->languages);
            foreach ($this->pg_language->languages as $lang_id => $lang_data) {
                $validate_lang[$lang_id] = $data['lang_' . $lang_id];
            }
        }
        ...
        if ($this->input->post('btn_save')) {
            ...
            if (!empty($validate_data["errors"])) {
                ...
                $save_data = $validate_data["data"];
                $this->Countries_model->saveCity($id_city, $save_data);
    ...
    public function countryDelete($country_code)
    {
    ...
    public function regionDelete($country_code, $id_region)
    {
        if (!empty($id_region)) {
    ...
    public function cityDelete($country_code, $id_region, $id_city)
    {
        if (!empty($id_city)) {
    ...
    public function ajaxSaveRegionSorter($country_code)
    {
        $item_data = $this->input->post('sorter');
        $item_data = json_decode($item_data, true);
        $item_data = $item_data["parent_0"];
    ...
    public function ajaxSaveCitySorter($country_code, $id_region)
    {
        $item_data = $this->input->post('sorter');
        $item_data = json_decode($item_data, true);
        $item_data = $item_data["parent_0"];
    ...
    public function ajaxGetRegions()
    {
        ...
        $country = $this->Countries_model->getCacheCountryByCode($country_code);
        $result = [];
        if (!empty($country)) {
            $regions_list = $this->Countries_model->getCacheRegions($country_code);
            $installed_regions_list = $this->Countries_model->getRegionsByCode($country_code, ['code' => 'ASC']);
            $this->view->assign('country', $country);
            $this->view->assign('list', $regions_list);
            $this->view->assign('installed', $installed_regions_list);
            $result = ['content' => $this->view->fetch('install_region', 'admin', 'countries')];
        }
        $this->view->assign($result);
        $this->view->render();
    }
    
    public function ajaxInstallRegions()
    {
        $country_code = $this->input->post('country_code', true);
        $regions = array_unique($this->input->post('region', true));
        $result = [];
        if (!empty($country_code) && !empty($regions)) {
            @set_time_limit(0);
            foreach ($regions as $region) {
                $result[$region]['id'] = $region;
                $result[$region]['status'] = $this->Countries_model->installCities($country_code, $region, $this->pg_language->languages);
            }
        }
