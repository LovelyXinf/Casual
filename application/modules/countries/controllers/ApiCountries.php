...
class ApiCountries extends \Controller
{
    ...
    */
    public function getRegions()
    {
        ...
        $regions = $this->Countries_model->getRegions($country_code,
            ['priority' => 'ASC', 'lang_' . $this->pg_language->current_lang_id => 'ASC'],
            [], [], $this->pg_language->current_lang_id);
        $count = count($regions);
        if ($count) {
    ...
    */
	...
	// TODO: в приложениях сделано так что хотят сразу все данные
        ...
        $order_by = array('priority' => 'ASC', 'lang_' . $this->pg_language->current_lang_id => 'ASC');

        if ($is_simple) {
            $order_by = array('custom_order_by' => '(CASE WHEN name LIKE "'.$search_string.'%" THEN 2 ELSE 3 END) ASC' );
            $items_on_page = 25;
        }
