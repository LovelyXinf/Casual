/* global site_rtl_settings, site_url */
...
function locationAutocomplete(optionArr) {
    this.properties = {
        ...
        id_input_block: 'countries-input-block-',
        timeout_obj: null,
        timeout: 500,
    ...
    this.Init = function (options) {
        ...
        _self.properties.id_input_block = _self.properties.id_input_block + _self.properties.rand;
    ...
    this.bindLoadLocations = function () {
        ...
        if (_self.properties.isAdmin === false) {
            var minTop = $('#' + _self.properties.id_text).offset().top + $('#' + _self.properties.id_text).outerHeight() + 'px';
            var maxTop = $('#' + _self.properties.id_text).offset().top + $('#' + _self.properties.id_text).outerHeight() - 46 + 'px';
            $(window).scroll(function () {
                var top = $(document).scrollTop();
                if (top < 10) {
                    $('#' + _self.properties.id_select).css('top', minTop);
                } else {
                    $('#' + _self.properties.id_select).css('top', maxTop);
                }
            });
            $(window).resize(function () {
               $('#' + _self.properties.id_select).hide();
            });
        }
    }
    ...
    this.initBox = function () {
        $("body").click(function (event) {
            if ($(event.target).closest('#' + _self.properties.id_select).length === 0) {
                _self.closeBox();
            }
        });
        ...
        if (_self.properties.isRadius === 1) {
            $('#' + _self.properties.id_hidden_city).change(function () {
                if ((_self.properties.lat > 0 || _self.properties.lat < 0)
                        && (_self.properties.lon > 0 || _self.properties.lon < 0)) {
    ...
    this.resetDropDown = function () {
        ...
        $('#' + _self.properties.id_select).css({
            width: $('#' + _self.properties.id_input_block).width() + 'px',
            ...
            if (localStorage.getItem("userLocation")) {
                ...
                $.ajax({
                    ...
                    success: function (resp) {
                        if (resp.data.city) {
                            _self.set_values_text(resp.data.city.country_code, resp.data.city.id_region,
                                    resp.data.city.id, resp.data.city.output_name,
                                    resp.data.city.latitude, resp.data.city.longitude);
