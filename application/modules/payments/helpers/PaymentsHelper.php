...
namespace Pg\modules\payments\helpers {
    ...
    if (!function_exists('sendPaymentApi')) {
        ...
        function sendPaymentApi($payment_type_gid, $id_user, $amount, $currency_gid, $system_gid, $payment_data = array())
        {
            $ci = &get_instance();
    ...
    if (!function_exists('receivePayment')) {
        function receivePayment($system_gid, $request_data)
        {
            ...
            if ($data["status"] == 1) {
                $ci->analytics->track('payment_status_success', [
                ...
                $ci->analytics->track('payment_status_fail', [
                    'category' => 'Payment fail',
                    'action' => $data['payment_data']['name'],
                    'label' => $data['payment_data']['offline_line_2'] . '(' . $data['amount'] . ' ' . $data['currency_gid'] . ')',
                    'value' => $data['amount']
                ]);
    ...
    if (!function_exists('currencyFormat')) {
         ...
         * @return string
         */
        function currencyFormat($params = array())
        {            
            $ci = &get_instance();
            $ci->load->model('payments/models/Payment_currency_model');
            $pattern_value = '/\[value\|([^]]*)\]/';
    ...
    if (!function_exists('userPaymentsHistory')) {
        function userPaymentsHistory($params = array())
        {
            $ci = &get_instance();
            $ci->load->model('payments/models/Payment_currency_model');
    ...
    if (!function_exists('currencyFormatRegexp')) {
         ...
         * @return string
         */
        function currencyFormatRegexp($params = array())
        {
            $ci = &get_instance();
            $ci->load->model('payments/models/Payment_currency_model');
            $pattern_value = '/\[value\|([^]]*)\]/';
