...
class AdminProperties extends \Controller
{
    ...
    public function propertyItems($lang_id, $ds_gid, $option_gid = null)
    {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $name_expr = $this->config->item('name', 'reg_exps');
            if (empty($option_gid)) {
                     ...
                     if (!empty($guid_data["errors"])) {
                        ...
                        foreach ($types as $type) {
                            foreach ($lang_data as $lid => $string) {
                                if (preg_match($name_expr, $string)) {echo$type.'<br>';
            ...
            $is_err = false;
            foreach ($lang_data as $lid => $string) {
                if (empty($string) || ($ds_gid === UsersTypesModel::USER_TYPE && !preg_match($name_expr, $string))) {
