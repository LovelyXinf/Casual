...
use Pg\modules\seo_advanced\helpers\SeoAnalyticsHelper;
 ...
 * @author      Pilot Group Ltd <http://www.pilotgroup.net/>
 */
class AdminSeoAdvanced extends \Controller
{
    ...
    public function listing($module_gid = '')
    {
        ...
        if ($this->input->post('btn_save_prefix')) {
            $this->load->model('Install_model');

           
            $seo_settings_data_full = $this->pg_seo->get_settings_full();
            $seo_settings_data_modules = array();

            foreach ($seo_settings_data_full as $key => $value) {
                if ($value['module_gid']) {
                    if (!in_array($value['module_gid'], $seo_settings_data_modules)) {
                        $seo_settings_data_modules[] = $value['module_gid'];
                    }
                } 
            }

            foreach ($seo_modules as $seo_module => $val) {
                if (!in_array($seo_module, $seo_settings_data_modules)) {
                    $seo_settings_data_modules[] = $seo_module; 
                }
            }   

            $cur_settings = array();

            foreach ($seo_settings_data_modules as $seo_module_key => $val_module) {
                $user_settings = $this->pg_seo->get_all_settings('user', $val_module);
                if ($user_settings) {
                    foreach ($user_settings as $key => $set_data) {
                        $cur_settings[$set_data["module_gid"]][$set_data["method"]] = $set_data;
                    }
                }      
            }

            foreach ($cur_settings as $key_set => $methods) {          
                foreach ($methods as $key_m => $val_m) {
                    $langs_file = $this->Install_model->language_file_read($val_m["module_gid"], 'arbitrary');
                    if (!$langs_file) {
                        $this->system_messages->addMessage(View::MSG_ERROR, 'Empty ' . $val_m["module_gid"] . ' langs data');
                    } else {
                        $new_prefix = $this->input->post('new_prefix', true);
                        $post_data = array(
                            'title'  => isset($langs_file["seo_tags_{$val_m["method"]}_title"]) ? $langs_file["seo_tags_{$val_m["method"]}_title"] : null,
                        );

                        if ($post_data['title'] && $new_prefix) {
                            foreach ($post_data['title'] as $key_title => $value) {
                                $post_data['title'][$key_title] = $new_prefix . " : " . $value;
                            }
                        } elseif ($new_prefix) {
                            foreach ($languages as $key_l => $lang) {
                                $post_data['title'][$lang['id']] = $new_prefix . " : " 
                                . ucfirst(str_replace('_', ' ', $val_m["module_gid"])) . " - " 
                                . ucfirst(str_replace('_', ' ', $val_m["method"]));
                            }    
                        }

                        $this->ci->pg_seo->set_settings($val_m["controller"], $val_m["module_gid"], $val_m["method"], $post_data);
                    }
                }   
            }
        }
        ...
        $this->view->assign("env", ENVIRONMENT);
    ...
    public function tracker()
    {
        ...
        if ($this->input->post('btn_save')) {
            ...
            if (!empty($validate_data["errors"])) {
                ...
                $this->view->clearCache($this->view->useCache() ? 'seo_advanced' : null);
    ...
    public function analytics()
    {
        ...
        $url = SeoAnalyticsHelper::prepareUrl(
            $this->input->post('url') ? $this->input->post('url') : base_url());
        ...
        if ((!$domain || $this->input->post('btn_save')) && $url) {
            ...
            // yandex tic
            $domain['tic'] = $domain['registered'] ? SeoAnalyticsHelper::yandexTIC($url) : 0;
            // alexa backlinks
            $domain['alexa_backlinks'] = SeoAnalyticsHelper::backlinks($url, 'alexa');
            // alexa traffic rank
            $domain['alexa_rank'] = SeoAnalyticsHelper::alexaRank($url);
            // google backlinks
            $domain['google_backlinks'] = SeoAnalyticsHelper::backlinks($url, 'google');
            // yahoo backlinks
            $domain['yahoo_backlinks'] = SeoAnalyticsHelper::backlinks($url, 'yahoo');
            // technorati rank
            $domain['technorati_rank'] = SeoAnalyticsHelper::getTechnoratiRank($url);
            // technorati authority
            $domain['technoraty_authority'] = SeoAnalyticsHelper::getTechnoratiAuthority($url);
            // dmoz listed
            $domain['dmoz_listed'] = SeoAnalyticsHelper::dmozListed($url);
            // google directory listed
            //$domain['google_listed'] = SeoAnalyticsHelper::googleListed($url);
            // google indexed
            $domain['google_indexed'] = SeoAnalyticsHelper::googleIndexed($url);
            // yahoo indexed
            $domain['yahoo_indexed'] = SeoAnalyticsHelper::yahooIndexed($url);
            // yandex indexed
            $domain['yandex_indexed'] = SeoAnalyticsHelper::yandexIndexed($url);

            if ($url == SeoAnalyticsHelper::prepareUrl(base_url())) {
    ...
    private function getPriorityUrls()
    {
        ...
        foreach ($modules as $gid => $module) {
            if (!empty($module["get_sitemap_urls_method"])) {
                ...
                $urls[$gid] = $this->{$module["model_name"]}->{$module["get_sitemap_urls_method"]}(false, false);
            }
        }
