...
class MenuModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('menu', CacheManager::PROVIDER_REDIS);
        $this->ci->cache->registerService('menu_items', CacheManager::PROVIDER_REDIS);
    }
    ...
    public function getMenuByGid($gid)
    {
        if (!isset($this->menu_cache[$gid])) {
            ...
            // TODO: cache
            $results = $this->ci->cache->get('menu', 'all', function () use ($fields) {
                $ci = &get_instance();
    ...
    public function deleteMenu($menu_id)
    {
        ...
        $this->ci->cache->flush('menu');
        $this->ci->cache->flush('menu_items');
    ...
    private function getAllMenuItems()
    {
        if ($this->menu_items_all === null) {
            ...
            $this->menu_items_all = $this->ci->cache->get('menu_items', 'all', function () use ($fields) {
                $ci = &get_instance();
    ...
    public function isModerateAccessToItem($item, $permissions, $params = [])
    {
        ...
        if (is_array($params) && isset($params['moderate_sections']) &&
    ...
    public function isActiveItem($item)
    {
        if (!empty($this->curent_active_item_id[$item['menu_id']])) {
            ...
            if ($item['controller'] == $this->ci->router->fetch_class(true) &&
    ...
    public function breadcrumbsSetActive($text, $url = '')
    {
        // Cut
