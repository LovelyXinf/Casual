...
class IndicatorsModel extends \Model
{
    ...
    public function get($auth_type = null, $user_id = null)
    {
        ...
        $this->ci->db->select('i.gid')->from(INDICATORS_TABLE . ' AS i');
        if (!empty($auth_type)) {
            $this->ci->db->join(INDICATORS_TYPES_TABLE . ' AS t', 't.gid = i.gid')
                ->where('t.auth_type', $auth_type);
            if ('user' === $auth_type) {
                $this->ci->db->where('i.user_id', $user_id);
            }
    ...
    public function getByType($indicator_type, $user_id = null)
    {
        ...
        $this->db->from(INDICATORS_TABLE);
        $this->db->where('gid', $indicator_type);
    ...
    public function clearEmptyItems()
    {
        ...
        $this->ci->db->from(INDICATORS_TABLE);
        $this->ci->db->where('user_id', 0);        
        $indicators = $this->ci->db->get()->result_array();
        $data = [];
     ...
     * @return boolean
     */
    public function saveType($gid = null, $data = array())
    {
        if (empty($data)) {
