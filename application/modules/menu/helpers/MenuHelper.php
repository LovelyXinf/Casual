...
namespace Pg\modules\menu\helpers {
    if (!function_exists('getAdminMainMenu')) {
        function getAdminMainMenu()
        {
            ...
            $menu_items = $ci->Menu_model->getMenuActiveItemsList(
                $menu_data['id'],
            ...
            if (!empty($items) && is_array($items)) {
                foreach ($items as $item_gid => $item_data) {
                    if ($process_type == 'create') {
                        ...
                        $items[$item_gid]['id'] = linkedInstallSetMenuItem(
                            $item_gid,
                            $menu_data['id'],
                            $item_data['action'],
                            $parent_id,
                        ...
                        if (!$item_id) {
                            $items[$item_gid]['id'] = $item_id = linkedInstallSetMenuItem(
                                $item_gid,
                                $menu_data['id'],
                                'none'
                            );
                        }
                        linkedInstallSetMenuItemLang(
                            $item_id,
                            $menu_data['id'],
                            'update',
                            $item_lang_data,
                        ...
                        if ('create' == $items[$item_gid]['action']) {
                            $return[$new_prefix]         = linkedInstallSetMenuItemLang(
                                $item_id,
                                $menu_data['id'],
                                'export',
                                $lang_data
                            );
                            $return[$new_prefix_tooltip] = linkedInstallSetMenuItemLang(
                                $item_id,
                                $menu_data['id'],
                                'export',
                                $lang_data,
                        ...
                        if (!empty($items[$item_gid]['items'])) {
                            $temp   = linkedInstallProcessMenuItems(
                                $structure,
                                $process_type,
                                $menu_gid,
                                $item_id,
...
namespace {
    ...
    if (!function_exists('linked_install_set_menu_item')) {
        ...
        {
            return Pg\modules\menu\helpers\linkedInstallSetMenuItem(
                $gid,
                $menu_id,
                $type,
                $parent_id,
    ...
    if (!function_exists('linked_install_set_menu_item_lang')) {
        ...
        = [], $lang_type = 'value')
        {
            return Pg\modules\menu\helpers\linkedInstallSetMenuItemLang(
                $item_id,
                $menu_id,
                $type,
                $lang_data,
    ...
    if (!function_exists('linked_install_process_menu_items')) {
        ...
        {
            return Pg\modules\menu\helpers\linkedInstallProcessMenuItems(
                $structure,
                $process_type,
                $menu_gid,
                $parent_id,
    ...
    /* <custom_R> */
    if (!function_exists('getOperatorMainMenu')) {

        function getOperatorMainMenu()
        {
            $ci = &get_instance();
            $ci->load->model('Menu_model');

            // add link to menu
            $menu_data  = $ci->Menu_model->getMenuByGid('operator_menu');
            $menu_items = $ci->Menu_model->getMenuActiveItemsList($menu_data['id']);
            $ci->view->assign('menu', $menu_items);
            // $ci->view->assign('use_material_design_icons', 1);
            return $ci->view->fetch('main_menu', 'operator', 'menu');
        }
    }
    /* </custom_R> */
}
