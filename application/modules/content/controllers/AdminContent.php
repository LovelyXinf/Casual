...
class AdminContent extends \Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(["Content_model", "Menu_model"]);
    ...
    public function index($lang_id = null)
    {
        ...
        $pages = $this->Content_model->getPagesList($lang_id);
    ...
    public function edit($lang_id, $parent_id = 0, $page_id = null, $section_gid = 'text')
    {
        ...
        $data = [];
        if ($page_id) {
            $data = $this->Content_model->getPageById($page_id, array_keys($languages));
        }
        ...
        if ($this->input->post('btn_save')) {
            switch ($section_gid) {
                case 'text':
                    ...
                    $validate_data = $this->Content_model->validatePage($page_id, $post_data);

                    $validate_logo = $this->Content_model->validateLogo('page_icon');
                    ...
                    if (!empty($validate_data["errors"]) || !empty($validate_logo["errors"])) {
                        $error = array_merge($validate_data["errors"], $validate_logo["errors"]);
                        ...
                        $page_id = $this->Content_model->savePage($page_id, $validate_data["data"]);

                        $this->Content_model->uploadLogo($page_id, 'page_icon');
                    ...
                    $data = array_merge($validate_data["data"], $post_data);
                break;
                case 'seo':
        ...
        $this->view->assign('languages', $languages);
        $this->view->assign("current_lang", $lang_id?:$this->pg_language->current_lang_id);
     ...
     * @return void
     */
    public function ajaxSaveSorter()
    {
     ...
     * @return void
     */
    public function ajaxActivate($status, $page_id)
    {
     ...
     * @return void
     */
    public function ajaxDelete($page_id)
    {
