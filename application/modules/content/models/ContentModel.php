...
class ContentModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('content_pages', CacheManager::PROVIDER_REDIS);
    }
     ...
     * @return array
     */
    public function getPagesList($lang_id, $parent_id = 0, $params = array())
    {
        ...
        if (!empty($results) && is_array($results)) {
            $results = $this->formatPages($results, array($lang_id));
     ...
     * @return array
     */
    public function getActivePagesList($lang_id, $parent_id = 0, $params = array())
    {
     ...
     * @return integer
     */
    public function getPagesCount($params = array())
    {
     ...
     * @return integer
     */
    public function getActivePagesCount($params = array())
    {
        $params["where"]["status"] = 1;
    ...
    public function getAllPages()
    {
        if ($this->pages_all === null) {
            ...
            $this->pages_all = $this->ci->cache->get('content_pages', 'all', function () use ($fields)
            {
                $ci = &get_instance();
    ...
    {
        $page_data = array();
        $result = $this->ci->db->select("gid")->from(CONTENT_TABLE)->get()->result_array();
    ...
    public function savePage($page_id, $attrs)
    {
        ...
        $this->ci->cache->flush('content_pages');
     ...
     * @return string
     */
    public function dynamicBlockGetInfoPages($params, $view, $width = 100)
    {
        $parent_id = 0;
        ...
        if ($params['show_subsections']) {
            ...
            $data['pages'] = $this->get_active_pages_list($this->ci->pg_language->current_lang_id, $parent_id, array('where' => array('parent_id' => $parent_id)));
     ...
     * @return string
     */
    public function dynamicBlockGetInfoPage($params, $view = 'default', $width = 100)
    {
     ...
     * @return array
     */
    public function formatPage($data, $langs_ids = array())
    {
        $pages = $this->formatPages(array($data), $langs_ids);
        return $pages[0];
    }
     ...
     * @return array
     */
    public function formatPages($data, $langs_ids = array())
    {
        $is_uploads_install = $this->ci->pg_module->is_module_installed('uploads');
        if ($is_uploads_install) {
            $this->ci->load->model('Uploads_model');
    ...
    public function __call($name, $args)
    {
        $methods = [
            ...
            '_dynamic_block_get_info_page' => 'dynamicBlockGetInfoPage',
            '_dynamic_block_get_info_pages' => 'dynamicBlockGetInfoPages',
