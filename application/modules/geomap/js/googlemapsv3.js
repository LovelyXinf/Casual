function GoogleMapsv3(defOptions) {
    ...
    this.renderControl = function () {
        ...
        if (!$.inArray(name, a_types))
            default_type = _self.MAP_TYPES[1];
        ...
        $('#zoom_in').bind('click', function () {
            if (_self.zoom < _self.max_zoom)
                _self.setZoom(_self.zoom + 1);
        });
    ...
    this.renderRoute = function () {
        ...
        for (var i = 0; i < route.steps.length; i++) {
            if (route.steps[i].instructions)
                content += '<p>' + route.steps[i].instructions + '</p>';
    ...
    this.getPlaceInfo = function (place) {
        ...
        if (place.international_phone_number)
            info += "<p>" + place.international_phone_number + "</p>";
        if (place.website) {
            var patt = /(http[s]*:\/\/)+(.*?)[\/]+/i;
        ...
        if (!place.icon)
            place.icon = "http://maps.google.com/mapfiles/kml/pal3/icon43.png";
    ...
    this.addAmenities = function () {
        if (_self.amenities_loaded || !_self.prop.amenities.length)
            return false;
    ...
    this.addAmenity = function (amenity, wait) {
        ...
        amenity = new Amenity(marker, $.grep(amenity.types, function (value) {
            return $.inArray(value, _self.prop.amenities) != -1;
        }));
    ...
    this.moveMarker = function (gid, latitude, longitude) {
        for (var i in _self.markers) {
            if (_self.markers[i].getGUID() != gid)
                continue;
    ...
    function Amenity(marker, types) {
        ...
        this.clear = function (types) {
            types = types || [];
            if (types.length) {
                _self.types = $.grep(_self.types, function (value) {
