...
class AdminNotifications extends \Controller
{
     ...
     * @var object
     */
    private $allow_template_var_edit = true;
    ...
    {
        $this->load->model('Notifications_model');
        $data = ($id) ? $this->Notifications_model->getNotificationById($id) : [];
        ...
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Notifications_model->validateNotification($id, $post_data, $langs_data);
            if (!empty($validate_data["errors"])) {
                $this->system_messages->addMessage(View::MSG_ERROR, $validate_data["errors"]);
            } else {
                $this->Notifications_model->saveNotification($id, $validate_data["data"], $validate_data["langs"]);
    ...
    public function settings()
    {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Sender_model->validateMailConfig($post_data);
            if (!empty($validate_data["errors"])) {
                $this->system_messages->addMessage(View::MSG_ERROR, $validate_data["errors"]);
    ...
    public function templateEdit($id_template = null)
    {
        ...
        $this->load->model('notifications/models/Templates_model');
        if ($id_template) {
            $data = $this->Templates_model->getTemplateById($id_template);
            $data_content = $this->Templates_model->getTemplateContent($id_template, $lang_ids);
        } else {
            $data = array(
                "content_type" => "text",
            );
        ...
        $data = $this->Templates_model->formatTemplate($data);
        ...
        if ($data["content_type"] == "html") {
            ...
            foreach ($languages as $lang_id => $lang) {
                ...
                if (!file_exists(SITE_PHYSICAL_PATH . $this->Templates_model->getContentPathData($data))) {
                    $data_content[$lang_id]["content_fck"] = create_editor("content[" . $lang_id . "]", isset($content["content"]) ? 
                        $content["content"] : "", 550, 400, 'Middle');
                } else {
                    $data_content[$lang_id]["content_fck"] = l('filter_content_in_file', 'notifications') . ' - ' . 
                        $this->Templates_model->getContentPathData($data);
                }
            }
        }
    ...
    private function formatPoolTable($order = "id", $order_direction = "DESC", $page = 1)
    {
        $this->load->model('notifications/models/Sender_model');
    ...
    {
        $this->formatPoolTable($order, $order_direction, $page);
    ...
    public function ajaxPoolData($order = "id", $order_direction = "DESC", $page = 1)
    {
        $this->formatPoolTable($order, $order_direction, $page);
    ...
    public function poolSend($id = null)
    {
