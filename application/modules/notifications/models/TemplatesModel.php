...
class TemplatesModel extends \Model
{
    ...
    {
        $data   = array();
        $result = $this->ci->db->select(implode(", ", $this->attrs))->from(NF_TEMPLATES_TABLE)->where("gid", $gid)->get()->result_array();
        if (!empty($result)) {
            $data = $result[0];
        }
    ...
    {
        $data   = array();
        $result = $this->ci->db->select(implode(", ", $this->attrs))->from(NF_TEMPLATES_TABLE)->where("id", $id)->get()->result_array();
        if (!empty($result)) {
            $data = $result[0];
        }
    ...
    public function getTemplatesList($page = null, $items_on_page = null, $order_by = null, $params = array(), $filter_object_ids
    = null)
    {
    ...
    public function getTemplatesCount($params = array(), $filter_object_ids = null)
    {
    ...
    public function getTemplateContent($id_template, $lang_ids = array())
    {
        ...
        $this->ci->db->select('id, id_template, id_lang, subject, content')->from(NF_TEMPLATES_CONTENT_TABLE)->where('id_template',
            $id_template)->where_in('id_lang', $lang_ids);
        $data    = array();
        $results = $this->ci->db->get()->result_array();
        if (!empty($results) && is_array($results)) {
            foreach ($results as $r) {
    ...
    public function addTemplatesContent($lang_id)
    {
        ...
        $default_tpls = $this->ci->db->select("$lang_id as id_lang, id_template, subject, content")
