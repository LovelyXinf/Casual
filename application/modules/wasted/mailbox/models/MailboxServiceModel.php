 ...
 */
class MailboxServiceModel extends \Model
{
    /**
     * Message data
     *
    ...
    private function get($params = [], $page = null, $limits = null, $order_by = null)
    {
        ...
        $this->ci->db->from(MAILBOX_SERVICES_TABLE);
     ...
     * @return array
     */
    public function getServiceById($mailbox_service_id)
    {
        $results = $this->get([
            'where' => ['id' => $mailbox_service_id]
        ]);
        if (!empty($results)) {
            return $results[0];
        } else {
    ...
    public function getServiceByUserService($user_id, $service_gid)
    {
        if ($this->ci->pg_module->is_module_installed('access_permissions') && 
            in_array($service_gid, $this->ci->Mailbox_model->services_buy_gids) === true) {
            $this->ci->load->model('Access_permissions_model');
            $user = $this->ci->Users_model->getUserById($user_id) ?: [];
            $module = [
                'module_gid' => MailboxModel::MODULE_GID,
                'method' => array_flip($this->ci->Mailbox_model->services_buy_gids)[$service_gid]
            ];
            $is_unlimited = $this->ci->Access_permissions_model->isUnlimitedService($user, $module);
     ...
     * @return integer
     */
    public function saveService($mailbox_service_id = null, $data)
    {
        $data['date_modified'] = date('Y-m-d H:i:s');
        if (is_null($mailbox_service_id)) {
            $data["date_add"] = date('Y-m-d H:i:s');
            $this->ci->db->insert(MAILBOX_SERVICES_TABLE, $data);
            $mailbox_service_id = $this->ci->db->insert_id();
        } else {
            $this->ci->db->where('id', $mailbox_service_id);
            $this->ci->db->update(MAILBOX_SERVICES_TABLE, $data);
        }
    ...
    public function removeService($user_id, $service)
    {
        ...
        $this->ci->db->delete(MAILBOX_SERVICES_TABLE);
    }
     ...
     * @return array
     */
    public function validateService($mailbox_service_id, $data)
    {
        $return = ['errors' => [], 'data' => []];
