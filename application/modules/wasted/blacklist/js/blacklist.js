function blacklist(optionArr) {
    ...
    var setBtn = function (id, type) {
        if ('add' === type) {
            $(id + ' ' + _self.properties.addUserButton).closest('dd').removeClass('hide');
            $(id + ' ' + _self.properties.removeUserButton).closest('dd').addClass('hide');
        } else if ('remove' === type) {
            $(id + ' ' + _self.properties.addUserButton).closest('dd').addClass('hide');
            $(id + ' ' + _self.properties.removeUserButton).closest('dd').removeClass('hide');
        }
    };
    ...
    this.add_user = function (userId) {
        _request(_self.properties.addUserUrl, userId, function () {
            if (_self.properties.toggle) {
                ...
                if ($('.likemecontrols').length > 0) {
                    $('.likemecontrols').addClass('hide');
                }
            } else {
    ...
    this.remove_user = function (userId) {
        _request(_self.properties.removeUserUrl, userId, function () {
            if (_self.properties.toggle) {
                ...
                if ($('.likemecontrols').length > 0) {
                    $('.likemecontrols').removeClass('hide');
                }
            } else {
                $(_self.properties.blockId + userId).remove();
            }
