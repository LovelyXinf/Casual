...
class AdminPolls extends \Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(['Menu_model', 'Polls_model']);
    ...
    public function edit($id = 0)
    {
        ...
        $poll_data = $this->Polls_model->getPollById($id);
        ...
        if ($this->input->post('btn_save', true)) {
            ...
            $poll_data     = $post_data;
            $validate_data = $this->Polls_model->validatePoll($post_data);
            if (!empty($validate_data["errors"])) {
                ...
                $new_poll_id = $this->Polls_model->savePoll($save_data);
    ...
    public function results($poll_id = 0, $filter = 'all', $order = 'user_id', $order_direction = 'ASC', $page = 1)
    {
        ...
        $poll_data  = $this->Polls_model->getPollById($poll_id);
        ...
        if ($results_count > 0) {
            $results = $this->Polls_model->getResultsList($page, $items_on_page, [$order => $order_direction], $attrs);
