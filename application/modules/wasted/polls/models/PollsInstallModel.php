...
class PollsInstallModel extends \Model
{
        ...
        'user_footer_menu' => array(
            ...
            'items'  => array(
                'footer-menu-links-item' => array(
                    'action' => 'none',
                    'items'  => array(
                        'footer-menu-polls-item' => array('action' => 'create', 'link' => 'polls',  'status' => 1, 'sorter' => 2),
                    ),
                ),
            ),
        ),
    ...
    public function __construct()
    {
        ...
        $this->access_permissions = Setup::getModuleData(
                PollsModel::MODULE_GID, Setup::TYPE_ACCESS_PERMISSIONS
        );
    }
    ...
    private $moderators_methods = array(
        array('module' => 'polls', 'method' => 'index', 'is_default' => 1, 'group_id' => 7, 'is_hidden' => 0, 'parent_module' => '')
    );
    ...
    public function prepareInstalling()
    {
        ...
        foreach ($this->ci->pg_language->languages as $lang_id => $value) {
            $this->ci->Polls_model->langDedicateModuleCallbackAdd($lang_id);
        }
    }
