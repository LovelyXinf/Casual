...
class KissesModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('kisses', CacheManager::PROVIDER_REDIS);
    }
    ...
    public function getAllKisses()
    {
        if ($this->kisses_all === null) {
            ...
            // TODO: cache
            $this->kisses_all = $this->ci->cache->get('kisses', 'all', function () use ($fields)
            {
                $ci = &get_instance();
    ...
    public function save($id = null, $data)
    {
        ...
        // TODO: cache - clear data
        $this->ci->cache->flash('kisses');
     ...
     * @return array
     */
    public function getList($page = 1, $items_on_page = 100, $params = array(), $order_by = array('sorter' => 'ASC'), $filter_object_ids = null, $filter_object_not_ids = null)
    {
    ...
    public function postUpload($file_name)
    {
        ...
        $result['is_saved'] = $this->saveKissesInternal($data, array());
     ...
     * return error or succes
     */
    private function saveKissesInternal($attrs, $where = array())
    {
    ...
    public function validateKisses($kisses_id, $data)
    {
        ...
        if (!empty($data['message'])) {
            $return['data']['message'] = mb_substr($data['message'], 0, $this->ci->pg_module->get_module_config('kisses', 'number_max_symbols'), 'UTF-8');
    ...
    public function saveUserKisses($data)
    {
        ...
        $kisses_id = $this->ci->db->insert_id();
     ...
     * @return array
     */
    public function getListUserKisses($params = array(), $page = 1, $items_on_page = 20, $order_by = null)
    {
     ...
     * @return array
     */
    public function getCountKissesUsers($params = array())
    {
        if ($this->ci->session->userdata('auth_type') == 'user') {
            $user_id = $this->ci->session->userdata('user_id');
            $this->ci->load->model('Blacklist_model');
