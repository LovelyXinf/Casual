 ...
 */
class FavouritesCallbacksModel extends \Model
{
    ...
    public function addCallback($module, $model, $method, $event_status = '')
    {
        ...
        $this->ci->db->insert(FAVOURITES_CALLBACKS_TABLE, $attrs);
    ...
    {
        $this->ci->db->where('module', $module)->delete(FAVOURITES_CALLBACKS_TABLE);
    ...
    {
        $this->ci->db->where('id', $id)->delete(FAVOURITES_CALLBACKS_TABLE);
    ...
    public function getCallbacks($event_status = '', $module = '')
    {
        ...
        $result = $this->ci->db->select($this->fields_str)->from(FAVOURITES_CALLBACKS_TABLE)->get()->result_array();
