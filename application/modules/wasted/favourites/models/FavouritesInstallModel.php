 ...
 */
class FavouritesInstallModel extends \Model
{
   ...
   /**
    ...
    public function __construct()
    {
        ...
        $this->modules_data = Setup::getModuleData(
                FavouritesModel::MODULE_GID, Setup::TYPE_MODULES_DATA
        );
        $this->access_permissions = Setup::getModuleData(
                FavouritesModel::MODULE_GID, Setup::TYPE_ACCESS_PERMISSIONS
        );
        $this->demo_content = Setup::getModuleData(
                FavouritesModel::MODULE_GID, Setup::TYPE_DEMO_CONTENT
        );
    }
    ...
    public function installMenuLangUpdate($langs_ids = null)
    {
        ...
        $langs_file = $this->ci->Install_model->language_file_read(FavouritesModel::MODULE_GID, 'menu', $langs_ids);
        if (!$langs_file) {
    ...
    public function installSiteMap()
    {
        ...
        $this->ci->Site_map_model->setSitemapModule(FavouritesModel::MODULE_GID, [
    ...
    {
        $this->ci->load->model('Site_map_model');
        $this->ci->Site_map_model->deleteSitemapModule(FavouritesModel::MODULE_GID);
    }

    public function installBanners()
    {
        $this->ci->load->model(['Favourites_model', 'banners/models/Banner_group_model']);
        $this->ci->Banner_group_model->setModule(FavouritesModel::MODULE_GID, "Favourites_model", "bannerAvailablePages");
        $group_id = $this->ci->Banner_group_model->getGroupIdByGid('users_groups');
        $pages = $this->ci->Favourites_model->bannerAvailablePages();
        if ($pages) {
    ...
    public function deinstallBanners()
    {
        ...
        $this->ci->Banner_group_model->deleteModule(FavouritesModel::MODULE_GID);
        $group_id = $this->ci->Banner_group_model->getGroupIdByGid('users_groups');
    ...
    {
        $this->ci->pg_seo->set_seo_module(FavouritesModel::MODULE_GID, [
            'module_gid' => FavouritesModel::MODULE_GID,
            'model_name' => 'Favourites_model',
    ...
    {
        $langs_file = $this->ci->Install_model->language_file_read(FavouritesModel::MODULE_GID, 'arbitrary', $langs_ids);
        if (!$langs_file) {
            log_message('info', 'Empty favourites arbitrary langs data');
            return false;
        }
        foreach ($this->modules_data['seo_pages'] as $page) {
            $this->ci->pg_seo->set_settings('user', FavouritesModel::MODULE_GID, $page, [
    ...
    public function arbitraryLangExport($langs_ids = null)
    {
        ...
        $seo_settings = $this->ci->pg_seo->get_all_settings_from_cache('user', FavouritesModel::MODULE_GID);
    ...
    {
        $this->ci->pg_seo->delete_seo_module(FavouritesModel::MODULE_GID);
    }
    ...
    public function installNotifications()
    {
        ...
        foreach ($this->modules_data['notifications']['templates'] as $tpl) {
            $template_data = [
                'module' => FavouritesModel::MODULE_GID,
                'gid' => $tpl['gid'],
                'name' => $tpl['name'],
    ...
    public function installNotificationsLangUpdate($langs_ids = null)
    {
        ...
        $langs_file = $this->ci->Install_model->language_file_read(FavouritesModel::MODULE_GID, 'notifications', $langs_ids);
        if (!$langs_file) {
