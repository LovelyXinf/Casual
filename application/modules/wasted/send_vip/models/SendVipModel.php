...
class SendVipModel extends \Model
{
    ...
    public function validateTransaction($validate_data, $koef = 0, $transfer_fee = null)
    {
        ...
        $user = $this->ci->Users_model->getUserById($this->ci->session->userdata('user_id'));
    ...
    private function validateMembership($data)
    {
        ...
        $access_type = $this->ci->Access_permissions_settings_model->getSubscriptionType(
            AccessPermissionsSettingsModel::TYPE);
        if ($access_type == 'user_types') {
            $this->ci->Access_permissions_settings_model->getAccessData(
                $this->ci->Access_permissions_model->roles[AccessPermissionsModel::USER]
            )->user_type = $membership_data[1];
        }

        return $this->ci->Access_permissions_settings_model->getAccessData(
            $this->ci->Access_permissions_model->roles[AccessPermissionsModel::USER]
    ...
    public function formatConfirmData(array $data)
    {
        return [
            ...
            'transfer_fee' => number_format($data['data']['transfer_fee'], 2, '.', ''),
    ...
    public function statusTransaction($transaction_id, $status)
    {
        ...
        if ($transaction_data['status'] == 'waiting') {
            ...
            $this->ci->load->model(['access_permissions/models/Access_permissions_groups_model',
                'Access_permissions_model','Users_model']);
            ...
            if ($status == 'approve') {
                ...
                $this->ci->Access_permissions_model->paymentStatus([
                ...
                $user = $this->ci->Users_model->getUserById($transaction_data['id_sender']);
                ...
                $price = $this->ci->Access_permissions_groups_model->getPeriodPrice([
                    'group' => $membership[0],
                    'period' => $membership[1],
                ]);
                ...
                $this->ci->Users_payments_model->updateUserAccount(
                    $payment_data, 
                    1, 
                    'decline_send_vip',
    ...
    {
        $this->ci->load->model(['Users_model', 'notifications/models/Notifications_model']);
        
        $receiver = $this->ci->Users_model->getUserById($data['id_user']);
        
        return $this->ci->Notifications_model->sendNotification(
            $receiver['email'],
            'send_vip_msg',
            [
                ...
                'sender' => $this->ci->Users_model->getUserById($data['id_sender'])['output_name'],
            ],
            '',
            $receiver['lang_id']
         );
    ...
    public function formatTransactionData($data = [])
    {
        ...
        $users_list = $this->ci->Users_model->getUsersListByKey(null, null, null, [], $users_ids);
