...
class SendVip extends \Controller
{
    ...
    public function save()
    {
        ...
        if (!empty($transaction_data['errors']) || !in_array($payment_gid, $payment_gids)) {
            ...
            if ($payment_gid == 'account') {
                ...
                \Pg\modules\payments\helpers\sendPayment('send_vip', $transaction_data['data']['id_sender'],
                    $transaction_data['data']['full_amount'], $currency_gid, $payment_gid, $payment_data, 'form');
            }
        }
    }
    ...
    public function ajaxGetSendVipBlock($user_id = null)
    {
        ...
        exit(\Pg\modules\send_vip\helpers\sendVipBlock(['user_id' => $user_id]));
    }
