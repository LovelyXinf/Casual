...
class ChatboxContactListModel extends \Model
{
    ...
    const TABLE             = DB_PREFIX . 'chatbox_contact_list';
    ...
    public function getByUserIdAndContactId($user_id, $contact_id)
    {
        ...
        $results = $this->ci->db->select($fields_str)
            ->from(self::TABLE)
    ...
    public function validate($data = [])
    {
        ...
        if (isset($data['count_new'])) {
            $return['data']['count_new'] = intval($data['count_new']);
    ...
    public function updateContact($user_id, $contact_id, $count_new = 0)
    {
        $contact_data = $this->getByUserIdAndContactId($user_id, $contact_id);
    ...
    {
        $result = $this->ci->db->select('SUM(count_new) AS cnt')
            ->from(self::TABLE)
            ->where('user_id', $user_id)
            ->get()->result_array();
    ...
    public function getListIm($user_id, $contact = null, $result = array(), $without_mess = false, $only_mess = false) {
        ...
        foreach ($contact_list  as $key => $value) {
            if (!$without_mess) {
                if ((!isset($result['im_messages'][$value['contact_id']])) || 
            ...
            if (!$without_mess) {
                ...
                if (isset($result[$contact['id_contact']])) {
                    $count_im = intval($result[$contact['id_contact']]['count_new']);
                }
                $count_cb = intval($contact['count_new']);
    ...
    public function mergeDataContacts($contact_list = array(), $im_list = array(), $user_id, $is_backend = true) 
    {
        $contact_list_dump = array();
        $im_list_dump = array();
        ...
        if ($im_list) {   
            ...
            foreach ($im_list_dump as $key => $value) {
                $im_count_new = intval($value['count_new']);
                
                if (isset($contact_list_dump[$key])) {
                    $chatbox_count_new = intval($contact_list_dump[$key]['count_new']);
    ...
    public function backendCheckNewMessages($params = [])
    {
        ...
        if ($l_time) {
            ...
            $return['contact_id'] = intval($params['contact_id']);
            $get_messages         = false;
            $im_contacts          = array();   
                ...
                if (!empty($contacts)) {
                    ...
                    foreach ($contacts_chatbox as $key => $value) {
                        if (isset($im_contacts_dump[$value['contact_id']])) {
                            $im_contacts_dump[$value['contact_id']]['count_new'] = intval($im_contacts_dump[$value['contact_id']]['count_new']) + intval($value['count_new']);
            ...
            if (!empty($contacts)) {
                if (empty($im_contacts)) {
                    ...
                    foreach ($contacts as $key => $value) {
                        ...
                        $contacts[$key]['count_new'] = intval($contacts[$key]['count_new']) + intval($im_contacts_dump[$value['contact_id']]['count_new']); 
                    }
                }
            ...
            if ($get_messages) {
                ...
                if (!empty($messages)) {
                    ...
                    foreach ($messages as $key => $item) {
                        if ($item['contact_id'] == $item['contact_id'] && $item['dir'] == 'o') {
                            continue;
                        }
                        $message = [
                            'id'        => $item['id'],
        ...
        if ($this->ci->pg_module->is_module_installed('im')) {
            ...
            $result_im = intval($this->ci->Im_messages_model->get_unread_count($user_id, 'i'));
            $return['count_new'] = $result_im + intval($return['count_new']); 
        }
