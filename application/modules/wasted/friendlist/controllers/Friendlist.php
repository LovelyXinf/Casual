...
class Friendlist extends \Controller
{
    ...
    private function listBlock($type = 'accept', $action = 'view', $page = 1)
    {
        ...
        if ($items_count) {
            $list = $this->Friendlist_model->getList($user_id, $type, $page, $items_on_page, $order_by, $search);
        }
        ...
        $this->view->assign('counts', $this->getCounts());
    ...
    public function index($action = 'view', $page = 1)
    {
        ...
        $this->view->assign('form_action', site_url('friendlist/index'));
        $this->listBlock($this->list_types['friendlist'], $action, $page);
    }

    public function friendsRequests($action = 'view', $page = 1)
    {
        $this->load->model('Menu_model');
        $this->Menu_model->breadcrumbs_set_parent('friendlist_item');
        $this->view->assign('form_action', site_url('friendlist/friends_requests'));
        $this->listBlock($this->list_types['friends_requests'], $action, $page);
    }

    public function friendsInvites($action = 'view', $page = 1)
    {
        $this->load->model('Menu_model');
        $this->Menu_model->breadcrumbs_set_parent('friendlist_item');
        $this->view->assign('form_action', site_url('friendlist/friends_invites'));
    ...
    public function request($id_dest_user, $ajax = false)
    {
        ...
        if (isset($result['success'])) {
            // send notification
            $this->load->model(['Notifications_model', 'Users_model']);
            $dest_user_data = $this->Users_model->getUserById($id_dest_user);
            $user_data = $this->Users_model->getUserById($id_user, true);
            $notification_data = [
                'fname' => $dest_user_data['output_name'],
                'user' => $user_data['output_name'],
                'comment' => $comment,
                'link_1' => $user_data['link'],
                'link_2' => site_url() . 'friendlist/friends_requests',
                'image' => $user_data['media']['user_logo']['thumbs']['middle']
            ];
            $this->Notifications_model->sendNotification($dest_user_data['email'], 'friends_request', $notification_data, '', $dest_user_data['lang_id']);
            if (!$ajax) {
    ...
    public function ajaxRequest($id_dest_user, $mode = 'button')
    {
    ...
    public function ajaxRequestBlock($id_dest_user)
    {
    ...
    public function ajaxAccept($id_dest_user, $mode = 'button')
    {
    ...
    public function ajaxDecline($id_dest_user, $mode = 'button')
    {
    ...
    public function ajaxRemove($id_dest_user, $mode = 'button')
    {
     ...
     * @return array
     */
    public function ajaxGetFriendsData()
    {
        $return = [];

        $user_id = $this->session->userdata('user_id');
        $friends_ids = $this->Friendlist_model->getFriendlistUsersIds($user_id);
     ...
     * @return array
     */
    public function ajaxGetSelectedFriends()
    {
        $return = array();
    ...
    public function ajax_get_friends_form($max_select = 1)
    {
        ...
        if (!empty($selected)) {
            ...
            $selected = array_intersect($selected, $friends_ids);
            $this->load->model('Users_model');
            $selected_users = $this->Users_model->getUsersListByKey(null, null, array('nickname' => 'asc'), [], $selected);
