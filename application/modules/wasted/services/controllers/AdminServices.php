...
class AdminServices extends \Controller
{
    ...
    public function edit($service_id = null)
    {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Services_model->validateService($service_id, $post_data);
            if (!empty($validate_data["errors"])) {
                ...
                $this->Services_model->saveService($service_id, $validate_data["data"], $langs['name'], $langs['description']);

                $this->system_messages->addMessage(View::MSG_SUCCESS, l('success_update_service_data', 'services'));

                $this->load->library('Analytics');
                $event = $this->analytics->getEvent('services', 'edit_service', 'admin');
        ...
        if (empty($data["lds_array"])) {
            $data["lds_array"] = [];
        }

        $this->view->assign('template_block', $this->getTemplateAdminParamBlock($template, $data["price"], $data["data_admin"], $data["can_free"]));
        $this->view->assign('lds_block', $this->getTemplateLdsBlock($template, $data["lds_array"]));
    ...
    public function ajaxGetTemplateAdminParamBlock($template_gid)
    {
        $template = $this->Services_model->get_template_by_gid($template_gid);
        echo $this->getTemplateAdminParamBlock($template);
    }

    private function getTemplateAdminParamBlock($template, $price = 0, $values = "", $can_free = 0)
    {
        if (!empty($values)) {
        ...
        $this->view->assign('can_free', $can_free);
        $this->view->assign('template_data', $template);

        return $this->view->fetch('block_service_param');
    }

    private function getTemplateLdsBlock($template, $values = "")
    {
    ...
    {
        if ($status) {
            $this->Services_model->activateService($service_id);
        } else {
            $this->Services_model->deactivateService($service_id);
        }
    ...
    public function templateEdit($template_id = null)
    {
