...
class ApiServices extends \Controller
{
        ...
        if ($type) {
            ...
            if (!empty($service_return['errors'])) {
                ...
                if (!empty($origin_return['errors'])) {
                    ...
                    if ('account' === $type) {
                        $payment = $this->Services_model->accountPayment($data, $user_id, $user_form_data, $activate_immediately);
                        if ($payment !== true) {
                            $errors[] = $payment;
                        } else {
    ...
    */
    public function get()
    {
        ...
        $this->load->model('Services_model');
        $service = $this->Services_model->getServiceList(array('where' => array('gid' => $gid)), null, null);
