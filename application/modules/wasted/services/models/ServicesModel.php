...
class ServicesModel extends \Model
{
     ...
     * @var string
     */
    const PAYMENT_TYPE_GID_PREFIX = 'services_';
    /**
     * Module GUID
     *
     * @var string
     */
    const MODULE_GID = 'services';
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('services', CacheManager::PROVIDER_REDIS);
        $this->ci->cache->registerService('services_templates', CacheManager::PROVIDER_REDIS);
    }
    ...
    private function getAllServices()
    {
        if ($this->services_all === null) {
            ...
            $this->services_all = $this->ci->cache->get('services', 'all', function () use ($fields)
            {
                $ci = &get_instance();
    ...
    private function getAllTemplates()
    {
        if ($this->templates_all === null) {
            ...
            $this->templates_all = $this->ci->cache->get('services_templates', 'all', function () use ($fields)
            {
                $ci = &get_instance();
    ...
    public function getTemplateList($params = array(), $filter_object_ids = null, $order_by = null, $lang_id = '')
    {
    ...
    public function getTemplateCount($params = array(), $filter_object_ids = null)
    {
    ...
    public function saveTemplate($id, $data, $name = null)
    {
        ...
        if (!empty($name)) {
            $this->ci->pg_language->pages->set_string_langs('services', "template_name_" . $id, $name,
                array_keys($this->ci->pg_language->languages));
        }

        // TODO: cache
        $this->ci->cache->flush('services_templates');
    ...
    public function getServiceList($params = array(), $filter_object_ids = null, $order_by = null, $lang_id = '')
    {
    ...
    public function formatServices($data, $lang_id = '')
    {
        ...
        foreach ($data as $k => $service) {
            if (isset($template_by_gid[$service["template_gid"]])) {
    ...
    public function getServiceCount($params = array(), $filter_object_ids = null)
    {
    ...
    public function saveService($id, $data, $name = [], $description = [])
    {
        ...
        if (!empty($name)) {
            $this->ci->pg_language->pages->set_string_langs('services', 'service_name_' . $id, $name,
                $lang_ids);
        }
        if (!empty($description)) {
            $this->ci->pg_language->pages->set_string_langs('services', 'service_name_' . $id . '_description',
                $description, $lang_ids);
        }
        ...
        // TODO: cache - clear services
        $this->ci->cache->flush('services');
    ...
    {
        $services_data = $this->getServiceList(array('where' => array('id_membership' => $membership_id)));
    ...
    {
        $this->saveService($service_id, array('status' => $status));
    }
