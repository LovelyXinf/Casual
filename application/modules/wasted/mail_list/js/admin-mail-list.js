function adminMailList(optionArr) {
    ...
    this.initControls = function () {
        $(document)
           .off('ifChanged', _self.properties.id.groupingAll).on('ifChanged', _self.properties.id.groupingAll, function () {
            _self.groupingAll(this);
        }).off('click', _self.properties.id.btnSave).on('click', _self.properties.id.btnSave, function () {
            _self.save_filter($(_self.properties.id.searchForm).serializeArray());
        });
    };
    
    this.groupingAll = function (obj) {
        if ($(obj).prop( "checked" )) {
             $("input[data='table_records']").iCheck('check');
        } else {
            $("input[data='table_records']").iCheck('uncheck');
        }
    };
