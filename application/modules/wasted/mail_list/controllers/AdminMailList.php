...
class AdminMailList extends \Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('Menu_model', 'Users_model', 'Mail_list_model'));
    ...
    public function users($filter = 'all', $page = 1, $user_type = 0)
    {
        ...
        $subscriptions = $this->Subscriptions_model->getSubscriptionsList();
        ...
        if ($this->input->post('btn_cancel', true)) {
            ...
            if ($this->input->post('id_filter', true)) {
                $_SESSION['mail_list'] = $this->Mail_list_model->getFilter($this->input->post('id_filter', true));
            }
        ...
        } elseif (empty($current_settings['id_subscription']) && !empty($subscriptions)) {
        ...
        $search_attrs = $this->Mail_list_model->formatSearchAttrs($current_settings);
        $users = $this->Mail_list_model->getUsers($search_attrs, $page, $this->_items_on_page);
    ...
    public function ajaxSaveFilter()
    {
        ...
        if (is_array($data)) {
            $this->Mail_list_model->saveFilter($data);
            $data = ['success' => l('success_settings_saved', 'start')];
            $this->view->assign(['success' => l('success_settings_saved', 'start')]);
        } else {
            $data = ['error' => l('error_system', 'start')];
        }
