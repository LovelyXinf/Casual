    ...
    if (!function_exists('sendMoneyViewBlock')) {
        ...
        function sendMoneyViewBlock()
        {
            ...
            if (!empty($id_users_arr)) {
                $users_arr = $ci->Users_model->getUsersListByKey(null, null, null,
                    null, $id_users_arr);
            }
            if (empty($users_arr)) {
                return [];
            ...
            foreach ($transactions as $key => $transaction) {
                ...
                $transactions[$key]['amount']       = number_format($transactions[$key]['amount'],
                    2, '.', '');
                $transactions[$key]['transfer_fee'] = $transaction['full_amount'] - $transaction['amount'];
                if ($transaction['id_user'] == $id_user) {
                    $transactions[$key]['comment'] = l('send_money_gift_from',
                            'send_money') .
                    ...
                    $transactions[$key]['comment'] = l('send_money_gift_for',
                            'send_money') .  ' <a href="#" data-action="set_user_ids" data-gid="donate" data-href="' . $users_arr[$transaction['id_user']]['link'] . '">' . $users_arr[$transaction['id_user']]['output_name'] . '</a>';
                }
