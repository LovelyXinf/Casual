 ...
 **/
class AdminIncompleteSignup extends \Controller
{
    /**
     * Constructor
     */
    ...
    public function notRegistered($filter = "not_registered", $user_type = 'all', $order = "date_created", $order_direction = "DESC", $page = 1)
    {
        ...
        if ($users_count > 0) {
            $results = $this->Incomplete_signup_model->getUnregisteredUsers($page, $items_on_page, array($order => $order_direction), $attrs);
            $users = $this->Incomplete_signup_model->format($results);
    ...
    public function deleteSelect($user_id)
    {
        $this->load->model('Incomplete_signup_model');
        $this->Incomplete_signup_model->deleteUnregisteredUserById($user_id);
