function Wall() {
    ...
    this.bind_delete = function () {
        $('.js-delete_wall_event').off('click').on('click', function (event) {    
    ...
    this.initGallery = function (event_object) {
        if (event_object) {
            ...
            if (_self.properties.images.length > 1) {
                _self.properties.gallery.create_right(function () {
                    _self.nextGalleryImage();
                    _self.properties.gallery.reposition_load_block();
                });
                _self.properties.gallery.create_left(function () {
    ...
    this.newPost = function (onsuccess) {
        ...
        $.ajax({
            ...
            success: function (resp, textStatus, jqXHR) {
                ...
                if (resp.msg) {
                    var status = resp.error ? 'error' : 'success';
                    error_object.show_error_block(resp.msg, status);
                }
                if (typeof onsuccess == 'function') {
                    onsuccess();
                }
