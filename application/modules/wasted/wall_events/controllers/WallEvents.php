 ...
 * */
class WallEvents extends \Controller
{
    ...
    public function ajaxGetEvents($type = 'all')
    {
        ...
        $result = $this->getEvents($post_data, $use_load, $items_on_page);
        $this->view->assign($result);
        $this->view->render();
    }

    private function getEvents($data, $use_load = false, $items_on_page = null, $page = null)
    {
        $result['status'] = 0;
        $result['error'] = '';
        $result['all_loaded'] = 0;
    ...
    public function ajaxUserPermissions()
    {
        $user_id = $this->session->userdata('user_id');
        $user_perm = $this->Wall_events_permissions_model->getUserPermissions($user_id);
    ...
    public function ajaxPost()
    {
        ...
        $result = $this->post($id_wall, $data, 0, $required_fields);
        $this->view->assign($result);
        $this->view->render();
    }
    ...
    public function postUpload($id_wall, $place = 'homepage')
    {
        ...
        $result = $this->post($id_wall, $data, $id);
    ...
    public function postForm($id_wall, $place = 'homepage')
    {
        $user_id = $this->session->userdata('user_id');
        $id_wall = ($place == 'homepage' || $place == 'myprofile') ? $user_id : intval($id_wall);
        $data['text'] = trim(strip_tags($this->input->post('text', true)));
        ...
        $required_fields = (!$data['text'] && $data['embed_code']) ? array('embed_data') : array('text');
        $this->post($id_wall, $data, 0, $required_fields);
        redirect();
    }

    private function post($id_wall, $data = array(), $id = 0, $required_fields = array())
    {
        $user_id = $this->session->userdata('user_id');
