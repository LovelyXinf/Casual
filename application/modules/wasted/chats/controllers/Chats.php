...
class Chats extends \Controller
{
    ...
    public function call($chat_gid, $method_name)
    {
        ...
        call_user_func_array(array($chat, $method), $args);
        exit;
    }
    ...
    {
        if ((!in_array($this->active->getGid(), array('pg_videochat', 'oovoochat')))) {
            show_404();
        }
    ...
    {
        if (empty($user_id) || (!in_array($this->active->getGid(), array('pg_videochat', 'oovoochat')))) {
            show_404();
        }
    ...
    {
        if (empty($chat_id) || (!in_array($this->active->getGid(), array('pg_videochat', 'oovoochat')))) {
            show_404();
        }
    ...
    public function goToChat($chat_id = 0, $is_oovoo_session = 0)
    {
        ...
        if (empty($chat_id) || (!in_array($chat_gid, array('pg_videochat', 'oovoochat')))) {
        ...
        $this->view->assign('complete_lang', str_replace('[link]', 
            site_url() . 'chats', l('text_complete_lang', 'chats')));
    ...
    private function processDuration($chat, $disable_amount_warning=false) 
    {
        ...
        if ($settings['fee_type'] == 'payed') {
            ...
            if (!$this->active->writeOffForChat($chat['invite_user_id'], str_replace(['[with_name]', '[date_chat]'], 
            ...
            if (!$disable_amount_warning && !empty($chat['available_time'])) {
                ...
                if ($available_time <= 30) {
                    $this->system_messages->addMessage(View::MSG_ERROR, 
                        str_replace('[available_time]', $available_time, 
    ...
    {
        $this->view->assign('messages', $this->active->getMessages($chat_id, 
