...
class ImMessagesModel extends \Model
{
    ...
    public function addMessage($id_user, $id_contact, $text)
    {
        ...
        if ($this->ci->pg_module->is_module_installed('mobile') && $this->ci->pg_module->get_module_config('mobile', 'use_notifications')) {
            $this->ci->load->model('mobile/models/Mobile_model');
            ...
            $this->ci->Mobile_model->pushNotification($id_contact, $title, $text, "ChatActivity", "im", $args);
        }
    ...
    public function backendGetLastMessages($params = array()) {
    ...
    public function getLastMessagesChatbox($chatbox_messages, $id_user, $contact = null, $count = 20, $chatbox_from_date = array()) {
        $params = array();
        $order_by = array();
    ...
    public function getLastMessages($id_user, $contact = null, $count = 20, $for_chatbox_data = false)
    {
        ...
        if ($this->ci->pg_module->is_module_installed('chatbox')) {
            $messages_chatbox = array();
            if (!$contact) {
                $this->ci->load->model('chatbox/models/Chatbox_contact_list_model');
                $messages = $this->ci->Chatbox_contact_list_model->getListIm($id_user, null, array('im_messages' => $messages), false, true);
            } else {
    ...
    public function getNewMessages($id_user, $id_contact, $from_id = null, $from_chatbox_id = null, $from_date_add = array())
    {
        $params['where']['id_user'] = intval($id_user);
        $params['where']['id_contact'] = intval($id_contact);
        if ($from_id) {
