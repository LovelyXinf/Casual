...
class AdminSubscriptions extends \Controller
{
    ...
    public function edit($id = null)
    {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Subscriptions_model->validateSubscription($id, $post_data, $langs_data);
            ...
            if (!empty($validate_data["errors"])) {
                ...
                $this->system_messages->addMessage(View::MSG_SUCCESS, ($id) ? l('success_update_subscription', 'subscriptions') : l('success_add_subscription', 'subscriptions'));
    ...
    public function sendSubscription($id, $page = 1, $limit = 1000)
    {
        if (!$id) {
            return;
        }
    ...
    public function ajaxSendSubscription($id, $page = 1, $limit = 1000)
    {
        $result = $this->sendSubscription($id, $page, $limit);
        exit(json_encode($result));
    }

    public function ajaxStartSubscribe($id)
    {
        if (!$id) {
            return;
        }
        $this->load->model('subscriptions/models/Subscriptions_users_model');
        $this->view->assign('total_users', $this->Subscriptions_users_model->getSubscriptionUsersCount($id));
