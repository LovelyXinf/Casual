   ...
   var isInitialized = false;
    ...
    this.init = function (options) {
        ...
        $('body').append('<div id="' + _self.properties.id + '_full_text" class="shoutbox_full_text" style="display:none;"><div>')
                .on('mouseleave', '#' + _self.properties.id + '_full_text', function () {
                    $('#' + _self.properties.id + '_full_text').fadeOut(0);
                });
    ...
    this.initScroll = function () {
        try {
            _p.msg_chat_obj.slimScroll({railVisible: true, height: '250px', size: '5px', position: _self.properties.position, endScrollY: function () {
                    _p.loadOldMessage();
                }});
        } catch (e) {
        }
    ...
    _p.loadOldMessage = function () {
        if (!_self.properties.isInitialized)
            return;
        if (_self.properties.load_old)
            return;
        ...
        $.ajax({
            ...
            success: function (resp, textStatus, jqXHR) {
                ...
                // TODO: разрешить подгрузку следующих сообщений через 1с
                setTimeout(function () {
    ...
    this.firstOpeningShoutBox = function () {
        $.ajax({
            url: _self.properties.site_url + _self.properties.get_messages_url,
            dataType: 'json',
            success: function (resp) {
                _self.properties.new_msgs = resp.msg;
                ...
                _self.properties.isFirstOpening = true;
            }
        });
    };
    ...
    this.setCounterNewMessages = function () {
        ...
        _p.counter_to = setTimeout(function () {
            _self.setCounterNewMessages();
        }, 500);
        return this;
    };
    ...
    _p.renderMessages = function (messages, type) {
        ...
        for (var i in messages)
            if (messages.hasOwnProperty(i) && typeof messages[i].user_info !== 'undefined') {
                if (!$('#shoutbox_msg_' + messages[i].id).size()) {
                    ...
                    var elem = $('<div id="shoutbox_msg_' + messages[i].id + '" data-msg-id="' + messages[i].id + '" class="shoutbox-contact box-sizing"><div class="shoutbox-contact__image"><a data-action="set_user_ids" data-gid="shoutbox" data-href="' + messages[i].user_info.link + '" href="#"><img src="' + messages[i].user_info.media.user_logo.thumbs.small + '"></a></div><div class="shoutbox-contact__info"><div class="shoutbox_msg_header"><span class="shoutbox_user_name">' + user_name + '</span><span class="date">, ' + messages[i].date_str + '</span></div><div class="shoutbox_msg_body">' + messages[i].message + '</div></div></div>');
                    if (type == 'replace_old') {
                    ...
                    if ((type != 'replace_old') && (type != 'init') && (type != 'replace_my')) {
                        $(elem).addClass('new-msg').animate({backgroundColor: "#FFFFFF"}, {duration: 2000, complete: function () {
                                $(elem).removeClass('new-msg').removeAttr('style')
                            }});
    ...
    _p.initDomObjects = function () {
        ...
        }).off('click', 'input[name="sendbtn"]').on('click', 'input[name="sendbtn"]', function () {
            _self.sendMessage();
    ...
    this.showShoutbox = function () {
        ...
        _self.properties.is_opened = true;
        if (_self.properties.isFirstOpening === false) {
            _self.firstOpeningShoutBox();
        }
        $('.shoutbox-msg-window__textarea').focus();
        return this;
    };
    ...
    this.hideShoutbox = function () {
        ...
        _self.properties.is_opened = false;
        return this;
    };
