    ...
    this.create_album = function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp.status) {
                    var prop = '';
                    if (_self.properties.permissions) {
                       prop = _self.properties.permissions.option[1];
                    }
                    $('#user_albums').append('<div class="clearfix album album-item" id="user_album_' + resp.data.album_id + '"><input type="checkbox"> <span title="' + name + '">' + name + '</span>' +
                        '<div class="fright" data-toggle="tooltip" data-placement="right" title="' +  prop + '">' +
                                            '<i class="fa fa-lock"></i>' +
                                    '</div></div> ');
                    $('#user_album_' + resp.data.album_id).fadeIn('slow');
                    $(_self.properties.albumNameInput).val('');
                    $('#user_title_album').removeClass('hide');
