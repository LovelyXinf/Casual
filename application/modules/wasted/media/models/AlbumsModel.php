...
class AlbumsModel extends \Model
{
     ...
     * @return array
     */
    public function getAlbumsList($params = array(), $filter_object_ids = null, $order_by = null, $page = null, $items_on_page = null, $formatted = true, $lang_id = false)
    {
    ...
    public function getAlbumById($album_id, $lang_id = false, $languages = array())
    {
        $select_attrs = $this->fields;
        if ($lang_id) {
            foreach ($select_attrs as $key => $value) {
    ...
    public function formatAlbums($data)
    {
        ...
        foreach ($data as $key => &$item) {
            ...
            if ($is_owner) {
                $user_type = 'owner';
            } elseif ($this->ci->session->userdata('auth_type') == 'user') {
                $user_type = 'user';
    ...
    public function isUserAlbumOwner($album_id)
    {
    ...
    public function isUserCanAddToAlbum($album_id)
    {
    ...
    public function getAlbumsCount($params = array(), $filter_object_ids = null)
    {
        $this->ci->db->select("COUNT(*) AS cnt");
        $this->ci->db->from(ALBUM_TABLE);
        if (!isset($params['where']['is_default'])) {
    ...
    public function updateAlbumsMediaCount($albums_ids)
    {
        $this->ci->load->model('media/models/Media_album_model');
        //counts all
    ...
    public function deleteUserAlbums($id_user)
    {
    ...
    {
        if (empty($this->media_cache['default_album'][$id_user])) {
            $result = $this->ci->db->select(implode(', ', $this->fields))
                    ...
                    ->get()->result_array();
            if (empty($result)) {
                $this->media_cache['default_album'][$id_user] = $this->addDefaultAlbum($id_user);
            } else {
                $this->media_cache['default_album'][$id_user] = array_shift($result);
            }
        }

        return $this->media_cache['default_album'][$id_user];
    }
    ...
    public function langDedicateModuleCallbackAdd($lang_id = false)
    {
        if ($lang_id) {
            ...
            $this->ci->dbforge->add_column(ALBUM_TABLE, [
                'lang_' . $lang_id => [
                    'type' => 'VARCHAR',
                    'constraint' => '255',
                    'null'       => false
    ...
    public function __call($name, $args)
    {
        $methods = [
            ...
            'delete_user_albums' => 'deleteUserAlbums',
            'format_albums' => 'formatAlbums',
            'get_album_by_id' => 'getAlbumById',
            'get_albums_count' => 'getAlbumsCount',
            'get_albums_list' => 'getAlbumsList',
            'get_default' => 'getDefault',
            'is_user_album_owner' => 'isUserAlbumOwner',
            'is_user_can_add_to_album' => 'isUserCanAddToAlbum',
            ...
            'update_albums_media_count' => 'updateAlbumsMediaCount',
            'validate_album' => 'validateAlbum',
        ];
