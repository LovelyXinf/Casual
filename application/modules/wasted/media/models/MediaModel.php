...
class MediaModel extends \Model
{
    ...
    public function getMediaById($media_id, $formatted = true, $format_owner = false, $format_user = false)
    {
    ...
    public function getMediaByUserId($id_user, $type = 'id_user')
    {
        if ($type === 'id_owner') {
    ...
    public function copyMedia($media_id)
    {
    ...
    public function validateImage($data, $file_name = '')
    {
        ...
        if (!empty($file_name)) {
            if (isset($_FILES[$file_name]) && is_array($_FILES[$file_name]) && !($_FILES[$file_name]["error"])) {
                ...
                $return["data"]["description"] = !empty($data["descriptions"][$file_return['data']['file_name']]) ? $data["descriptions"][$file_return['data']['file_name']] : $data["description"];
                $bw_count                      = $bw_count || $this->ci->Moderation_badwords_model->
                        check_badwords($this->moderation_type, $return["data"]["description"]);
    ...
    public function validateVideo($data, $video_name = '')
    {
        ...
        if (!empty($data["embed_code"])) {
            ...
            $embed_data = $this->ci->videoembed->get_video_data($data["embed_code"]);
            if ($embed_data !== false) {
    ...
    function validateAudio($data, $audio_name = '')
    {
        $return = array('errors' => array(), 'data' => array(), 'form_error' => 0);
            ...
            if (isset($_FILES[$audio_name]) && is_array($_FILES[$audio_name])) {
                if (empty($_FILES[$audio_name]["error"])) {
                    ...
                    if (!empty($audio_return["error"])) {
                        $return["errors"][] = (is_array($audio_return["error"])) ? implode("<br>",
                                $audio_return["error"]) : $audio_return["error"];
                    }
     ...
     * @return bool
     */
    public function saveImage($id, $data, $file_name = "", $moderation = false, $indicator = true)
    {
        $return = array('errors' => '');
        ...
        if (!empty($file_name) && !empty($id)) {
            ...
            $img_return = $this->ci->Uploads_model->{$upload_method}($this->file_config_gid, $data['id_owner'],
                $file_name);
            if (empty($img_return["errors"])) {
                ...
                $upload                         = $this->ci->Uploads_model->format_upload($this->file_config_gid,
                    $data['id_owner'], $img_return["file"]);
                ...
                if ((!$moderation || $moderation != 2) && !empty($data['status'])) {
     ...
     * @return bool
     */
    public function saveLocalImage($id, $data, $file_path = "", $moderation = false)
    {
        $return = array('errors' => '');
    ...
    public function updateChildren($id_parent, $data)
    {
        if (empty($id_parent)) {
            return false;
        }
    ...
    public function updateChildrenPermissions($id_media, $permissions = null)
    {
     ...
     * @return bool
     */
    public function saveVideo($id, $data, $video_name = "", $create_event = false)
    {
        $return = ['errors' => ''];
        ...
        if (!empty($video_name) && !empty($id) && isset($_FILES[$video_name]) && is_array($_FILES[$video_name]) && is_uploaded_file($_FILES[$video_name]["tmp_name"])) {
            $this->ci->load->model("Video_uploads_model");
            $video_return = $this->ci->Video_uploads_model->upload($this->video_config_gid, $data['id_owner'],
                $video_name, $id, $data, 'generate');
    ...
    public function saveAudio($id, $data, $file_name = "", $create_event = false, $indicator = true)
    {
        $return       = array('errors' => '');
        $create_event = true;
        ...
        if (!empty($file_name) && !empty($id)) {
            $this->ci->load->model("Audio_uploads_model");
            $audio_return = $this->ci->Audio_uploads_model->upload($this->audio_config_gid, $data['id_owner'],
                $file_name, $id, $data, 'generate');
     ...
     * @return array
     */
    public function getMedia($page = 1, $items_on_page = 20, $order_by = null, $params = array(), $filter_object_ids = null, $format_items
    = true, $format_owner = false, $format_user = false, $safe_format = false)
    {
        ...
        if ($this->ci->session->userdata('auth_type') == 'user') {
            ...
            if ($blocked_ids = $this->ci->Blacklist_model->getBlockedIds($user_id)) {
                $params["where_not_in"]["id_owner"] = $blocked_ids;
            }
        }
    ...
    public function getMediaByKey($page = 1, $items_on_page = 20, $order_by = null, $params = array(), $filter_object_ids
    = null, $format_items = true, $format_owner = false, $format_user = false, $safe_format = false)
    {
        $media        = $this->get_media($page, $items_on_page, $order_by, $params, $filter_object_ids, $format_items,
            $format_owner, $format_user, $safe_format);
        $media_by_key = array();
        foreach ($media as $m) {
            $media_by_key[$m['id']] = $m;
        }
     ...
     * @return array
     */
    public function getMediaIds($page = 1, $items_on_page = 20, $order_by = null, $params = array(), $filter_object_ids = null)
    {
        $objects = array();
    ...
    public function formatItems($data, $format_owner = false, $format_user = false, $safe_format = false)
    {
        ...
        foreach ($data as $key => &$item) {
            if ($item['upload_gid'] == $this->file_config_gid) {
                if (!empty($item["mediafile"]) && $item['permissions'] > self::PERM_OMIT) {
                    $item["media"]["mediafile"] = $this->ci->Uploads_model->format_upload($item['upload_gid'],
                        $item['id_owner'], $item['mediafile']);
                } else {
                    $item["media"]["mediafile"] = $this->ci->Uploads_model->format_default_upload($item['upload_gid']);
                }
                $item["media"]["mediafile"]['photo_title'] = str_replace('[id]', $item['id'],
                    l('text_media_photo', 'media'));
                $item["media"]["mediafile"]['photo_alt']   = str_replace('[id]', $item['id'],
                    l('text_media_photo', 'media'));
                ...
                if (!empty($item["media_video"])) {
                    if (!isset($item["media_video_data"]) || !is_array($item["media_video_data"])) {
                        $item["media_video_data"] = [];
                    }
                    ...
                    $item["video_content"] = $this->ci->Video_uploads_model->format_upload($this->video_config_gid,
                        $item['id_owner'], $media_video, $item['media_video_image'],
                        $item['media_video_data']['data']['upload_type'], $item["media_video_data"]['data']['isHTML5']);
                }
            } elseif ($item['upload_gid'] == $this->audio_config_gid) {
                if ($this->ci->pg_module->is_module_active('audio_uploads')) {
                    $item["media"]["mediafile"] = $this->ci->Audio_uploads_model->format_upload($this->audio_config_gid,
                        $item['id_owner'], $item['mediafile'], '', false, $item['id']);
                }
            }
            if ($this->format_user) {
                if ($format_user) {
            ...
            if ($this->format_likes_count) {
                ...
                if ($this->user_id) {
                    $is_liked         = count($this->ci->Likes_model->get_likes_by_user($this->user_id,
                                'media' . $item['id'])) > 0 ? 1 : 0;
    ...
    public function getMediaCount($params = array(), $filter_object_ids = null)
    {
        if ($this->ci->session->userdata('auth_type') == 'user') {
            $user_id = $this->ci->session->userdata('user_id');
            $this->ci->load->model('Blacklist_model');
        ...
        if (!empty($media['media_video'])) {
            ...
            if ($media['id_user'] == $media['id_owner']) {
                if ($media['media_video'] !== 'embed' && $media['upload_gid'] != 'gallery_audio') {
                    ...
                    $this->ci->db->delete(MEDIA_TABLE);
                    $this->ci->load->model('Video_uploads_model');
                    $this->ci->Video_uploads_model->delete_upload($this->video_config_gid, $media['id_owner'],
                        $media['media_video'], $media['media_video_image']);
                } elseif ($media['media_video'] === 'embed') {
                    $this->ci->load->model('Video_uploads_model');
                    $this->ci->Video_uploads_model->delete_embed_video_image($this->video_config_gid,
                        $media['id_owner'], $media['media_video_image']);
                }
            }
        } elseif ($media['upload_gid'] == 'gallery_audio') {
            $this->ci->load->model('Audio_uploads_model');
            $this->ci->Audio_uploads_model->delete_upload($this->audio_config_gid, $media['id_owner'],
                $media['mediafile']);
        }
    ...
    public function getParentMediaListIds($id)
    {
        if (empty($this->media_cache['parent_media_list_ids'][$id])) {
            $this->ci->db->select('id');
            $this->ci->db->from(MEDIA_TABLE);
            ...
            $this->media_cache['parent_media_list_ids'][$id] = $objects;
        }

        return $this->media_cache['parent_media_list_ids'][$id];
    }
     ...
     * @return
     */
    public function unmarkAdult($object_id)
    {
        $attrs['is_adult'] = 0;
        $this->save_image($object_id, $attrs);
    ...
    public function getList($user_id, $param = 'all', $page = 1, $album_id = 0, $use_permissions = true, $order_by = array(
        'date_add' => 'DESC'))
    {
        ...
        $show_adult = ($user_id && $user_id == $this->user_id) ? true : $this->ci->session->userdata('show_adult');
        if (!$show_adult) {
        ...
        $media                   = (!$album_id || $media_ids) ? $this->get_media($exists_page, $items_on_page,
                $order_by, $where, $media_ids) : array();
        $tpl_data['media']       = $media;
        $tpl_data['media_count'] = $media_count;
        $tpl_data['user_id']     = $this->user_id;
        $tpl_data['subsection']  = $param;
    ...
    public function getGalleryList($count, $param = 'all', $loaded_count = 0, $album_id = 0, $order_by = ['date_add' => 'DESC'], $params
    = [])
    {
        ...
        if (!$this->ci->session->userdata('show_adult')) {
        ...
        if (!$album_id || $media_ids) {
            $media_count = $this->getMediaCount($params, $media_ids);
        ...
        $return['media'] = (!$album_id || $media_ids) ? $this->get_media(null, null, $order_by, $params, $media_ids,
    ...
    public function getAlbums($id_user, $page = 1)
    {
        $return                           = array('content' => '', 'have_more' => 0);
        $this->ci->load->model('media/models/Albums_model');
        $params                           = array();
        ...
        $albums = $this->ci->Albums_model->get_albums_list($params, null, null, $exists_page, $items_on_page, true,
            $lang_id);
    ...
    public function getAlbumsSelect($id_user)
    {
        $this->ci->load->model('media/models/Albums_model');
    ...
    public function getMediaPositionInfo($media_id, $param = 'all', $album_id = 0, $user_id = 0, $use_permissions = true, $order_by
    = array('date_add' => 'DESC'), $filter_duplicate = false)
    {
        ...
        $show_adult = ($this->user_id && $user_id == $this->user_id) ? true : $this->ci->session->userdata('show_adult');
        if (!$show_adult) {
            $where['where']['is_adult'] = 0;
        }
        switch ($param) {
    ...
    public function incrementMediaViews($media_id, $delta = '1')
    {
    ...
    public function decrementMediaViews($media_id, $delta = '1')
    {
        $this->db->set('views', 'views - ' . $delta, false);
        $this->ci->db->where('id', $media_id);
        $this->ci->db->update(MEDIA_TABLE);
    ...
    public function isUserMediaOwner($media_id)
    {
    ...
    public function isUserMediaUser($media_id)
    {
        $this->ci->db->select("COUNT(*) AS cnt");
        $this->ci->db->from(MEDIA_TABLE);
        $this->ci->db->where('id', $media_id);
    ...
    public function isAccessPermitted($media_id, $media = array())
    {
        if (!$media) {
            $media = $this->get_media_by_id($media_id);
        }
    ...
    public function getUserPermissions($id_owner)
    {
        if (!$this->user_id) {
     ...
     * @return string
     */
    public function callbackReviews($action, $data)
    {
        $id = $data['id_object'];
        switch ($action) {
            case 'update':
    ...
    private function dynamicBlockGetUsersMedia($type, $params, $view, $width, $gallery_params = array())
    {
        ...
        if (!empty($params['title_' . $this->ci->pg_language->current_lang_id])) {
            $data['title'] = $params['title_' . $this->ci->pg_language->current_lang_id];
        } elseif (!empty($params['title_' . $this->ci->pg_language->current_lang['code']])) {
            $data['title'] = $params['title_' . $this->ci->pg_language->current_lang['code']];
        } else {
            $data['title'] = '';
        }
        $data['media'] = $this->get_gallery_list($params['count'], $type, 0, 0, array('date_add' => 'DESC'),
            $gallery_params);
    ...
    public function dynamicBlockGetUsersPhotos($params, $view, $width)
    {
        return $this->dynamic_block_get_users_media('photo', $params, $view, $width);
    }

    public function dynamicBlockGetUsersVideos($params, $view, $width)
    {
    ...
    public function formatWallEvents($events, $use_spam = true)
    {
        $is_permissions_allowed = function ($media, $friends_ids, $user_id, $show_adult) {
            ...
            $perm = $user_id ? MediaModel::PERM_REGISTERED : MediaModel::PERM_ALL;
            if (isset($friends_ids[$media['id_owner']])) {
                $perm = MediaModel::PERM_FRIENDS;
            }
            if ($media['id_owner'] == $user_id) {
                $perm = MediaModel::PERM_OMIT;
            }
        ...
        if ($media_ids) {
            ...
            if ($medias) {
                ...
                foreach ($events as $key => $e) {
                    if (is_array($e['data'])) {
                        foreach ($e['data'] as $mkey => $media) {
                            if (!empty($medias[$media['id']]) && $is_permissions_allowed($medias[$media['id']],
                                    $friends_ids, $this->user_id, $this->ci->session->userdata('show_adult'))) {
                    ...
                    if ($e['data']) {
                        ...
                        $e['html_delete']       = "<span class='spam_object_delete'>" . l("error_is_deleted_wall_events_object",
                                "spam") . "</span>";
                        $formatted_events[$key] = $e;
                    }
                }
    ...
    private function createWallEvent($media_id)
    {
        $media = $this->get_media_by_id($media_id);
    ...
    public function getSitemapXmlUrls($generate = true)
    {
        ...
        foreach ($types as $type => $priority) {
            $user_settings = $this->ci->pg_seo->get_settings('user', 'media', $type);
            ...
            case 'get_content':
                ...
                foreach ($data as $id) {
                    if (($this->get_media_count(array('where_in' => array('id' => $id)))) == 0) {
                        $return[$id]["content"]["view"] = $return[$id]["content"]["list"] = "<span class='spam_object_delete'>" . l("error_is_deleted_media_object",
                                "spam") . "</span>";
                    ...
                    if ($media["upload_gid"] == "gallery_image") {
                        ...
                        if (preg_match_all("/id=\"default_player_([0-9]+)\">/is", $event['html'], $match)) {
                            ...
                            foreach ($match[1] as $key => $rand) {
                                $media['video_content']['embed'] = str_replace($rand, $rand . "_" . $key,
    ...
    public function getRecentMedia($count = '16', $param = 'all', $attrs = null)
    {
        return $this->get_gallery_list($count, $param, null, null, $order_by = array('date_add' => 'DESC'), $attrs);
    }

    public function callbackUserDelete($id_user, $type)
    {
        $this->deleteMediaByUserId($id_user, $type);
    }
     ...
     * @return void
     */
    protected function updateMediaData(array $data, array $params)
    {
        if (isset($params["where"]) && is_array($params["where"]) && count($params["where"])) {
            foreach ($params["where"] as $field => $value) {
                $this->ci->db->where($field, $value);
    ...
    private function deleteMediaByUserId($id_user, $type)
    {
        if ($type === 'gallery') {
     ...
     * @return array
     */
    public function saveAviaryFile($photo_id, $data, $file_url, $use_moderation)
    {
     ...
     * @return void
     */
    public function saveAviary($url, $data)
    {
        if (empty($data['id'])) {
            show_404();
        }
     ...
     * @return void
     */
    public function installRatingsFields($fields = array())
    {
        if (empty($fields)) {
            return;
        }
     ...
     * @return void
     */
    public function deinstallRatingsFields($fields = array())
    {
        if (empty($fields)) {
            return;
        }
    ...
    public function getRatingObjectById($id, $formatted = false, $safe_format = false)
    {
    ...
    public function rotation($photo_id, $rotate)
    {
        $result = ['data' => [], 'status' => 1, 'msg' => [], 'errors' => []];
        $media  = $this->getMediaById($photo_id);
        if ($rotate < 0) {
            $rotate += 360;
        } else if ($rotate != 'hor') {
        ...
        if ($media && $media['id_owner'] == $this->user_id) {
            $this->ci->load->model('Uploads_model');
            $this->ci->Uploads_model->rotateUpload($this->file_config_gid, $media['id_owner'], $media['mediafile'],
                $rotate);
    ...
    {
        return $this->getMediaIds(null,null,null,[
            'where' => [
                'id_owner' => $user_id,
                'status' => '0'
            ]
    ...
    {
        $methods = [
            '_dynamic_block_get_users_photos' => 'dynamicBlockGetUsersPhotos',
            '_dynamic_block_get_users_videos' => 'dynamicBlockGetUsersVideos',
            '_format_wall_events' => 'formatWallEvents',
            'get_seo_settings' => 'getSeoSettings',
            '_moder_get_list' => 'moderGetList',
            '_moder_mark_adult' => 'moderMarkAdult',
            '_moder_set_status' => 'moderSetStatus',
            'callback_ratings' => 'callbackRatings',
            'callback_reviews' => 'callbackReviews',
            ...
            'create_wall_event' => 'createWallEvent',
            'decrement_media_views' => 'decrementMediaViews',
            'deinstall_ratings_fields' => 'deinstallRatingsFields',
            'delete_media' => 'deleteMedia',
            'delete_media_by_user_id' => 'deleteMediaByUserId',
            'dynamic_block_get_users_media' => 'dynamicBlockGetUsersMedia',
            'format_items' => 'formatItems',
            'get_albums' => 'getAlbums',
            'get_albums_select' => 'getAlbumsSelect',
            'get_gallery_list' => 'getGalleryList',
            'get_list' => 'getList',
            'get_media' => 'getMedia',
            'get_media_by_id' => 'getMediaById',
            'get_media_by_key' => 'getMediaByKey',
            'get_media_by_user_id' => 'getMediaByUserId',
            'get_media_count' => 'getMediaCount',
            'get_media_ids' => 'getMediaIds',
            'get_media_position_info' => 'getMediaPositionInfo',
            'get_media_type_by_id' => 'getMediaTypeById',
            'get_parent_media_list_ids' => 'getParentMediaListIds',
            'get_rating_object_by_id' => 'getRatingObjectById',
            'get_recent_media' => 'getRecentMedia',
            'get_sitemap_urls' => 'getSitemapUrls',
            'get_sitemap_xml_urls' => 'getSitemapXmlUrls',
            'get_user_permissions' => 'getUserPermissions',
            'increment_media_views' => 'incrementMediaViews',
            'install_ratings_fields' => 'installRatingsFields',
            'is_access_permitted' => 'isAccessPermitted',
            'is_user_media_owner' => 'isUserMediaOwner',
            'is_user_media_user' => 'isUserMediaUser',
            'mark_adult' => 'markAdult',
            'request_seo_rewrite' => 'requestSeoRewrite',
            'save_audio' => 'saveAudio',
            'save_aviary' => 'saveAviary',
            'save_aviary_file' => 'saveAviaryFile',
            'save_image' => 'saveImage',
            'save_local_image' => 'saveLocalImage',
            'save_video' => 'saveVideo',
            'spam_callback' => 'spamCallback',
            'unmark_adult' => 'unmarkAdult',
            'update_children' => 'updateChildren',
            'update_children_permissions' => 'updateChildrenPermissions',
            'validate_audio' => 'validateAudio',
            'validate_image' => 'validateImage',
            'validate_video' => 'validateVideo',
