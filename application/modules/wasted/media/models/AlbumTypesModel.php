...
class AlbumTypesModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('media_album_types', CacheManager::PROVIDER_REDIS);
    }
    ...
    private function getAllTypes() 
    {
        if ($this->types_all == null) {
            ...
            // TODO: cache
            $this->types_all = $this->ci->cache->get('media_album_types', 'all', function () use ($fields)
            {
                $ci = &get_instance();

                $results = $ci->db->select(implode(", ", $this->fields))
    ...
    public function saveType($type_id, $data)
    {
        ...
        // TODO: cache
        $this->ci->cache->flush('media_album_types');
