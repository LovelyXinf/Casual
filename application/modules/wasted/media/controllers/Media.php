...
class Media extends \Controller {
    ...
    private function addMediaTpl($type) {
    ...
    private function toFavorites($media) {
        $this->load->model('media/models/Albums_model');
        $this->load->model('media/models/Media_album_model');
    ...
    public function saveImage() {
        $return = array('errors' => array(), 'warnings' => array(), 'name' => '', 'success' => array());
    ...
    public function ajaxGetUserRecentMedia() {
        ...
        if ($user_id) {
            ...
            if (!empty($data['media'])) {
                ...
                $result = array('html' => $this->view->fetch('user_media_block' . $params['template'], 'user', 'media'));
                $this->view->assign($result);
            }
        }
