function media(optionArr) {
    ...
    this.init_item_block_info = function () {
        $('#' + _self.properties.galleryContentDiv)
            ...
            var cont_id = contentObj.show_load_block('<div class="center"><div class="alert_text">'+alert_text+'</div><button data-href="'+ajax_link+'" class="alert_ok alert_ok_delete btn btn-primary mt20 mlr20">'+_self.properties.btnOk+'</button> <button class="alert_cancel alert_cancel_delete btn btn-cancel mt20 mlr20">'+_self.properties.btnCancel+'</button></div>');
            
            $('.alert_ok.alert_ok_delete').unbind('click').bind('click', function(){
                var href = $(this).data('href');
                 _self.delete_media(href);
                $('.load_content_close').click();
                $('#' + cont_id).parent().hide();
            });
            
            $('.alert_cancel.alert_cancel_delete').unbind('click').bind('click', function(){
                $('.load_content_close').click();
                $('#' + cont_id).parent().hide();
            });
        })
        ...
        $.ajax({
            ...
            success: function (data) {
                    if (typeof data.info.access_denied !== 'undefined' && data.info.access_denied.length > 0) {
                        _self.properties.errorObj.show_error_block(data.info.access_denied, 'info');
    ...
    this.refresh_recent_photos = function () {
        $.ajax({
            ...
            type: 'POST',
            data: {count: _self.properties.recentPhotos, upload_gid: 'photo', id_user: _self.properties.idUser, 'template': _self.properties.recentTemplate},
            dataType: "JSON",
            cache: false,
            success: function (data) {
                if (typeof data.info.access_denied !== 'undefined' && data.info.access_denied.length > 0) {
