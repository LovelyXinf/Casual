...
function winks(optionArr) {
    ...
    this.properties = {
        ...
        wink_button_rand: '',
        repliedWinksClass: '.replied-winks',
    ...
    this.hideRepliedWinks = function () {
        $(_self.properties.repliedWinksClass).each(function(k, el){
            setTimeout(function(){
                $(el).fadeOut(5000);
            }, k*5000);
        });
    ...
    this.bindEvents = function () {
        ...
        $(_self.properties.btnWink).on('click', function () {
            var btnObj = $(this);
            if (!btnObj.data(_self.properties.dataPending) && !btnObj.hasClass('link-disabled')) {
                ...
                request(_self.properties.winkUrl, {user_id: destUser, type: 'new'})
                        .done(function (data) {
                            if (data['errors']) {
                                ...
                                sendAnalytics('winks_send_message', 'communication', 'user');
                                _self.bindEvents();
                            }
                        }).fail(function (data) {
                    console.error(data);
