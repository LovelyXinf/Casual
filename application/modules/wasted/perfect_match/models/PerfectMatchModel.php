...
class PerfectMatchModel extends \Model
{
    ...
    const MODULE_GID = 'perfect_match';
    ...
    {
        $users_perfect_match = $this->ci->db->select(implode(',', $this->fields_all))
        ...
        foreach ($users_perfect_match as $upm) {
            ...
            if (!$for_save) {
                ...
                if (!is_array($upm['full_criteria']['looking_user_type'])) {
                    if (isset($upm['full_criteria']['looking_user_type']) && strpos($upm['full_criteria']['looking_user_type'], ',') !== false) {
    ...
    public function validate($data, $type = 'save')
    {
        ...
        if (!empty($data["looking_lat"])) {
            $return["data"]["looking_lat"] = floatval($data["looking_lat"]);
        } elseif (!empty($data["lat"])) {
            $return["data"]["looking_lat"] = floatval($data["lat"]);
        }
        
        if (!empty($data["looking_lon"])) {
            $return["data"]["looking_lon"] = floatval($data["looking_lon"]);
        } elseif (!empty($data["lon"])) {
            $return["data"]["looking_lon"] = floatval($data["lon"]);
        }
    ...
    public function cronSendMatches()
    {
		...
		$this->ci->load->model(['Mobile_model', 'Users_model', 'Field_editor_model', 'field_editor/models/Field_editor_forms_model']);
        ...
        $users_data = $this->ci->Users_model->getUsersListByKey(null, null, null, ['where' => ['activity' => 1]], $new_user_ids);
        ...
        foreach ($users_data as $user) {
            ...
            $criteria = array_merge_recursive($this->ci->Field_editor_forms_model->get_search_criteria(
                $this->perfect_match_form_gid, $full_criteria, $this->form_editor_type, false), $common_criteria);
			...
			foreach ($matched_users as $matched_user) {
				if (!isset($matches[$matched_user['id']])) {
                    ...
                    $match_ids[$user['id']][] = $matched_user['id'];
				}
				$matches[$matched_user['id']]++;				
			}
			if (!empty($match_ids[$user['id']])) {
                $this->setListMatchIds(['id_user' => $user['id'], 'match_ids' => serialize($match_ids[$user['id']])]);
                unset($match_ids[$user['id']]);
            }
        }
        ...
        foreach ($matches as $user_id => $count) {
            ...
            $this->ci->Mobile_model->pushNotification($user_id, $title, $text, "", "", ['count' => $count], site_url() . 'perfect_match/index');
		}
