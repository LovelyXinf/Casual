...
class AdminNews extends \Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(["Menu_model", "News_model"]);
        $this->Menu_model->setMenuActiveItem('admin_menu', 'content_items');
    }
    ...
    public function edit($news_id = null, $section_gid = 'text')
    {
        ...
        $data = [];
        if ($news_id) {
            $data = $this->News_model->getNewsById($news_id, array_keys($languages));
        }
            ...
            switch ($section_gid) {
                case 'text':
                    ...
                    foreach ($languages as $lid => $lang_data) {
                        $post_data["content"][$lid] = $this->input->post('content_' . $lid);
                    }
                    $validate_data = $this->News_model->validateNews($news_id, $post_data, 'news_icon', 'news_video');
                    if (!empty($validate_data["errors"])) {
                        ...
                        if ($this->input->post('news_video_delete') && $news_id && $data["video"]) {
                            ...
                            $format = $this->News_model->format_single_news($data);
                            $this->Video_uploads_model->deleteUpload($this->News_model->video_config_id, $format["prefix"], $format["video"], $format["video_image"], $format["video_data"]["data"]["upload_type"]);
                ...
                case 'seo':
                    ...
                    foreach ($seo_fields as $key => $section_data) {
                        if ($this->input->post('btn_save_' . $section_data['gid'])) {                            
                            ...
                            $validate_data = $this->Seo_advanced_model->validateSeoTags($news_id, $post_data);
                            if (empty($validate_data['errors'])) {
                                $news_data['id_seo_settings'] = $this->Seo_advanced_model->saveSeoTags($data['id_seo_settings'], $validate_data['data']);
                                if (!$data['id_seo_settings']) {
                                    $data['id_seo_settings'] = $news_data['id_seo_settings'];
                                    $this->News_model->saveNews($news_id, $news_data);
                                }
        ...
        switch ($section_gid) {
            case 'text':
                ...
                foreach ($languages as $lid => $lang_data) {
                     if (is_array($data['content']) && !$data['content_' . $lid]) {
                        if ($data['content'][$lid]) {
                            $data['content_' . $lid] = $data['content'][$lid];
                        }
                    ...
                    if (is_array($data['name']) && !$data['name_' . $lid]) {
                        if ($data['name'][$lid]) {
                            $data['name_' . $lid] = $data['name'][$lid];
                        }
                       
                    }
                    if (is_array($data['annotation']) && !$data['annotation_' . $lid]) {
                        if ($data['annotation'][$lid]) {
                            $data['annotation_' . $lid] = $data['annotation'][$lid];
                        }
    ...
    public function feedEdit($id = null)
    {
        $this->load->model("news/models/Feeds_model");
        if ($id) {
            $data = $this->Feeds_model->getFeedById($id);
        } else {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Feeds_model->validateFeed($id, $post_data);
            if (!empty($validate_data["errors"])) {
                ...
                $id = $this->Feeds_model->saveFeed($id, $validate_data["data"]);
     ...
     * @return void
     */
    public function feedActivate($id, $status = 0)
    {
        $this->load->model("news/models/Feeds_model");
        $id = intval($id);
        if (!empty($id)) {
     ...
     * @return void
     */
    public function feedParse($id)
    {
        $this->load->model("news/models/Feeds_model");
        if (!empty($id)) {
            $feed_data = $this->Feeds_model->getFeedById(intval($id));
            $content = $this->Feeds_model->getFeedContent($feed_data["link"], $feed_data["max_news"]);
            if (!empty($content["errors"])) {
                $this->system_messages->addMessage(View::MSG_ERROR, $content["errors"]);
            } else {
                $saved_news = $this->Feeds_model->saveFeedNews($id, $content["items"]);
                if ($saved_news) {
