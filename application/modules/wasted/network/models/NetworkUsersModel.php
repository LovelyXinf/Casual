...
class NetworkUsersModel extends NetworkModel
{
    ...
    private function getTempByAddIn()
    {
        $temp_data = $this->ci->db->select($this->db_fields)
            ->from(NET_TEMP_TABLE)
            ->where('type', self::TYPE_IN)
            ->where('action', self::ACTION_ADD)
            ->order_by('created ASC')
            ->limit($this->process_temp_limit[self::ACTION_ADD])
            ->get()->result_array();

        return $this->validateTempBySettings($temp_data);
    }

    private function validateTempBySettings(array $temp_data)
    {
        if (empty($temp_data)) {
            return array();
        }

        $config = $this->getConfig($this->cfg_filter);
        $search_fields = array('user_type', 'orientation', 'lang', 'land', 'country_code');
        $new_search = array();
        $new_temp_data = array();
        ...
        if (isset($config['lang'])) {
            foreach ($config['lang'] as $key => $value) {
                $config['lang'][$key] = strtolower($value);
            }
        }

        foreach ($temp_data as $key => $value) {
            foreach (unserialize($value['data']) as $key_d => $value_d) {
                if (in_array($key_d, $new_search)) {
                    if (!in_array($value_d, $config[$key_d])) {
                        $new_temp_data['type'][$value['id']] = self::TYPE_OUT;
    ...
    {
        $temp_data = $this->validateTempBySettings($temp_data);
            ...
            if ($is_upload_photos) {
                ...
                foreach ($photos as $photo) {
                    ...
                    $img_data = array(
                        ...
                        "settings" => serialize(array("width" => $image_size[0],
                            "height" => $image_size[1])),
                        "status" => 1,
                    );
    ...
    public function formatUserIn(array $user)
    {
        ...
        if (isset($user['region'])) {
            $id_region                  = $this->formatRegionIn(
                $user['region'],
                isset($user['country_code']) ? $user['country_code'] : null
            );
        ...
        if (isset($user['city'])) {
            $id_city                  = $this->formatCityIn(
                $user['city'],
                isset($formated_user['id_region']) ? $formated_user['id_region'] : null,
                isset($user['country_code']) ? $user['country_code'] : null
            );
    ...
    public function getFormFields($lang_id = null)
    {
        ...
        return array(
            array('site_type' => $this->ci->pg_language->ds->get_reference(
                self::MODULE_GID,
                'site_type',
                $lang_id
            )),
            array('user_type' => $this->ci->pg_language->ds->get_reference(
                self::MODULE_GID,
                'user_type',
                $lang_id
            )),
            array('orientation' => $this->ci->pg_language->ds->get_reference(
                self::MODULE_GID,
                'orientation',
                $lang_id
            )),
    ...
    public function validateSettings(array $settings)
    {
        ...
        $alike_fields = array('site_type', 'user_type', 'orientation', 'lang', 'land',
            'country_code');
    ...
    public function getStat()
    {
        ...
        $net_errors = $this->ci->Users_model->getUsersList(
            ...
            null,
            null,
            array('where' => array('net_status' => 'error')),
            array(),
            false,
            true
        );
        ...
        $stat = array(
            'count' => array(
                ...
                'from_network' => $this->ci->Users_model->getUsersCount(array(
                    'where' => array(
                        'net_status' => self::STATUS_OK,
                        'net_is_incomer' => 1,
                    ),
                )),
                'to_network' => $this->ci->Users_model->getUsersCount(array(
                    'where' => array(
                        'net_status' => self::STATUS_OK,
                        'net_is_incomer' => 0,
                    ),
                )),
                'agreed' => $this->ci->Users_model->getUsersCount(array(
                    'where' => array(
                        'net_user_decision' => self::DECISION_AGREE,
                    ),
                )),
                'disagreed' => $this->ci->Users_model->getUsersCount(array(
                    'where' => array(
