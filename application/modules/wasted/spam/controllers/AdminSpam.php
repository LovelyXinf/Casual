...
class AdminSpam extends \Controller
{
    ...
    {
        $this->load->model(["spam/models/Spam_alert_model", "spam/models/Spam_type_model"]);

        $this->Menu_model->setMenuActiveItem("admin_spam_menu", "spam_alerts_item");

        $spam_types = $this->Spam_type_model->getTypes(1);
     ...
     * @param integer $ids
     */
    public function alertsDelete($action = "without_object", $ids = null)
    {
        $errors = false;
        $messages = array();
        if (!$ids) {
     ...
     * @param integer $ids
     */
    public function alertsBan($ids = null)
    {
        $errors = false;
        $messages = array();
        if (!$ids) {
     ...
     * @param integer $ids
     */
    public function alertsUnban($ids = null)
    {
        $errors = false;
        $messages = array();
        if (!$ids) {
     ...
     */
    public function typesEdit($type_id)
    {
        $this->load->model("spam/models/Spam_type_model");
     ...
     */
    public function typeActivate($type_id, $status = 0)
    {
        $this->load->model("spam/models/Spam_type_model");
        if (!empty($type_id)) {
            if ($status) {
     ...
     */
    public function typeSendMail($type_id, $status = 0)
    {
        $this->load->model("spam/models/Spam_type_model");
        if (!empty($type_id)) {
            if ($status) {
     ...
     */
    public function reasonsEdit($lang_id = null, $option_gid = null)
    {
        $this->load->model("spam/models/Spam_reason_model");
     ...
     */
    public function ajaxReasonsSaveSorter()
    {
        $this->load->model("spam/models/Spam_reason_model");
     ...
     */
    public function ajaxReasonsDelete($option_gid)
    {
        $this->load->model("spam/models/Spam_reason_model");
    ...
    public function settings()
    {
        ...
        if ($this->input->post("btn_save")) {
            ...
            $validate_data = $this->Spam_type_model->validateSettings($post_data);
            if (!empty($validate_data["errors"])) {
                $this->system_messages->addMessage(View::MSG_ERROR, $validate_data["errors"]);
                ...
                $this->system_messages->addMessage(View::MSG_SUCCESS, l("success_settings_saved", "start"));
                $data = $validate_data["data"];
            }
        } else {
