    ...
    if (!function_exists('showBannerPlace')) {
        ...
        function showBannerPlace($place_id)
        {
            ...
            $ci->load->model(['banners/models/Banner_place_model',
                'banners/models/Banner_group_model', 'Banners_model']);
            if (!is_numeric($place_id)) {
                $place = $ci->Banner_place_model->getByKeyword($place_id);
                $place_id = $place['id'];
            } else {
            ...
            $group_id = $ci->Banner_group_model->getGroupIdByPageLink($uri);
            if (!$group_id) {
                $group_ids = $ci->Banner_group_model->searchGroupsIdByPageLink($uri);
                if (empty($group_ids)) {
                    return;
                }
            } else {
            ...
            $banners = $ci->Banners_model->showRotationBanners($group_ids, $place_id, $place['places_in_rotation']);
