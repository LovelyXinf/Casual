...
class BannersModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('banners', CacheManager::PROVIDER_REDIS);
        $this->ci->cache->registerService('banners_positions', CacheManager::PROVIDER_REDIS);
    }
    ...
    public function saveBannerStatus($id, $status)
    {
        ...
        $is_rotation = $this->ci->Banner_place_model->isRotationByPlaceID($banner["banner_place_id"]);if ($banner["banner_type"] == 2 && $is_rotation === true && $status== 1) {
    ...
    public function saveBannerViews($id, $views)
    {
        ...
        foreach ($banners_raw as &$banner_raw) {
            if ($banner_raw['id'] == $id) {
                ...
                // TODO: cache
                $this->ci->cache->set('banners', 'all', $banners_raw, function () use ($id, $views)
                {
                    $ci = &get_instance();
                    $ci->db->where('id', $id)
    ...
    public function saveBannerClicks($id, $clicks)
    {
        ...
        foreach ($banners_raw as &$banner_raw) {
            if ($banner_raw['id'] == $id) {
                ...
                // TODO: cache
                $this->ci->cache->set('banners', 'all', $banners_raw, function () use ($id, $clicks)
                {
                    $ci = &get_instance();
                    $ci->db->where('id', $id)
    ...
    public function validateBanner($id, $data, $file_name = "")
    {
        ...
        if (isset($data["name"])) {
            ...
            if (empty($return["data"]["name"]) || !preg_match($name_expr, $return["data"]["name"])) {
                $return["errors"][] = l('banner_edit_error_name_empty', 'banners') . '111';
            } else {
        ...
        if (isset($data["banner_place_id"])) {
            ...
            if ($return["data"]["banner_type"] == 2 && $is_rotation === true 
     ...
     * @return array
     */
    public function getBanners($page = 1, $items_on_page = 20, $order_by = null, $params = array(), $filter_object_ids = null)
    {
        $objects = array();
     ...
     * @return integer
     */
    public function cntBanners($params = array(), $filter_object_ids = null)
    {
     ...
     * @return array
     */
public function getBannersJoinGroups($page = 1, $items_on_page = 20, $order_by = null, $params = array(), $filter_object_ids = null)
    {
    ...
    public function getAllBanners()
    {
        if ($this->banners_all === null) {
            ...
            $this->banners_all = $this->ci->cache->get('banners', 'all', function () use ($fields)
            {
                $ci = &get_instance();
    ...
    public function getAllPositions()
    {
        if ($this->positions_all === null) {
            ...
            $this->positions_all = $this->ci->cache->get('banners_positions', 'all', function () use ($fields)
            {
                $ci = &get_instance();
    ...
    public function formatBanners($data)
    {
        ...
        if ($this->format_settings['get_user'] && !empty($user_ids)) {
            $this->ci->load->model('Users_model');
            $users = $this->ci->Users_model->getUsersListByKey(null, null, null, array(), array_unique($user_ids), true);
    ...
    public function delete($id)
    {
        ...
        // TODO: cache
        $this->ci->cache->flash('banners');
        $this->ci->cache->flash('banners_positions');
    }
     ...
     * @return void
     */
    public function addBannerGroups($banner_id, $banner_groups, $place_id, $is_admin, $group_position = array())
    {
     ...
     * @return array
     */
    public function serviceValidateBanner($user_id, $data, $service_data = array(), $price = '')
    {
     ...
     */
    public function dynamicBlockBannerPlaceMethod($params = array(), $view = '')
    {
    ...
    {
        $methods = [
            '_dynamic_block_banner_place_method' => 'dynamicBlockBannerPlaceMethod',
