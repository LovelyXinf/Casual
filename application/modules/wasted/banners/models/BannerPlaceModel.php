...
class BannerPlaceModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('banners_places', CacheManager::PROVIDER_REDIS);
    }
    ...
    public function getAllPlaces($access = false)
    {
        if ($this->places_all === null) {
            ...
            $this->places_all = $this->ci->cache->get('banners_places', 'all', function () use ($fields)
            {
                $ci = &get_instance();
    ...
    public function savePlace($id, $data)
    {
        ...
        if (isset($saved_place_groups) && is_array($saved_place_groups) && count($saved_place_groups) > 0) {
            ...
            foreach ($saved_place_groups as $group_id) {
                $this->ci->db->insert(TABLE_BANNERS_PLACE_GROUP, array('place_id' => $id, 'group_id' => $group_id));
            }
        }

        // TODO: clear cache
        $this->ci->cache->flush('banners_places');
    ...
    public function savePlaceGroup($place_id, $group_id)
    {
        if ($place_id && $group_id) {
            $this->ci->db->insert(TABLE_BANNERS_PLACE_GROUP, array('place_id' => $place_id, 'group_id' => $group_id));
        }
    }
