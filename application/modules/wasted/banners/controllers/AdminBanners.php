...
class AdminBanners extends \Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(['Menu_model','Banners_model']);
    ...
    public function edit($banner_id = null, $approve = 0)
    {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Banners_model->validateBanner($banner_id, $post_data, 'banner_image_file');
    ...
    {
        if ($banner_id) {
            $this->Banners_model->saveBannerStatus($banner_id, $status);
            $this->system_messages->addMessage(View::MSG_SUCCESS, l('success_update_banner_data', 'banners'));
        }
     ...
     * @return void
     */
    public function editPlace($id = null)
    {
        $this->load->model('banners/models/Banner_place_model');
        $data = !is_null($id) ? $this->Banner_place_model->get($id) : [];
        ...
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Banner_place_model->validatePlace($id, $post_data);

            if (!empty($validate_data["errors"])) {
                $this->system_messages->addMessage(View::MSG_ERROR, $validate_data["errors"]);
                $data = array_merge($data, $validate_data["data"]);
            } else {
                $place_id = $this->Banner_place_model->savePlace($id, $validate_data["data"]);
