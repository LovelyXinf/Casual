        ...
        function getUserGroup($params = [])
        {
            ...
            if (!isset($params['type']) || $params['type'] != 'choise') {
                if (!empty($params['user_type'])) {
                    $ci->Access_permissions_settings_model->getAccessData(
                        $ci->Access_permissions_model->roles[AccessPermissionsModel::USER], ['user_type' => $params['user_type']]
    ...
    if (!function_exists('getUserGroupInfo')) {
        ...
        function getUserGroupInfo($params = [])
        {
            ...
            return !empty($group) ? $group[0] :
                [
                   'data' => ['current_name' => $ci->Access_permissions_groups_model->getGroupByGid(
                    \Pg\modules\access_permissions\models\AccessPermissionsGroupsModel::DEFAULT_GROUP, true
                        )['current_name']]
                ];
        }            
