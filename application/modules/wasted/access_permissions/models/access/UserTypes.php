...
class UserTypes extends AccessAbstract
{
    ...
    public function getGroupsList(array $params)
    {
        ...
        foreach ($groups as $key => $group) {
            $groups[$key]['periods'] = $this->ci->Access_Permissions_Groups_Model->getPeriodsList([$group['gid'] . '_' . $this->user_type . '_group']);
        }
        return $groups;
    }
    ...
    private function formatGroupPeriods($group, $data)
    {
        ...
        $add_fields = [$data['group_gid'] . '_' . $this->user_type . '_group'];
    ...
    public function validatePermissions(array $data, array $module,  $is_check = false)
    {
        ...
        if (!empty($data)) {
            ...
            $methods_data = current($this->ci->Access_permissions_modules_model->getModulesList([
                'where' => ['module_gid' => $module['module_gid'], 'method' => NULL, 'access' => 2]
            ]));
            $where_in = [];
            $where_not_in = [];
            ...
            } else if (!empty($methods_data['not_methods'])) {
    ...
    public function formatPermissions(array $data, array $module)
    {
        ...
        if (!empty($data['permissions'])) {
            foreach ($data['permissions'] as $permission) {
                $role_data = explode('_', str_replace('_' . $this->user_type, '', $permission['role']));
     ...
     * @return void
     */
    public function deleteUserType($user_type)
    {
        $this->ci->load->model(AccessPermissionsModel::MODULE_GID
            . '/models/Access_Permissions_Groups_Model');
        $groups = $this->ci->Access_Permissions_Groups_Model->getGroupsList(['where' => [
                'is_default' => 0,
                'is_trial' => 0
         ]]);
        $roles = ['user_' . $user_type];
        $this->ci->load->dbforge();
        foreach ($groups as $group) {
            $this->ci->dbforge->drop_column(AccessPermissionsGroupsModel::GROUP_PERIOD_TABLE, $group['gid'] . '_' . $user_type . '_group');
            $roles[] = 'user_' . $group['gid'] . '_' . $user_type;
        }
    ...
    private function addFieldsPeriodTable($groups, $user_types)
    {
        ...
        foreach ($groups as $group_gid => $groups_data) {
            if ($group_gid != 'default') {
                foreach ($user_types as $type) {
                    $this->ci->dbforge->add_column(AccessPermissionsGroupsModel::GROUP_PERIOD_TABLE, [$group_gid . '_' . $type . '_group' => ['type' => 'DECIMAL', 'constraint' => '10,2', 'null' => false, 'default' => 1]]);
                }
            }
        }
    }
    ...
    public function validatePeriod($data)
    {
        ...
        foreach ($groups as $group)  {
            if (isset($data[$group['gid'] .  '_group'])) {
                $result['data'][$group['gid'] . '_' . $data['user_type'] . '_group'] = abs(floatval($data[$group['gid'] . '_group']));
            } else {
    ...
    {
        if (!$id) {
            $this->ci->db->insert(AccessPermissionsGroupsModel::GROUP_PERIOD_TABLE, $attrs);
            $id = $this->ci->db->insert_id();
        } else {
            $this->ci->db->where('id', $id);
            $this->ci->db->update(AccessPermissionsGroupsModel::GROUP_PERIOD_TABLE, $attrs);
        }
        return $id;
    }
