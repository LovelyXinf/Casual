...
class AccessPermissionsModel extends \Model
{
     ...
     * @var string
     */
    const PAYMENT_TYPE_GID_PREFIX = 'access_permissions_';
    ...
    public function __construct()
    {
        parent::__construct();
        // TODO: cache
        $this->ci->cache->registerService('access_permissions', CacheManager::PROVIDER_REDIS);
    }
    ...
    public function groupPayment(array $data, $user_id)
    {
        ...
        if ($data['pay_system_gid'] == 'account') {
            ...
            //$result = $this->systemPayment($group_data, $user_id, $data['pay_system_gid'], $data['user_type']);
...
//        $group = $this->ci->Access_permissions_groups_model->getGroupByGid($name[0]);
    ...
    public function applyGroup(array $data, $user_id)
    {
        ...
        $user_group_id = isset($user_group['id']) ? $user_group['id'] : null;
        $save_data = [
            'id_user' => $user_id,
            'group_gid' => $data['gid'],
            'id_period' => (int)$data['period']['id'],
            'data' => serialize($data),
            'is_active' => 1,
            ...
            if (!empty($user_current_groups)) {
                foreach ($user_current_groups as $uc_group) {
                    if ($uc_group['group_gid'] != $data['gid']) {
                        $this->ci->Access_permissions_users_model->deleteUserFromGroup($user_id, $uc_group['group_gid']);
                    }
                }
            }
        }
     ...
     * @return boolean
     */
    public function isAccessGroups($params)
    {
        // TODO: cache
        $result = $this->ci->cache->get('access_permissions', 'count', function () use ($params)
        {
            $ci = &get_instance();            
