...
function AccessPermissions(optionArr) {
    this.properties = {
        ...
        dataAction: {
            ...
            accessPermissionsPage: '[data-action="access-permissions-page"]',
        },
        dataContent: {
        ...
        url: {
            ...
            loadAccessPermissionsPage: 'access_permissions/index/',
        ...
        contentObj: new loadingContent({
            ...
            onClose: function () {
                _self.serviceRedirect();
            }
        }),
        currency_output: false,
    };
    ...
    this.initControls = function () {
        ...
        }).off('click', _self.properties.dataAction.accessPermissionsPage).on('click', _self.properties.dataAction.accessPermissionsPage, function () {
            _self.accessPermissionsPage();    
    ...
    this.headerAdvertisingImage = function () {
        if (_self.properties.headerAdvertisingImage !== false) {
            if ($(_self.properties.class.advertisingImage).is(':visible') === false) {
                ...
                if ($('body').is('header')) {
                    var addToElem = $('header');
                } else {
                    var addToElem = $('.pre-main-inner-content').first();
                }
                addToElem.after(function () {
    ...
    this.accessPermissionsPage = function () {        
        locationHref(_self.properties.siteUrl + _self.properties.url.loadAccessPermissionsPage, true);
    };
    ...
    this.setPeriod = function (obj) {
        ...
        $('#period-' + obj.data("group")).find('div').html(function(){
            return _self.currencyOutput(obj.data("price"));
        });
    ...
    this.setPrice = function (price) {
        ...
        $(_self.properties.id.setPrice).html(
            _self.currencyOutput(_tempData.price)
        );
    };
    ...
    this.payment = function (obj) {
        sendAnalytics('membership_system_' + obj.data('pay_system') + '_payment_send', 'membership_system_payment_send', 'user');
        sendAnalytics('membership_group_' + obj.data('group') + '_payment_send', 'membership_group_payment_send', 'user');
            ...
            _self.query(
                ...
                function (data) { 
                    ...
                    if (data.success && data.success.length > 0) {
                        sendAnalytics('membership_system_' + obj.data('pay_system') + '_success', 'membership_payments_system_success', 'user');
                        sendAnalytics('membership_group_' + obj.data('group') + '_success', 'membership_payments_group_success', 'user');
                    } else {
                        sendAnalytics('membership_system_' + obj.data('pay_system') + '_payment_fail', 'membership_group_payment_system_fail', 'user');
                        sendAnalytics('membership_group_' + obj.data('group') + '_payment_fail', 'membership_group_payment_fail', 'user');
                    }
                }
            );
        } else {
    ...
    this.groupToggle = function (obj) {
        if ($('#' + obj.data('group') + '-block .module').length > 0) {
            ...
            if (groupObj.hasClass('active') === false) {
                ...
                obj.closest('.modules').removeClass('collapsed-list');
            } else {
                obj.closest('.modules').addClass('collapsed-list');
    ...
    this.savingCalculation = function () {
        ...
        if (saving > 0) {
            ...
            $(_self.properties.dataAction.saving).html(
                _self.currencyOutput(saving)
            );
        } else {
    ...
    this.serviceRedirect = function () {
        var redirectUrl = $(_self.properties.dataAction.close).data('redirect');
        if (redirectUrl) {
            locationHref(redirectUrl, true);
        }
    };

    this.currencyOutput = function (price) {
        if (typeof _self.properties.currency_output === 'function') {
            try {
                return _self.properties.currency_output(price);
            } catch (e) {
                console.log(e);
            }
        } 

        return price + "<span class='currency'>" + _self.properties.currency + "</span>";
    };
