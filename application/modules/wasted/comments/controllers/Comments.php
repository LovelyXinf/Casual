...
class Comments extends \Controller
{
    ...
    public function ajaxAddComment()
    {
        ...
        $comments_type = $this->Comments_types_model->getCommentsTypeByGid($data['gid']);
        ...
        $validate_comment = $this->Comments_model->validateComment($data);
    ...
    public function ajaxLoadComments()
    {
        ...
        if ($result['comments']['comments'] || $with_form) {
            ...
            if (!empty($user[0]['media']['user_logo']['thumbs']['small'])) {
                ...
                $default_logo = $this->ci->Uploads_model->format_default_upload($this->Users_model->upload_config_id);
                $this->view->assign('user_img', $default_logo['thumbs']['small']);
            }
            $this->view->assign('comments', $result['comments']);
    ...
    public function ajaxLikeComment()
    {
