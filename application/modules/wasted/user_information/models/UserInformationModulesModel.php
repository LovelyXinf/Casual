...
{   
     ...
     * Class constructor
     *
     * @return UserInformationModulesModel
     */
     ...
     * @return void
     */
    public function archiveCreate($data = [])
    {
     ...
     * $return array
     */
    public function getData($user_data)
    {
        $result = [];
        ...
        foreach ($modules as $module_data) {
            ...
            if (class_exists(NS_MODULES . $module . '\\models\\' . $ucfirst_module . 'UserInformationModel') !== false) {
                $model = ucfirst($module) . '_user_information_model';
                $this->ci->load->model([$module . '/models/' . $model, 'Users_model']);
                $user = $this->ci->Users_model->getUserById($user_data['user_id'], true);
