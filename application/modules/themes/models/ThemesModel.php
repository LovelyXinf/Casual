...
class ThemesModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('themes', CacheManager::PROVIDER_REDIS);
        $this->ci->cache->registerService('themes_colorsets', CacheManager::PROVIDER_REDIS);
        $this->ci->cache->registerService('colorset_logo', CacheManager::PROVIDER_REDIS);
    }

    public function getInstalledThemesList($params = array(), $filter_object_ids = null)
    {
    ...
    public function getInstalledThemesByKey($params = array(), $filter_object_ids = null)
    {
        $return = array();
    ...
    public function getInstalledThemesCount($params = array(), $filter_object_ids = null)
    {
    ...
    public function setIsGenerated($set_id, $is_generated = 1)
    {
        ...
        // TODO: cache
        $this->ci->cache->flush('themes_colorsets');
    }
    ...
    public function validateSet($id, $data, $files = array(), $theme_data = array())
    {
    ...
    {
        if ($this->colorset_logo_all === null) {
            $fields = $this->colorset_logo_fields;

            $this->colorset_logo_all = $this->ci->cache->get('colorset_logo', 'all', function () use ($fields)
            {
                $ci = &get_instance();
    ...
    private function getColoursetLogoInternal($params = array())
    {
    ...
    public function getColoursetLogo($params = array(), $theme_data = array(), $is_filter = false)
    {
    ...
    private function saveColoursetLogoParams($id = null, $data = array())
    {
        if (empty($id)) {
    ...
    public function saveLogoParams($id_theme, $data, $id_set = null, $lang_id = null, $is_activate = false)
    {
        ...
        if($set['active'] || $is_activate) {
            // данные таблицы THEMES_TABLE используются как дефолтные
            // if (!empty($data)) {
            //     $this->ci->db->where('id', $id_theme);
            //     $this->ci->db->update(THEMES_TABLE, $data);
            // }
    ...
    public function dynamicBlockGetLogoBlock()
    {
    ...
    {
        $methods = [
            '_dynamic_block_get_logo_block' => 'dynamicBlockGetLogoBlock',
