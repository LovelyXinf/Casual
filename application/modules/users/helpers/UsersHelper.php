...
namespace Pg\modules\users\helpers {
    ...
    if (!function_exists('authLinks')) {
        ...
        function authLinks(array $params = [])
        {
            ...
            if ($ci->session->userdata('logo') && file_exists($ci->session->userdata('logo')) === false) {
    ...
    if (!function_exists('lastRegistered')) {
        ...
        function lastRegistered($params)
        {
            ...
            $attrs['order_by']    = array('field' => 'date_created', 'direction' => 'DESC',);
    ...
    if (!function_exists('incorrectEmail')) {
        function incorrectEmail() {
            ...
            if ($ci->pg_module->get_module_config('users', 'user_advanced_email_validate')) {
                ...
                if ($user['checked_email'] && !$user['valid_email']) {
                    if (!$ci->session->userdata('is_not_valid_email_show')) {
    ...
    if (!function_exists('userSelect')) {
        ...
        function userSelect($selected = array(), $max_select = 0, $var_name = 'id_user')
        {
            $ci = &get_instance();
            $ci->load->model("Users_model");
            ...
            if (!empty($selected)) {
                $data["selected"]     = $ci->Users_model->getUsersList(null, null, null, array(), $selected, true);
    ...
    if (!function_exists('adminHomeUsersBlock')) {
        ...
        function adminHomeUsersBlock()
        {
            ...
            if ($user_type == 'moderator') {
                ...
                if ((is_array($methods_users) && !in_array('index', $methods_users))
                    || (is_array($methods_moderation) && !in_array('index', $methods_moderation))) {
                    $show = true;
                } else {
            ...
            $stat_users["blocked"]   = $ci->Users_model->getUsersCount(array("where" => array('approved' => 0)));
            $stat_users["unconfirm"] = $ci->Users_model->getUsersCount(array("where" => array('confirm' => 0)));
    ...
    if (!function_exists('usersSearchForm')) {
        ...
        function usersSearchForm($object = 'user', $type = 'line', $show_data = false, $params = [])
        {
            ...
            $form_settings = [
                ...
                'load_users' => 'users/ajaxLoadUsers/',
                'form_url' => 'users/search/',
                'search_url' => 'users/ajax_search/',
                'count_url' => 'users/ajax_search_counts/'
            ];
            ...
            if ($show_data) {
                ...
                foreach ($fields_for_select as $field) {
                    if (!empty($validate_settings['data'][$field]) || !empty($validate_settings['data'][$field . '_min'])
                ...
                $age_min = isset($ci->session->userdata("users_search")['age_min']) ? $ci->session->userdata("users_search")['age_min'] : $ci->pg_module->get_module_config('users', 'age_min');
                $age_max = isset($ci->session->userdata("users_search")['age_max']) ? $ci->session->userdata("users_search")['age_max'] : $ci->pg_module->get_module_config('users', 'age_max');

                if (empty($validate_settings['data']['age_min'])) {
                    $validate_settings['data']['age_min'] = $age_min;
                }

                if (empty($validate_settings['data']['age_max'])) {
                    $validate_settings['data']['age_max'] =  $age_max;
                }
            }
            ...
            if ($object == 'user' && $type == 'advanced') {
                ...
                $form = $ci->Field_editor_forms_model->getFormByGid($ci->Users_model->advanced_search_form_gid,
    ...
    if (!function_exists('visitors')) {
        ...
        function visitors($attrs)
        {
            ...
            $count = count($ci->Users_views_model->getViewersDailyUnique($ci->session->userdata('user_id'), null,
                    null, array('view_date' => 'DESC'), array(), 'all', 1));
    ...
    if (!function_exists('formatAvatar')) {
        ...
        function formatAvatar(array $params)
        {
           ...
           $logo_data = (isset($params['user']['media']['user_logo'])) ?
                ((isset($params['get_original_file_url']) && !empty($params['get_original_file_url'])) ?
                    $params['user']['media']['user_logo']['file_url'] :
                    $params['user']['media']['user_logo']['thumbs'][$params['size']] 
                ) :
                $params['user']['media']['user_logo_moderation']['thumbs'][$params['size']];
            
            $logo_params = [
                'size_name' => $params['size'],
                'src' => $logo_data . '?' . strtotime($params['user']['date_modified']),
    ...
    if (!function_exists('menuSettings')) {
        ...
        function menuSettings($params)
        {
            ...
            if ($ci->pg_module->is_module_installed('user_information')) {
                ...
                $alert['download_my_data'] = $ci->User_information_model->status($params['user']['id'])['status'] == 'ready' ? 1 : 0;
    ...
    if (!function_exists('onlineNow')) {
        ...
        function onlineNow()
        {
            $ci = &get_instance();
            $ci->load->model('Users_model');
            $online_count = $ci->Users_model->getUsersCount(['where' => ['online_status' => 1]]);
            $ci->view->assign('online_count', $online_count);
