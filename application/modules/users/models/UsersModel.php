...
class UsersModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('users', CacheManager::PROVIDER_REDIS);
        if ($this->ci->pg_module->is_module_installed('couples')) {
            $this->is_couples_installed = true;
            $this->fields = \Pg\modules\couples\models\CouplesModel::addFields(USERS_TABLE, $this->fields);
        }
        if ($this->ci->pg_module->is_module_installed('network')) {
            $this->fields = array_merge($this->fields,
                \Pg\modules\network\models\NetworkUsersModel::getUsersFields());
        }
    ...
    public function getUsersList($page = null, $items_on_page = null, $order_by = null, $params = [], $filter_object_ids = [], $formatted = true, $safe_format = false, $lang_id = '')
    {
        ...
        if ($this->ci->session->userdata('auth_type') == 'user' && $controller != 'blacklist') {
            $user_id = $this->ci->session->userdata('user_id');
            $this->ci->load->model('Blacklist_model');

            $is_check_from_another = false;
            
            if ($controller == 'like_me') {
        ...
        $fields = $this->fields_all;
        $get_data_callback = function ($filter_object_ids, $resort_by_keys=false) use ($fields, $params, $page, $items_on_page, $order_by)
        {
            $ci = &get_instance();
    ...
    = false)
    {
        ...
        if (!empty($ids)) {
            ...
            foreach ($ids as $id) {
                if (!isset($data[$id])) {
                    $data[$id] = array_merge($this->formatDefaultUser($id),
    ...
    public function getFeaturedUsers($count = 20, $user_type = 0)
    {
        if ($this->ci->pg_module->is_module_installed('services')) {
            ...
            return $this->formatUsers(
                $this->ci->db
                    ->where_in('id', $ids)
                    ->order_by("FIND_IN_SET(id, '" . implode(',',$ids) . "')")
                    ->limit($count, 0)
                    ->get()->result_array()
            );
        }
    ...
    public function saveUser($user_id = null, $attrs = [], $file_name = "", $use_icon_moderation
    = true, $load_image = false)
    {
        ...
        if ($file_name && $user_id) {
            ...
            if (isset($img_return) && empty($img_return['errors']) && !empty($img_return['file'])) {
    ...
    private function userStatus($user_id)
    {
        ...
        if ($user['confirm'] == 0) {
            ...
            $was_approved_before = $this->profile_before['approved'] === '1';

            return ($is_admin/* && $was_approved_before*/) ? 
            self::STATUS_ADMIN_DECLINED : self::STATUS_NOT_APPROVED;
        } else {
            return ($is_admin) ? 
            self::STATUS_ADMIN_APPROVED : self::STATUS_SAVED;
        }
    }
    ...
    {
        $event_data['module'] = UsersModel::MODULE_GID;
    ...
    public function checkAvailableUserActivation($user_id)
    {
    ...
    public function activateUser($user_id, $status = 1)
    {
        ...
        $this->ci->Users_blocked_callbacks_model->executeCallbacks([$user_id], $status);
    ...
    public function validate($user_id = null, $data = [], $file_name = "", $section_gid = null, $type = 'select')
    {
        ...
        if (isset($data["referral_code"]) && $data["referral_code"] && $this->ci->pg_module->get_module_config('referral_links', 'is_active')) {
            $this->ci->load->model('Referral_links_model');
            $ref_data = $this->ci->Referral_links_model->getReferralData(array('uniq_code' => strip_tags($data["referral_code"])));
            if ($ref_data) {
                $is_user  = $this->getUserById($ref_data['user_id']);
                if (!$is_user) {
                    unset($return["data"]["referral_code"]);
                } else {
                    if ($ref_data['is_used'] || $ref_data['ref_email'] != $data['email']) {
                        unset($return["data"]["referral_code"]);
                    } else {
        ...
        if ($this->is_couples_installed === true) {
            if (\Pg\modules\couples\models\CouplesModel::isCouplesFields($data) === true) {
        ...
        if (!is_null($section_gid)) {
            ...
            if ($this->is_couples_installed === true && empty($return['couple']) && $user_id
                && \Pg\modules\couples\models\CouplesModel::isCouplesFields($data) === true) {
                $this->ci->load->model('Couples_model');
                if ($type == 'save') {
    ...
    public function formatUsers($data, $safe_format = false, $lang_id = null)
    {
                ...
                if ($blacklist_user_ids && $this->ci->pg_module->is_module_installed('blacklist')) {   
                    ...
                    foreach ($blacklist_data as $blacklist_item) {
                        if ($blacklist_item['id_dest_user'] == $viewer_id) {
                            ...
                            $bloked_by_user_id[$blacklist_item['id_dest_user']] = 1;
                        }
                    }
                    foreach ($data as $key => $user) {
                        if (isset($this->blacklist[$user['id']])) {
                            ...
                            if (isset($bloked_by_user_id[$user['id']])) {
        ...
        foreach ($data as $key => $user) {
            ...
            if (isset($user['birth_date'])) {
                $user['birth-date'] = $user['birth_date'] = tpl_date_format($user['birth_date'],
                    $date_formats);
            }
        ...
        if ($method == "account") {
            ...
            return array(
                "templates" => array('nickname', 'first-name', 'second-name',
                    ...
                    'output_name'),
                "url_vars"  => array(
                    "id" => array("id" => 'literal', "nickname_str" => 'literal'),
                ),
                "url_postfix" => array(
                    'section-code' => array('section-code' => 'literal',
                        'section-name' => 'literal'),
    ...
    public function getSitemapXmlUrls($generate = true, $is_full = true)
    {
        ...
        $user_settings = $this->ci->pg_seo->get_settings('user', 'users',
            'login_form');
        ...
        if (!$user_settings['noindex']) {
            $return[] = array(
                "url" => rewrite_link('users', 'search', array(), false, null, $lang_canonical),
                "priority" => 0.5,
            );
        }
    ...
    public function dynamicBlockLastRegistered($params, $view = '', $width = 100)
    {
        $count = $params["count"];
        ...
        $users = $this->getActiveUsersList(1, $count,
            ...
            case 0:
                ...
                if ($mtype['mtype'] == 0 || $mtype['mtype'] == 1) {
                    ...
                    if ($user['user_logo_postmoderation']) { 
                        ...
                        $img_data = $this->ci->Uploads_model->uploadUrl($this->upload_config_id, $object_id,
    ...
    {
        $return = array('main_fields' => [], 'fe_fields' => [],
            'default_lang_id' => $this->ci->pg_language->get_default_lang_id(),
            'object_lang_id' => 1);
        $this->set_additional_fields($fields);
        $data                              = $this->getUserById($id);
        $hide_user_names                   = $this->ci->pg_module->get_module_config('users',
            'hide_user_names');
    ...
    public function updateProfileCompletion($filter_object_ids = array())
    {
        ...
        if ($fields_sql) {
            $this->ci->db->set('profile_completion',
    ...
    public function serviceCronUserActivateInSearch()
    {
        ...
        if (!empty($results) && is_array($results) && $results[0]["cnt"] > 0) {
            ...
            $data["activity"] = '0';
            $this->ci->db->where('activated_end_date <',
    ...
    public function serviceCronUsersFeatured()
    {
        ...
        if (!empty($results) && is_array($results) && $results[0]["cnt"] > 0) {
            // TODO: cache - убрать из кэша
            $data["featured_end_date"] = self::DB_DEFAULT_DATE;
            $this->ci->db->where('featured_end_date <',
                    date(self::DB_DATE_FORMAT_SEARCH))
                ->where('UNIX_TIMESTAMP(featured_end_date) >', '86400')
                ->update(USERS_TABLE, $data);
            $clean                     = $results[0]["cnt"];
    ...
    public function serviceCronHideOnSite()
    {
        ...
        if (!empty($results) && is_array($results) && $results[0]["cnt"] > 0) {
            // TODO: cache - убрать из кэша
            $data["hide_on_site_end_date"] = self::DB_DEFAULT_DATE;
            $this->ci->db->where('hide_on_site_end_date <',
                    date(self::DB_DATE_FORMAT_SEARCH))
                ->where('UNIX_TIMESTAMP(hide_on_site_end_date) >', '86400')
                ->update(USERS_TABLE, $data);
            $clean                         = $results[0]["cnt"];
    ...
    public function serviceCronHighlightInSearch()
    {
        ...
        if (!empty($results) && is_array($results) && $results[0]["cnt"] > 0) {
            // TODO: cache - убрать из кэша
            $data["highlight_in_search_end_date"] = self::DB_DEFAULT_DATE;
            $this->ci->db->where('highlight_in_search_end_date <',
                    date(self::DB_DATE_FORMAT_SEARCH))
    ...
    public function serviceCronUpInSearch()
    {
        ...
        if (!empty($results) && is_array($results) && $results[0]["cnt"] > 0) {
            // TODO: cache - убрать из кэша
            $data["up_in_search_end_date"] = self::DB_DEFAULT_DATE;
            $this->ci->db->where('up_in_search_end_date <',
                    date(self::DB_DATE_FORMAT_SEARCH))
                ->where('UNIX_TIMESTAMP(up_in_search_end_date) >', '86400')
                ->update(USERS_TABLE, $data);
            $clean                         = $results[0]["cnt"];
    ...
    public function serviceCronRegionLeader()
    {
        ...
        $this->ci->db->update(USERS_TABLE, array('leader_bid' => 0));
    ...
    public function getCommonCriteria($data, $is_public_data = true)
    {
        ...
        if (isset($data['radius']) && !empty($data['radius']) && !empty($data['lat']) && !empty($data['lon'])) {
            ...
            $data['min_lon'] = $data['lon'] - $data['radius'] / abs(cos(deg2rad($data['lat'])) * self::KM_IN_RAD);
            $data['max_lon'] = $data['lon'] + $data['radius'] / abs(cos(deg2rad($data['lat'])) * self::KM_IN_RAD);
    ...
    public function cronValidateEmails() {
        if ($this->pg_module->get_module_config(self::MODULE_GID, 'user_advanced_email_validate')) {
            ...
            $users = $this->getUsersList(1, 100, array('id' => 'DESC'),  array('where' => $where), null, false);
            foreach ($users as $user) {
    ...
    public function cronBlockInactiveUsers()
    {
        if ($this->pg_module->get_module_config(self::MODULE_GID, 'user_advanced_email_validate')) {
            ...
            $users                         = $this->getUsersList(1, 10000, null,
    ...
    private function serviceAvailableDefaultAction($id_user, $template_gid)
    {
        ...
        if ($this->ci->pg_module->is_module_installed('services')) {
            $this->ci->load->model('services/models/Services_users_model');
            $service_access = $this->ci->Services_users_model->isServiceAccess($id_user,
                $template_gid);
    ...
    private function serviceValidate($service_gid, $user_id, $data, $service_data = [], $price = '')
    {
        return ["errors" => array(), "data" => $data];
    }

    public function serviceActivateAbilityDelete($id_user, $id_user_service, $is_ajax
    = 0)
    {
        ...
        $this->ci->load->model('services/models/Services_users_model');
        $user_service = $this->ci->Services_users_model->getUserServiceById($id_user,
            $id_user_service);
        if (empty($user_service) || !$user_service["status"]) {
            ...
            $this->ci->Services_users_model->saveService($id_user_service,
                $user_service);
            $return['status']  = 1;
            $return['message'] = l('success_service_activating', 'services');
    ...
    public function serviceActivateHideOnSite($id_user, $id_user_service)
    {
        ...
        if (empty($user_service) || !$user_service["status"] || $user_service['count'] < 1) {
            ...
            $user_service['date_expired'] = $this->serviceSetHideOnSite($id_user,
    ...
    public function serviceActivateHighlightInSearch($id_user, $id_user_service)
    {
        ...
        if (empty($user_service) || !$user_service["status"] || $user_service['count'] < 1) {
            ...
            $user_service['date_expired'] = $this->serviceSetHighlightInSearch($id_user,
                $user_service['service']['data_admin']['period']);
            --$user_service['count'];
            if ($user_service['count'] < 1) {
                $user_service["status"] = 0;
    ...
    public function serviceActivateUsersFeatured($id_user, $id_user_service)
    {
        ...
        if (empty($user_service) || !$user_service["status"]) {
            ...
            $user_service['date_expired'] = $this->serviceSetUsersFeatured($id_user,
                $user_service['service']['data_admin']['period']);
            --$user_service['count'];
            if ($user_service['count'] < 1) {
                $user_service["status"] = 0;
    ...
    public function serviceActivateUserActivateInSearch($id_user, $id_user_service)
    {
        ...
        if (empty($user_service) || !$user_service["status"]) {
            ...
            $user_service['date_expired'] = $this->serviceSetUserActivateInSearch($id_user,
                $user_service['service']['data_admin']['period']);
            --$user_service['count'];
            if ($user_service['count'] < 1) {
                $user_service["status"] = 0;
    ...
    public function dynamicBlockGetRegistrationLoginForm($params, $view, $width
    = 100)
    {
    ...
    private function dynamicBlockGetUsers($type, $params, $view, $width)
    {
        ...
        switch ($type) {
            case 'active':
                $users = $this->getActiveUsers((int)$params['count'],
                    $user_type);
                break;
            case 'featured':
                $users = $this->getFeaturedUsers((int)$params['count'],
                    $user_type);
                break;
            case 'new':
            default:
                $users = $this->getNewUsers((int)$params['count'],
                    $user_type);
                break;
        }
    ...
    public function dynamicBlockGetNewUsers($params, $view, $width = 100)
    {
        return $this->dynamicBlockGetUsers('new', $params, $view, $width);
    }

    public function dynamicBlockGetActiveUsers($params, $view, $width = 100)
    {
        return $this->dynamicBlockGetUsers('active', $params, $view, $width);
    }

    public function dynamicBlockGetFeaturedUsers($params, $view, $width = 100)
    {
        return $this->dynamicBlockGetUsers('featured', $params, $view,
                $width = 100);
    }

    public function dynamicBlockGetAuthLinks($params, $view, $width = 100)
    {
        $data['rand']   = rand(1, 999999);
        $data['params'] = $params;
    ...
    public function dynamicBlockGetLangSelect($params, $view, $width = 100)
    {
        $data['rand']    = rand(1, 999999);
        $data['params']  = $params;
    ...
    public function spamCallback($action, $data)
    {
        switch ($action) {
            case "ban":
                $this->saveUser((int) $data, array("banned" => 1));

                return "banned";
            case "unban":
                $this->saveUser((int) $data, array("banned" => 0));
            ...
            case 'get_content':
                ...
                $return   = [];
                foreach ($data as $id) {
                    if (($this->getUsersCount(array('where_in' => array(
                                'id' => $id)))) == 0) {
                ...
                $users = $this->getUsersList(null, null, null, null,
                    (array) $new_data);
                foreach ($users as $user) {
            ...
            case 'get_object':
                ...
                $users = $this->getUsersListByKey(null, null, null, null,
                    (array) $data);
    ...
    public function callbackUserDelete($id_user, $callback_type, $callbacks_gid)
    {
        ...
        if ($this->is_couples_installed === true && 
            $data['user_type'] === 'couple' && $data['is_coupled'] !== 1) {
        ...
        if (!empty($data['id'])) {
            ...
            if ($data['user_logo_moderation']) {
                $this->ci->Uploads_model->deleteUpload($this->upload_config_id,
    ...
    public function setVisit($user_id)
    {
        ...
        if (!$this->getVisit($user_id)) {
            $this->ci->db->insert(USERS_SITE_VISIT_TABLE, $attrs);
     ...
     * @return array
     */
    private function formatUserRoles($roles, $user_type)
    {
        $format = [];
    ...
    public function registerUser($data)
    {
        ...
        $this->ci->load->model('users/models/Groups_model');
        if ($this->ci->Groups_model->isTrialGroup() == 1) {
            $this->ci->load->model('Access_permissions_model');
        ...
        $this->addToNetwork($user_id);
        
        $event = new EventUsers();
        $event->setSearchFrom($user_id);
        $event->setData($data);
        EventDispatcher::getInstance()->dispatch('user_register', $event);
     ...
     * @return void
     */
    public function addToNetwork($user_id)
    {
        if ($this->ci->pg_module->is_module_installed('network')) {
            if ($this->ci->pg_module->is_module_active('network')) {
                ...
                if ($value_set === false) {
                    $this->ci->load->model('network/models/Network_users_model');
                    $this->ci->Network_users_model->saveUserDecision($user_id,
                        \Pg\modules\network\models\NetworkUsersModel::DECISION_AGREE);
                }
            }
        }
    }
    ...
    public static function siteVisits($data)
    {
        ...
        $event_handler->dispatch('visits', $event);
    }
    ...
    public function isActiveService(string $gid)
    {
        ...
        if ($service['status']) {
            ...
            return [
                'status' => 0,
                'success' => l('success_activate_user', UsersModel::MODULE_GID)
            ];
        }
    }
     ...
     * @return string
     */
    public static function formatUserName($data = [])
    {
        if (!empty($data['output_name'])) {
    ...
    public function __call($name, $args)
    {
        $methods = [
            ...
            '_dynamic_block_get_new_users' => 'dynamicBlockGetNewUsers',
            '_dynamic_block_get_active_users' => 'dynamicBlockGetActiveUsers',
            '_dynamic_block_get_auth_links' => 'dynamicBlockGetAuthLinks',
            '_dynamic_block_get_featured_users' => 'dynamicBlockGetFeaturedUsers',
            '_dynamic_block_get_lang_select' => 'dynamicBlockGetLangSelect',
            '_dynamic_block_get_registration_login_form' => 'dynamicBlockGetRegistrationLoginForm',
            '_dynamic_block_last_registered' => 'dynamicBlockLastRegistered',
