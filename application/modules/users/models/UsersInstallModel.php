...
class UsersInstallModel extends \Model
{
    ...
    public function installMenu()
    {
        ...
        foreach ($this->modules_data['menu'] as $gid => $menu_data) {
            ...
            \Pg\modules\menu\helpers\linkedInstallProcessMenuItems($this->modules_data['menu'], 'create', $gid, 0,
    ...
    public function installMenuLangUpdate($langs_ids = null)
    {
        ...
        foreach ($this->modules_data['menu'] as $gid => $menu_data) {
            \Pg\modules\menu\helpers\linkedInstallProcessMenuItems($this->modules_data['menu'], 'update', $gid, 0,
    ...
    public function installMenuLangExport($langs_ids)
    {
        ...
        $return = array();
        foreach ($this->modules_data['menu'] as $gid => $menu_data) {
            $temp   = \Pg\modules\menu\helpers\linkedInstallProcessMenuItems($this->modules_data['menu'], 'export',
    ...
    public function deinstallMenu()
    {
        ...
        foreach ($this->modules_data['menu'] as $gid => $menu_data) {
            if ($menu_data['action'] == 'create') {
                ...
                \Pg\modules\menu\helpers\linkedInstallDeleteMenuItems($gid,
                    $this->modules_data['menu'][$gid]['items']);
            }
        }
    }
    ...
    public function installModerationLangUpdate($langs_ids = null)
    {
        ...
        $this->ci->load->model('moderation/models/Moderation_type_model');
        $this->ci->Moderation_type_model->updateLangs($this->modules_data['moderation_types'],
            $langs_file);
    }
    ...
    public function installModerationLangExport($langs_ids = null)
    {
        ...
        return array('moderation' => $this->ci->Moderation_type_model->exportLangs($this->modules_data['moderation_types'],
                $langs_ids));
    }
    ...
    public function installModeratorsLangUpdate($langs_ids = null)
    {
        ...
        foreach ($methods as $method) {
            if (!empty($langs_file[$method['method']])) {
                $this->ci->Moderators_model->saveMethod($method['id'], array(),
    ...
    public function installNotifications()
    {
        ...
        foreach ($this->modules_data['notifications']['templates'] as $tpl) {
            ...
            $tpl_ids[$tpl['gid']] = $this->ci->Templates_model->saveTemplate(null,
                $template_data);
        }
        ...
        foreach ($this->modules_data['notifications']['notifications'] as $notification) {
            ...
            $this->ci->Notifications_model->saveNotification(null,
                $notification_data);
        }
    }
    ...
    public function installNotificationsLangUpdate($langs_ids = null)
    {
        ...
        $this->ci->Notifications_model->updateLangs($this->modules_data['notifications'],
    ...
    public function installNotificationsLangExport($langs_ids = null)
    {
        ...
        $this->ci->load->model('Notifications_model');
        $langs = $this->ci->Notifications_model->exportLangs($this->modules_data['notifications'],
            $langs_ids);
    ...
    public function arbitraryInstalling()
    {
        ...
        $this->ci->Users_delete_callbacks_model->addCallback('users',
        ...
        foreach ($this->demo_content['users'] as $user) {
            ...
            if (!empty($user_id)) {
                ...
                $this->ci->Start_desktop_notifications_model->saveUserNotifications(
                    $user_id, array_keys($this->ci->Start_desktop_notifications_model->notifications_gid)
                 );
             }
        }
    ...
    public function arbitraryLangExport($langs_ids = null)
    {
        ...
        foreach ($this->pages as $page) {
            ...
            $arbitrary_return["seo_tags_{$page}_title"]          = $settings["title"];
            $arbitrary_return["seo_tags_{$page}_keyword"]        = $settings["keyword"];
            $arbitrary_return["seo_tags_{$page}_description"]    = $settings["description"];
            $arbitrary_return["seo_tags_{$page}_header"]         = $settings["header"];
            $arbitrary_return["seo_tags_{$page}_og_title"]       = $settings["og_title"];
            $arbitrary_return["seo_tags_{$page}_og_type"]        = $settings["og_type"];
            $arbitrary_return["seo_tags_{$page}_og_description"] = $settings["og_description"];
        }
    ...
    public function installFieldEditor()
    {
        ...
        if (!empty($forms)) {
            $this->ci->Field_editor_model->importTypeStructure('users', $this->field_editor['fe_sections'],
                $this->field_editor['fe_fields'], $forms);
        }
    ...
    public function installFieldEditorLangExport($langs_ids = null)
    {
        $this->ci->load->model('Field_editor_model');
        $this->ci->Field_editor_model->initialize('users');
        list($fe_sections, $fe_fields) = $this->ci->Field_editor_model->export_type_structure('users',
    ...
    public function installServices()
    {
        ...
        foreach ($this->modules_data['services'] as $services) {
            ...
            $id_tpl = $this->ci->Services_model->saveTemplate(null,
                $validate_data['data']);
            ...
            foreach ($services['services'] as $service) {
                ...
                $validate_data = $this->ci->Services_model->validateService(null,
                    $service);
                if (!empty($validate_data['errors'])) {
                    continue;
                }
                $this->ci->Services_model->saveService(null,
                    $validate_data['data']);
            }
        }
    ...
    public function installServicesLangUpdate($langs_ids = null)
    {
        ...
        $this->ci->load->model('Services_model');
        $this->ci->Services_model->updateLangs($this->modules_data['lang_services'],
            $langs_file);
    ...
    {
        $this->ci->load->model('Services_model');
        return array('services' => $this->ci->Services_model->exportLangs(
            $this->modules_data['lang_services'], $langs_ids));
    }
    ...
    public function installGeomap()
    {
        ...
        foreach ($this->modules_data['geomap'] as $geomap) {
            ...
            $this->ci->Geomap_settings_model->saveSettings(
                $geomap['settings']['map_gid'], $geomap['settings']['id_user'],
    ...
    public function installGeomapLangUpdate($langs_ids = null)
    {
        ...
        $this->ci->Geomap_settings_model->updateLang($gids, $langs_file,
            $langs_ids);
    }
    ...
    public function installGeomapLangExport($langs_ids = null)
    {
        ...
        $this->ci->load->model('geomap/models/Geomap_settings_model');
        return array('geomap' => $this->ci->Geomap_settings_model->exportLang($gids, $langs_ids));
    }
    ...
    public function deinstallGeomap()
    {
        $this->ci->load->model("geomap/models/Geomap_settings_model");
        foreach ($this->modules_data['geomap'] as $geomap) {
            $this->ci->Geomap_settings_model->deleteSettings($geomap['settings']['map_gid'],
    ...
    public function installDynamicBlocksLangExport($langs_ids = null)
    {
        ...
        return array(
            'dynamic_blocks' => $this->ci->Dynamic_blocks_model->exportLangs(
                $this->dynamic_blocks, $langs_ids),
        );
    }
    ...
    public function deinstallDynamicBlocks()
    {
        ...
        foreach ($this->dynamic_blocks as $block) {
            $this->ci->Dynamic_blocks_model->deleteBlockByGid($block['gid']);
        }
    }
    ...
    public function installComments()
    {
        $comment_type = array(
            ...
            'settings' => array('disable_guest_access' => true),
        );
    ...
    public function installCommentsLangUpdate($langs_ids = null)
    {
        ...
        $this->ci->load->model('comments/models/Comments_types_model');
        $this->ci->Comments_types_model->updateLangs(array('user_avatar'), $langs_file);
    }
    ...
    public function installCommentsLangExport($langs_ids = null)
    {
        ...
        $this->ci->load->model('comments/models/Comments_types_model');
        return array('comments' => $this->ci->Comments_types_model->exportLangs(
    ...
    public function installRatings()
    {
        ...
        foreach ((array) $this->modules_data['ratings']["ratings"] as $rating_data) {
            ...
            $this->ci->Ratings_type_model->saveType(null,
                $validate_data["data"]);
        }
    }
    ...
    public function installRatingsLangUpdate($langs_ids = null)
    {
        ...
        foreach ((array) $this->modules_data['ratings']["ratings"] as $rating_data) {
            $this->ci->Ratings_model->updateLangs($rating_data, $langs_file,
                $langs_ids);
        }
    ...
    public function installRatingsLangExport($langs_ids)
    {
        ...
        foreach ((array) $this->modules_data['ratings']["ratings"] as $rating_data) {
            $langs = array_merge($langs,
                $this->ci->Ratings_model->exportLangs($rating_data['gid'],
                    $langs_ids));
        }
