...
class AuthModel extends \Model
{
    ...
    public function login($id, $is_reg = false)
    {
        ...
        if (!$decline_login) {
            ...
            $event->setSearchFrom(intval($id));
            $event->setData($user_data);
            $event_handler->dispatch('user_login', $event);
        }
    ...
    public function updateUserSessionData($id, $user_data = [])
    {
        ...
        if ($user_data) {
            $this->ci->Users_model->setUserOutputName($user_data);
            $session = [
                "auth_type"     => 'user',
            ...
            $logo = $this->ci->Uploads_model->format_upload($this->ci->Users_model->upload_config_id, $user_data["id"], $user_data[$logo_field]);
            if(isset($logo['thumbs']['small'])) {
                $session['logo'] = $logo['thumbs']['small'] . '?' . $session['date_modified'];
            } else {
                $session['logo'] = '';
            }
