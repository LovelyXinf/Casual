...
class GroupsModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('groups', CacheManager::PROVIDER_REDIS);
    }
    ...
    private function getAllGroups()
    {
        if ($this->groups_all === null) {
            ...
            $this->groups_all = $this->ci->cache->get('groups', 'all', function () use ($fields)
            {
                $ci = &get_instance();
    ...
    protected function createGUID($name = null)
    {
        if (!is_null($name)) {
            ...
            if (empty($return['data']['gid'])) {
                $return['errors'][] = l('error_group_name', 'users');
            } elseif (strlen($return['data']['gid']) > 25 or strlen($return['data']['gid']) < 5) {
                $return['errors'][] = l('error_group_name', 'users');
            } else {
