...
class UsersFieldsModel extends \Model
{
    ...
    public function getFieldData($field)
    {
        return [
            ...
            'view_type' => $this->fields[$field]['view_type'],
        ];
    }
    ...
    public function lookingUserType()
    {
        ...
        $age_range = range($this->ci->pg_module->get_module_config(UsersModel::MODULE_GID, 'age_min'),
        ...
        return [
            ...
            'option' => [
                'looking_user_type' => $this->ci->Properties_model->getProperty('looking_user_type',
                    $this->ci->pg_language->current_lang_id)['option'],
                'age_min' => $age_range,
                'age_max' => $age_range
            ]
         ];
    ...
    public function locationStr($user)
    {
        if (!empty($user["id_country"])) {
            ...
            return  \Pg\modules\countries\helpers\
            citiesOutputFormat($user_for_location)[$this->ci->session->userdata['user_id']];
        } else {
    ...
    public function checkboxCustomFieldTypeStr($user, $field)
    {
        ...
        return  l($this->Field_editor_model->formatItemFieldsForView(
            ['where' => ['gid' => $item]], $user)[$item]['value'] ?
    ...
    {
        $item = substr($field, 3);
        $data = $this->ci->Field_editor_model->getFormFieldsList($this->getUserData(),
            ['where' => ['gid' => $item]])[$item];
