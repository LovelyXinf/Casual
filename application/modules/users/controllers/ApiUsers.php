...
class ApiUsers extends \Controller
{
    ...
    */
    public function profile()
    {
        ...
        $data = $this->Users_model->formatUser(
            $this->Users_model->getUserById($user_id), false, $lang_id
        );
        ...
        if ($user_id != $this->user_id) {  
            $my_data = $this->Users_model->formatUser(
                    $this->Users_model->getUserById($this->user_id), false, $lang_id
            );     
        }  else {
            $my_data = $data;
        }
        ...
        if ($profile_section == 'view' || $profile_section == 'wall') {
            ...
            $fields_data = $this->Field_editor_model->getFormFieldsList($data, $params, null, array(), $lang_id);
    ...
    */
    public function registration()
    {
        ...
        $this->Users_model->fields_register = ['birth_date','email','nickname','password','user_type'];
        $validate_data = $this->Users_model->validate(null, $post_data, 'user_icon');
    ...
    */
    ...
    public function getUsersData()
    {
        ...
        $items = $this->Users_model->getUsersListByKey($page, $items_on_page, array("id" => "desc", "nickname" => "asc"), $params, array(), true, true);
    ...
    */
    public function getSelectedUsers()
    {
        ...
        if (!empty($selected)) {
            ...
            $return = $this->Users_model->getUsersList(null, null, array("nickname" => "asc"), $params, null, true, true);
        } else {
            $return = array();
        }
        $this->set_api_content('data', $return);
    ...
    */
    public function search()
    {
        ...
        $event_handler = EventDispatcher::getInstance();
        $event = new EventUsers();
        $event->setSearchFrom((int)$this->user_id);
    ...
    */
    ...
    private function searchListBlock($data = [], $order = "default", $order_direction = "DESC", $page = 1, $search_type = 'advanced')
    {
        ...
        if(isset($current_settings['distance'])) {
            $this->load->model('Nearest_users_model'); 
        ...
        if ($items_count > 0) {
            ...
            $users = $this->Users_model->getUsersList($page, $items_on_page, $order_array, $criteria, array(), true, true);
    ...
    */
    ...
    private function views($period = 'all', $type = 'my_guests', $page = 1)
    {
        ...
        if ($type == 'my_guests') {
            $all_viewers = $this->Users_views_model->getViewersDailyUnique($this->user_id, null, null, $order_by, array(), $period);
            $this->Users_views_model->removeViewersCounter($all_viewers);
        } else {
            $all_viewers = $this->Users_views_model->getViewsDailyUnique($this->user_id, null, null, $order_by, array(), $period);
        }
    ...
    */
    ...
    {        
        ...
        $this->load->model('Uploads_model');
        $this->Uploads_model->recropUpload($this->Users_model->upload_config_id, $this->user_id, $user[$logo_name],
            $recrop_data);        
        $return['success'] = l('photo_successfully_saved', 'users');
