...
class Users extends \Controller
{
    ...
    private function setAccountMenu($action)
    {
        switch ($action) {
            case 'services':
                ...
                $this->session->set_userdata(array('service_activate_redirect' => rewrite_link('users', 'account', array('action' => 'services'))));
    ...
    public function restore($code = false)
    {
        ...
        } else if ($this->input->post('btn_restore')) {
    ...
    public function ajaxGetUsersData($page = 1)
    {
        ...
        $items = $this->Users_model->getUsersListByKey($page, $items_on_page, array("nickname" => "asc"), $params, array(), true, true);
    ...
    public function ajaxGetSelectedUsers()
    {
        ...
        if (!empty($selected)) {
            $return['selected'] = $this->Users_model->getUsersList(null, null, array("nickname" => "asc"), array(), $selected, true, true);
        } else {
    ...
    public function ajaxGetUsersForm($max_select = 1)
    {
        ...
        if (!empty($selected)) {
            $data["selected"] = $this->Users_model->getUsersList(null, null, array("nickname" => "asc"), array(), $selected, false);
        } else {
            $data["selected"] = array();
        }
    ...
    private function searchListBlock($data = [], $order = "default", $order_direction = "DESC", $page = 1, $search_type = 'advanced')
    {
        ...
        $current_settings = $this->session->userdata("users_search") ?
            $this->session->userdata("users_search") : $this->Users_model->getDefaultSearchData();
        if (!empty($data)) {
    ...
    private function getAdvancedSearchCriteria($data)
    {
        ...
        $fe_criteria = $this->Field_editor_forms_model->getSearchCriteria(
            $this->Users_model->advanced_search_form_gid, $data, $this->Users_model->form_editor_type, false);
        if (!empty($data["search"])) {
    ...
    public function ajaxRecropAvatar($user_id = null)
    {
        ...
        $this->load->model('Uploads_model');
        $this->Uploads_model->recropUpload($this->Users_model->upload_config_id, $this->user_id, $user[$logo_name], $recrop_data);
    ...
    {
        setcookie(
            $cookie, '', (time() - 31500000), '/' . SITE_SUBFOLDER, COOKIE_SITE_SERVER, 0
        );
    }
   ...
   /* public function getUserField()
    {
        ...
        if ($this->Users_model->is_couples_installed === true && $this->session->userdata['couple_id']) {
            $this->load->model('couples/models/Couples_fields_model');
            exit(json_encode($this->Couples_fields_model->getFieldData($field)));
        }


        exit(json_encode($this->Users_fields_model->getFieldData($field)));
    }*/
    ...
    public function getUserField()
    {
        ...
        $field_data = $this->Users_fields_model->getFieldData($field);
          
        $sorter = 0; 
        foreach ($field_data['data']['option'][$field] as $key => $value) {
           $field_data['data']['sort'][++$sorter] = $key;
        }
        
        exit(json_encode($field_data));
    }
