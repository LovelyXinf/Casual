...
class AdminUsers extends \Controller
{
    ...
    public function index($filter = null, $user_type = 'all', $order = null, $order_direction = null, $page = null)
    {
        ...
        if (!empty($current_settings["last_active"]["from"])) {
            $attrs["where_sql"][]         = $search_params["where_sql"][] = "date_last_activity >= '" . date(UsersModel::DB_DATE_SIMPLE_FORMAT,
    ...
    public function delete()
    {
        ...
        if (!empty($user_ids)) {
            ...
            if (!empty($action_user) && $action_user == 'block_user') {
                ...
                foreach ($user_ids as $user_id) {
                    $this->Users_model->deleteUser($user_id, $callbacks_gid);
                }
    ...
    public function ajaxDeleteSelect($id_user = null, $deleted = 0)
    {
        ...
        if ($id_user) {
            ...
            $data['user_ids'] = array((int)$id_user);
        } else {
    ...
    public function deleted($filter = "deleted", $order = "nickname", $order_direction = "ASC", $page = 1)
    {
        ...
        if ($users_count > 0) {
            $users = $this->Users_deleted_model->getUsersList($page,
    ...
    public function ajaxGetUsersData($page = 1)
    {
        ...
        $items_on_page = $this->pg_module->get_module_config('start',
            'admin_items_per_page');
        $items         = $this->Users_model->getUsersListByKey($page,
    ...
    public function ajaxGetSelectedUsers()
    {
        ...
        if (!empty($selected)) {
            $return['selected'] = $this->Users_model->getUsersList(null, null,
    ...
    public function ajaxGetUsersForm($max_select = 1)
    {
        ...
        if (!empty($selected)) {
            $data["selected"] = $this->Users_model->getUsersList(null, null,
