function usersSelected(optionArr) {
    this.properties = {
        siteUrl: '',
        rand: '',
    ...
    this.initControls = function () {
        $(document)
           .off('ifChanged', _self.properties.id.blockUser).on('ifChanged', _self.properties.id.blockUser, function () {
            _self.blockUser();
        }).off('ifChanged', _self.properties.id.deleteUser).on('ifChanged', _self.properties.id.deleteUser, function () {
            _self.deleteUser();
        }).off('ifChanged', 'input[name^=module]').on('ifChanged', 'input[name^=module]', function () {
            _self.deleteUser();
        });
    };
    
    this.blockUser = function () {
        if (_self.properties.isUserDeleted) {
            $('input[name^=module]').each(function(index,item) {
                $(item).iCheck('disable');
            });
            $(_self.properties.id.fullDelete).prop('disabled', true);
        } else {
            $(_self.properties.id.fullDelete).prop('disabled', false);
        }
    };
    
    this.deleteUser = function () {
        if (_self.properties.isUserDeleted === 0) {
            $('input[name^=module]').each(function(index,item) {
                $(item).iCheck('enable');
            });
            ...
            $(_self.properties.id.deleteUser).iCheck('check');
            if($('input[name^=module]:checked').val()){
                $(_self.properties.id.fullDelete).prop('disabled', false);                
            }else{
                $(_self.properties.id.fullDelete).prop('disabled', true);
