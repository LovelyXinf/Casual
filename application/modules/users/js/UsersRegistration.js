...
function UsersRegistration(optionArr) {
    ...
    this.initControls = function () {
        ...
        .off('click', _self.properties.dataAction.userType).on('click', _self.properties.dataAction.userType, function () {
            ...
            if (($(e.target).data('block') !== "pages") || ($(e.target).data('act') !== 'close') || _self.properties.isRegistration === 1) return;
            ...
            for (var i = 1; i <= _self.properties.pages; i++) {
                if (i === page) {
                    ...
                    if (_self.properties.isAuth !== 0) {
                        htmlObj += '<i class="fa fa-circle-o" data-page="' + i + '" data-action="set-page"></i>';
                    } else {
                        htmlObj += '<i class="fa fa-circle-o" data-page="' + i + '"></i>';
                    }
                }
            }
            htmlObj += '</div>';
    ...
    this.firstPage = function () {
        ...
        if (typeof (formData.looking_user_type) === 'undefined') {
            var count_types =  $(_self.properties.data.UserTypeLook + ' li').size();
            if (count_types > 1) {
