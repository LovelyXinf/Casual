...
function usersSettings(optionArr) {
    ...
    this.saveFieldInfo = function (item, save) {
        ...
        var field_name = $(item).closest('.edit-field-js').data('field');
            ...
            if (obj.type === 'text') {
                ...
                if (Object.keys(obj.data.value).length == 1) {
                    for (var key in obj.data.value) {
                        ...
                        htmlObj += _self.selectElement(key, obj.data.value[key], obj.data.option[key], false, 1, obj.data.sort) + '</div>';
                    }
                } else {
    ...
    this.selectElement = function (name, value, options, multiple, saveBtn, sort, view_type) {
        ...
        if (view_type != undefined && view_type == 'checkbox') {
            ...
            htmlObj += '<select name="' + name + '" class="form-control"  ' + multiple + '>';
            for (var key in options) {
    ...
    this.multiCheckbox = function (name, value, option_name, option_val) {
        var isChecked = (value ===  1) ? 'checked' : '';
        return  '<div><input id="' + name + option_val + '" type="checkbox" name="' + name + '" value="' + option_val + '" ' + isChecked + '>\n\
        <label for="'+ name + option_val +'">'+ option_name +'</label></div>';
    };
