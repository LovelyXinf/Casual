...
CREATE TABLE IF NOT EXISTS `[prefix]modules` (
  ...
  `is_custom_module` tinyint(1) UNSIGNED NOT NULL DEFAULT '0',
  `date_add` datetime NOT NULL,
...
DROP TABLE IF EXISTS `[prefix]themes_coloursets_logo`;
CREATE TABLE IF NOT EXISTS `[prefix]themes_coloursets_logo` (
  `id` int(3) NOT NULL AUTO_INCREMENT,
  `id_set` int(3) NOT NULL,
  ...
  `text_logo` varchar(20) NOT NULL,
  `text_logo_mini` varchar(20) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `id_set` (`id_set`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8;
