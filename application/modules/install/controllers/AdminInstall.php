 ...
 * @author      Pilot Group Ltd <http://www.pilotgroup.net/>
 */
class AdminInstall extends \Controller
{
    /**
     * Setup step for ajax
     *
    ...
    const LICENSE_PATH = 'https://www.pilotgroup.net/license_data/';
    ...
    public function __construct()
    {
        ...
        if (INSTALL_DONE) {
            ...
            if (file_exists(SITE_PHYSICAL_PATH . "install.php")) {
                $this->autoinstall = include SITE_PHYSICAL_PATH . "install.php";
            }
        }
    ...
    public function copyrights()
    {
        ...
        if (!empty($this->autoinstall['license_agree'])) {
            redirect(site_url() . 'admin/install/install_admin');
        }
    ...
    public function installAdmin()
    {
        ...
        $is_autoinstall = !empty($this->autoinstall['ftp']);

        if ($save_button || $skip_button || $is_autoinstall) {
            ...
            } elseif ($is_autoinstall) {
                foreach (['host', 'path', 'user', 'password'] as $name) {
                    $data['ftp_' . $name] = isset($this->autoinstall['ftp'][$name]) ?
                        $this->autoinstall['ftp'][$name] : '';
                }
                $save_button = true;
            } else {
    ...
    public function installDatabase()
    {
        ...
        $is_save = $this->input->post('save_install_db');
        $is_autoinstall = !empty($this->autoinstall['database']);

        if ($is_save || $is_autoinstall) {
            if ($is_save) {
                ...
                foreach (['host', 'user', 'password', 'name', 'prefix'] as $name) {
                    $data['db_' . $name] = isset($this->autoinstall['database'][$name]) ?
                        $this->autoinstall['database'][$name] : '';
                }
    ...
    public function installLangs()
    {
        ...
        foreach ($data['available'] as $lang_data) {
            $availables[] = $lang_data['code'];
        }

        $is_save = $this->input->post('save_install_langs');
        $is_autoinstall = !empty($this->autoinstall['languages']);
        ...
        if ($is_save || $is_autoinstall) {
            if ($is_save) {
                ...
                if (in_array('all', $this->autoinstall['languages'])) {
                    $data['install'] = $availables;
                } else {
                    $data['install'] = array_intersect($this->autoinstall['languages'], $availables);
                }
            }
     ...
     * @return void
     */
    public function moduleInstall($module_gid)
    {
     ...
     * @return void
     */
    public function moduleUpdate($module_gid, $path = '')
    {
        if (!$path) {
            $path = $module_gid;
        }
     ...
     * @return void
     */
    public function ajaxModuleDelete($module_gid)
    {
     ...
     * @return void
     */
    public function moduleDelete($module_gid)
    {
        if (empty($module_gid)) {
     ...
     * @return void
     */
    public function moduleView($module_gid)
    {
        $module = $this->Install_model->get_module_config($module_gid);
        $this->view->assign('module', $module);
     ...
     * @return void
     */
    public function langs($action = null, $lang = null, $module = null)
    {
        ...
        switch ($action) {
            ...
            case 'update_at_once':
                ...
                break;
            case 'export':
                $this->Install_model->generate_module_lang_install($module, $lang);
                break;
        }
     ...
     * @return void
     */
    public function langsUpdate($lang_id, $module = null)
    {
     ...
     * @return string
     */
    public function langsUpdateBlock($lang_id, $module_gid = null)
    {
        $error = !$this->Install_model->update_langs($lang_id, $module_gid, 'update');
    ...
    {
        $params['where'] = $this->input->post('where', true);
        $modules = $this->Install_model->get_installed_modules('module_gid ASC', $params);
        $this->view->assign('modules', $modules);
        $this->view->render();
    }
     ...
     * @return void
     */
    public function ajaxOverallProduct()
    {
     ...
     * @return void
     */
    public function ajaxOverallProductUpdate($file, $path = 'product_updates')
    {
     ...
     * @return void
     */
    public function ajaxOverallModule($module_gid)
    {
     ...
     * @return void
     */
    public function ajaxStartInstall($module_gid, $install_type = 'product')
    {
     ...
     * @return void
     */
    public function ajaxDependencies($module_gid, $install_type = 'product')
    {
        $module_data = $this->Install_model->get_module_config($module_gid);
     ...
     * @return void
     */
    public function ajaxPermissions($module_gid, $install_type = 'product')
    {
        $module_data = $this->Install_model->get_module_config($module_gid);
     ...
     * @return void
     */
    public function ajaxRequirements($module_gid, $install_type = 'product')
    {
        $module_data = $this->Install_model->get_module_config($module_gid);
     ...
     * @return void
     */
    public function ajaxSql($module_gid, $install_type = 'product')
    {
        $module_data = $this->Install_model->get_module_config($module_gid);
     ...
     * @return void
     */
    public function ajaxLinked($module_gid)
    {
        $module_data = $this->Install_model->get_module_config($module_gid);
     ...
     * @return void
     */
    public function ajaxSettings($module_gid, $install_type = 'product')
    {
        $module_data = $this->Install_model->get_module_config($module_gid);
        ...
        if (!$submit && !empty($this->autoinstall['modules'][$module_gid])) {
            $_POST = array_merge($_POST, $this->autoinstall['modules'][$module_gid]);
            $submit = true;
        }
     ...
     * @return void
     */
    public function ajaxDemoContent($module_gid, $install_type = 'product')
    {
        $module_data = $this->Install_model->get_module_config($module_gid);
        $errors = array();
     ...
     * @return void
     */
    public function ajaxPublic($module_gid, $install_type = 'product', $file = '', $path = 'product_updates')
    {
    ...
    public function _get_license()
    {
        ...
        $save_file = SITE_PHYSICAL_PATH . 'license.txt';
        if (is_writable($save_file)) {
            $params['save_to'] = $save_file;
        }

        $client = new \GuzzleHttp\Client(['timeout' => 3]);        
        $response = $client->request('GET', $license_file, $params);
    ...
    public function _product_install_finish()
    {
        ...
        if (TRIAL_MODE && $this->pg_module->is_module_installed('network')) {
            $this->startNetwork();
        }
    ...
    {
        if (file_exists(SITE_PHYSICAL_PATH . "install.php")) {
            $autoinstall = include SITE_PHYSICAL_PATH . "install.php";
            if (!empty($autoinstall['modules']['network']['autostart']) ) {
                $this->load->model('Network_model');
                $domain = $this->Network_model->getDefaultDomain();
                if ($domain != 'localhost') {
                    @set_time_limit(300);
                    $key_request = $this->Network_model->requestKey($domain);
                    if (!empty($key_request['code']) && empty($key_request[View::MSG_ERROR])) {
                        $this->Network_model->set_config(array(
                            'key'    => $key_request['code'],
                            'domain' => $domain,
                        ));
                        $this->pg_module->set_module_config('network', 'is_registered', 1);
                        $this->Network_model->start();
                    }
     ...
     * @return void
     */
    public function generateInstallLang()
    {
     ...
     * @return void
     */
    public function generateInstallPermissions()
    {
     ...
     * @return void
     */
    public function generateInstallModuleSettings($module_gid)
    {
        $this->load->model('install/models/Backup_model');
     ...
     * @return void
     */
    public function generateModuleFilesBackup($module_gid)
    {
        $this->load->model('install/models/Backup_model');
     ...
     * @return void
     */
    public function generateModulesBackup()
    {
        $this->load->model('install/models/Backup_model');
    ...
    public function productUpdate($file, $path = 'product_updates')
    {
        $this->load->model('install/models/Updates_model');
    ...
    public function updateInstall($module_gid, $path = '')
    {
        $this->load->model('install/models/Updates_model');
    ...
    public function ajaxUpdateSql($module_gid, $install_type = 'product')
    {
        $this->load->model('install/models/Updates_model');
    ...
    public function ajaxFiles($module_gid, $install_type = 'product')
    {
        $this->load->model('install/models/Updates_model');
    ...
    public function ajaxChmod($module_gid, $install_type = 'product')
    {
        $this->load->model('install/models/Updates_model');
        $module_data = $this->Updates_model->get_update_config($module_gid);
    ...
    public function ajaxUpdateSettings($module_gid, $install_type = 'product')
    {
        $this->load->model('install/models/Updates_model');
    ...
    public function ajaxUpdatePublic($module_gid, $install_type = 'product', $file = '', $update_path = 'product_updates')
    {
        $this->load->model('install/models/Updates_model');
        $path = $module_gid;
        $module_gid = $this->Updates_model->get_module_by_path($path);
    ...
    public function updateInstallSettings($module_gid)
    {
        $this->load->model('install/models/Updates_model');
        $path = $module_gid;
        $module_gid = $this->Updates_model->get_module_by_path($path);
    ...
    public function libraryInstall($gid)
    {
        $this->load->model('install/models/Libraries_model');
        $this->load->library('pg_library');
        $libraries = $this->Libraries_model->get_installed_libraries();
    ...
    public function libraryUpdateInstall($gid)
    {
        $this->load->model('install/models/Libraries_model');
    ...
    public function ajaxGetLibraryUpdate()
    {
        $this->load->model('install/models/Libraries_model');
    ...
    public function ajaxUnpackLibraryUpdate()
    {
    ...
    public function ajaxCopyLibraryUpdate()
    {
        $gid = $this->input->post('gid');
        $targetDir = TEMPPATH . 'trash/' . $gid . '/';
    ...
    public function installerSettings()
    {
        $this->load->model('install/models/Ftp_model');
    ...
    {
        $this->load->model('install/models/System_utility_model');
        if ($this->input->post('save_data')) {
            $post_data = $this->input->post('modules_data', true);
            $this->System_utility_model->setModulesData($post_data);
        }

        $modules_data = $this->System_utility_model->getModulesData();
        $this->view->assign('modules_data', $modules_data);

        $this->view->setHeader('Update modules data');
        $this->view->assign('modules_setup', true);
        $this->view->render('update_modules_data');
    }
}
