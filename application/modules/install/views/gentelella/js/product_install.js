function productInstall(optionArr) {
    ...
    this.update_overall_progress = function (progress) {
        var postTitle = '';
        if (progress == 100) {
            postTitle += ' - The site is successfully installed!';
        } else if (_self.properties.currentModule) {
            postTitle += ' - ' + _self.properties.currentModule;
        }
        document.title = progress + '%' + postTitle;
        
        if (parent) {
            try {
                parent.postMessage(JSON.stringify({"progress": progress}), "*");
            } catch(e) {
    ...
    this.get_modules = function (params = null) {
    ...
    this.langs_update = function (cur_module = null) {
        if (!_self.properties.currentLang) {
            return;
        }
        var params = {};
        if (cur_module) {
            params = { 'where': {'module_gid': cur_module} };
        }
        _self.get_modules(params);
    ...
    this.langs_update_module = function () {
        ...
        if ('' === _self.properties.currentModule) {
            ...
            _self.update_overall_progress(Math.round(100));
