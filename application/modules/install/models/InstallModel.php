...
class InstallModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        if (defined('USE_PSR') && USE_PSR) {
            $this->prepare_installing_method = 'prepareInstalling';
            $this->arbitrary_installing_method = 'arbitraryInstalling';
            $this->arbitrary_deinstalling_method = 'arbitraryDeinstalling';
            $this->get_settings_form_method = 'getSettingsForm';
            $this->validate_requirements_method = 'validateRequirements';
            $this->arbitrary_lang_install_method = 'arbitraryLangInstall';
            $this->arbitrary_lang_export_method = 'arbitraryLangExport';
        }
    }
        ...
        if (!empty($sql_array)) {
            foreach ($sql_array as $sql) {
                ...
                if (!empty($sql)) {
                    ...
                    if (!$result && !preg_match('/^alter/i', $sql) && !preg_match('/^duplicate/i', mysqli_error())) {
    ...
    public function set_module_installed($module_gid)
    {
        ...
        $data = [
            ...
            'is_custom_module' => !empty($module_data['is_custom_module']) ? $module_data['is_custom_module'] : 0,
    ...
    public function get_installed_modules($order_by = 'module_gid ASC', $params = [])
    {
        $modules = array();
        $modules_by_id = $this->ci->pg_module->get_modules($order_by, $params);
        usort($modules_by_id, (function($a, $b){
            return strcmp($a["module_gid"], $b["module_gid"]);
        }));
    ...
    public function has_module_requirements($module_gid)
    {
        ...
        if (method_exists($this->ci->{$model_name}, $this->validate_requirements_method)) {
            return true;
        }

        // TODO: убрать после приведения к PSR
        if (method_exists($this->ci->{$model_name}, '__call')) {
            return true;
        }
    ...
    public function execute_method($model_name, $method, $min_version = null, $params = null)
    {
        ...
        if (!property_exists($this->ci, $model_name)) {
            return false;
        }
        ...
        // TODO: убрать после приведения к PSR
        elseif (method_exists($this->ci->{$model_name}, '__call')) {
            if (!is_null($params)) {
                return $this->ci->{$model_name}->{$method_name}($params);
            } else {
                return $this->ci->{$model_name}->{$method_name}();
    ...
    public function _language_install($module_gid, $lang_id = null, $delete_old = true)
    {
        ...
        // if language files(except default lang files) not full filled or missing
        foreach ($installed_langs as $installed_lang) {
            if (!file_exists($langs_path . $installed_lang['code'])) {
                mkdir($langs_path . $installed_lang['code'], '0777', true);
            }
        }
        $this->ci->load->model('languages/models/languages_tools_model');
        $this->ci->languages_tools_model->fillSort($default_lang_code, $langs_path);
    ...
    public function _settings_install($module_gid)
    {
        ...
        if(!empty($install_settings)) {
            ...
            foreach ($install_settings as $config_gid => $value) {
                if ($module_gid == 'mobile' && !TRIAL_MODE) {
                    if ($config_gid == 'android_url' || $config_gid == 'ios_url') {
                        $value = '';
                    }
                }
                $this->ci->pg_module->set_module_config($module_gid, $config_gid, $value);
    ...
    private function addAclPermissions($module_gid, array $permissions)
    {
        ...
        $this->ci->load->model([
            'Access_permissions_model', 'access_permissions/models/Access_permissions_settings_model',
        ]);
        $subscription_type = $this->ci->Access_permissions_settings_model
            ->getSubscriptionType(\Pg\modules\access_permissions\models\AccessPermissionsSettingsModel::TYPE);
        if ($subscription_type == 'user_types') {
            $additional_roles = $this->ci->Access_permissions_settings_model->getAccessData(
                $this->ci->Access_permissions_model->roles['user'])->getRoles();
        }
        ...
        foreach ($permissions as $controller => $data) {
            foreach($data as $action => $role_id) {
                ...
                if (!empty($additional_roles) && $role_id == 2 && INSTALL_DONE) {
                    foreach ($additional_roles as $add_role) {
                        $this->ci->acl->getManager()
                        ->role($add_role)
                        ->allow($action_view_page->getGid(), $res_page->getResourceType(), $res_page->getResourceId());
                    }
    ...
    private function deleteAclPermissions($module_gid, array $permissions)
    {
        $this->ci->acl->getManager()->setRole('guest');
        $this->ci->acl->getManager()->setRole(['user', 'admin', 'installer'], 'guest');
        $roles = [
        ...
        foreach ($permissions as $controller => $data) {
            foreach($data as $action => $role_id) {
                ...
                $this->ci->acl->getManager()
                        ->role($roles[$role_id])
                        ->clear($action_view_page->getGid(), $res_page->getResourceType(), $res_page->getResourceId());
                if ($role_id == 1) {
                    $this->ci->acl->getManager()
                        ->role('user')
                        ->clear($action_view_page->getGid(), $res_page->getResourceType(), $res_page->getResourceId());
    ...
    public function _permissions_deinstall($module_gid)
    {
        ...
        $this->deleteAclPermissions($module_gid, $_permissions);
        $this->ci->pg_module->clear_module_metods_access($module_gid);
    }
        ...
        if (!empty($config['linked_modules'][$action])) {
            ...
            foreach ($config_linked as $module => $method) {
                ...
                if (count(array_diff($modules, $installed_modules)) == 0) {
                    ...
                    // Langs
                    $method = (is_array($method) ? $method['name'] : $method);
        ...
        foreach ($installed_modules as $installed_module_gid) {
            ...
            foreach ($linked_config['linked_modules'][$action] as $linked_module => $linked_method) {
                ...
                if (in_array($module_gid, $linked_modules) && count(array_diff($linked_modules, $installed_modules)) == 0) {
                    ...
                    // Langs
                    $linked_method = (is_array($linked_method) ? $linked_method['name'] : $linked_method);

                    // TODO: убрать проверку __call после приведения к PSR
                    if (defined('USE_PSR') && USE_PSR) {
                        $linked_method .= 'LangUpdate';
                    } else {
                        $linked_method .= '_lang_update';
                    }
    ...
    public function has_module_settings($module_gid)
    {
        ...
        if (method_exists($this->ci->{$model_name}, $this->get_settings_form_method)) {
            return true;
        }
        // TODO: убрать после приведения к PSR
        elseif (method_exists($this->ci->{$model_name}, '__call')) {
    ...
    {
        $installed_modules = array_keys($this->get_installed_modules('module_gid ASC', ['where' => ['is_custom_module' => 0]]));
        if (is_null($modules)) {
        ...
        foreach ($modules as $module_gid) {
            ...
            foreach ($this->ci->pg_language->languages as $lang_id => $lang_data) {
                ...
                // arbitrary
                if (method_exists($this->ci->{$model}, $this->arbitrary_lang_export_method)) {
                    $langs_data = (array) $this->execute_method($model, $this->arbitrary_lang_export_method, null, (array) $lang_id);
                ...
                // TODO: убрать после приведения к PSR
                elseif (method_exists($this->ci->{$model}, '__call')) {
                    $langs_data = (array) $this->execute_method($model, $this->arbitrary_lang_export_method, null, (array) $lang_id);
                    foreach ($langs_data as $lmodule => $ldata) {
                        $lang_install_code = $this->ci->pg_language->generate_install_linked_lang($ldata, $lang_id);
                        if ($lang_install_code) {
                ...
                foreach ($linked_modules as $linked_module => $linked_method) {
                    ...
                    if (count(array_diff($linked_modules, $installed_modules)) == 0) {
                        $method_name = (is_array($linked_method) ? $linked_method['name'] : $linked_method);

                        // TODO: убрать проверку __call после приведения к PSR
                        if (defined('USE_PSR') && USE_PSR) {
                            $method_name .= 'LangExport';
                        } else {
                            $method_name .= '_lang_export';
                        }
     ...
     * @return boolean
     */
    public function update_langs($lang_id, $module_gid = null, $action)
    {
        if (!$lang_id) {
        ...
        foreach ($installed_modules as $installed_module_gid) {
            ...
            foreach ($config['linked_modules']['install'] as $linked_module => $linked_method) {
                ...
                if (count(array_diff($linked_modules, $installed_modules)) == 0) {
                    ...
                    // TODO: убрать проверку __call после приведения к PSR
                    if (defined('USE_PSR') && USE_PSR) {
                        $method_name .= 'LangUpdate';
                    } else {
                        $method_name .= '_lang_update';
                    }
