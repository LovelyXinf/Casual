...
class ChatboxInstallModel extends \Model
{
    ...
    private $menu = array(
        'user_top_menu' => array(
            ...
            "action" => "none",
            "items"  => array(
                'chatbox_item' => array("action" => "create", 'icon' => '','link' => 'chatbox/index', 'status' => 1, 'sorter' => 3)
            )
        ),
    );
    ...
    private $thumbs = array(
        array('prefix' => 'grand',  'width'=> 960, 'height' => 720, 'crop_param' => 'resize'),
        array('prefix' => 'great',  'width'=> 305, 'height' => 305, 'crop_param' => 'crop'),
        array('prefix' => 'hgreat', 'width'=> 305, 'height' => 200, 'crop_param' => 'crop'),
        array('prefix' => 'vgreat', 'width'=> 200, 'height' => 305, 'crop_param' => 'crop'),
        array('prefix' => 'big',    'width'=> 200, 'height' => 200, 'crop_param' => 'crop'),
        array('prefix' => 'middle', 'width'=> 100, 'height' => 100, 'crop_param' => 'crop'),
        array('prefix' => 'small',  'width'=> 60,  'height' => 60,  'crop_param' => 'crop'),
    );
    ...
    private $notifications = array(
        "templates" => array(
            array(
                ...
                "vars" => array("fname", "sname", "sender", "message"), 
                "content_type" => "text"),
        ),
        "notifications" => array(
            array('module' => ChatboxModel::MODULE_GID, "gid" => "chatbox_new_message", "template" => "chatbox_new_message", "send_type" => "que"),
        ),
    );
    ...
    public function __construct()
    {
        ...
        $this->access_permissions = Setup::getModuleData(
                ChatboxModel::MODULE_GID, Setup::TYPE_ACCESS_PERMISSIONS
        );
        $this->demo_content = Setup::getModuleData(
                ChatboxModel::MODULE_GID, Setup::TYPE_DEMO_CONTENT
        );
        $this->modules_data = Setup::getModuleData(
                ChatboxModel::MODULE_GID, Setup::TYPE_MODULES_DATA
        );
    }
    ...
    // {
    //     $langs_file = $this->ci->Install_model->language_file_read("chatbox", "services", $langs_ids);
    //     if (!$langs_file) {
    ...
    //     $this->ci->load->model('Cronjob_model');
    //     $cron_data = array();
    //     $cron_data["where"]["module"] = "chatbox";
    ...
    public function installBannersLangUpdate($langs_ids = null)
    {
        ...
        $banners_groups = array('banners_group_chatbox_groups');
    ...
    {
        $this->ci->pg_seo->set_seo_module('chatbox', array(
            'module_gid'              => 'chatbox',
            'model_name'              => 'Chatbox_model',
