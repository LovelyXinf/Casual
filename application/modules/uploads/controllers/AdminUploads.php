 ...
 **/
class AdminUploads extends \Controller
{
    /**
    ...
    public function configEdit($config_id = null)
    {
        if ($config_id) {
    ...
    public function configThumbs($config_id)
    {
    ...
    public function thumbEdit($config_id, $thumb_id = null)
    {
        if ($thumb_id) {
    ...
    public function thumbDelete($config_id, $thumb_id)
    {
    ...
    public function watermarkEdit($watermark_id = null)
    {
        if ($watermark_id) {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Uploads_config_model->validateWatermark($watermark_id, $post_data);
            if (!empty($validate_data["errors"])) {
                $this->system_messages->addMessage(View::MSG_ERROR, $validate_data["errors"]);
                $data = array_merge($data, $validate_data["data"]);
            } else {
                $data = $validate_data["data"];
                $this->Uploads_config_model->saveWatermark($watermark_id, $data);
        ...
        $this->view->assign('data', $this->Uploads_config_model->formatWatermark($data));
    ...
    public function wmPreview($wm_id = null, $temp_data_index = '')
    {
    ...
    public function wmSavePreviewData($type = 'file', $temp_data_index = 'wm_preview')
    {
        switch ($type) {
            case "file":
    ...
    public function wmClearPreviewData($temp_data_index = 'wm_preview')
    {
