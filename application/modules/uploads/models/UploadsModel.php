...
class UploadsModel extends \Model
{
    ...
    public function getMediaPath($config_gid, $postfix = '', $is_admin_access = false)
    {
        if (!empty($postfix)) {
            $postfix = $this->formatPostfix($postfix, $is_admin_access);
        } elseif ($postfix !== '') {
            $postfix .= '/';
        }
    ...
    public function getMediaUrl($config_gid, $postfix = '')
    {
        if (!empty($postfix)) {
    ...
    public function formatPostfix($postfix = '', $is_admin_access = false)
    {
        $postfix = rtrim($postfix, '/');
        $result = $postfix . '/';
        if ((strval(intval($postfix)) == $postfix) || $is_admin_access) {
    ...
    public function getConfig($config_gid)
    {
     ...
     * @return array
     */
    public function upload($config_gid, $postfix, $upload_gid, $original = '')
    {
        $return["errors"] = array();
        $return["file"] = "";

        $config_data = $this->getConfig($config_gid);

        $this->ci->load->helper('upload');
        $path = $this->getMediaPath($config_gid, $postfix);
        if (!empty($original)) {
            $this->createPath($path . $original);
        } else {
        ...
        if (!$type_check || !isset($_FILES[$upload_gid]['name'])) {
            ...
            if ($image_return["error"] != '') {
                ...
                if (!empty($original)) {
                    @copy($path . $file_name, $path . $original . $new_file_name);
                }
                ...
                if (isset($config_data["thumbs"]) && !empty($config_data["thumbs"])) {
                    $this->createThumbs($new_file_name, $path, $config_data["thumbs"]);
                    if (!empty($original)) {
                        $this->createThumbs($new_file_name, $path . $original, $config_data["thumbs"]);
                    }
                }
            }
        }
    ...
    public function uploadUrl($config_gid, $postfix, $file_url, $upload_gid = '')
    {
        $return["errors"] = array();
        $return["file"] = "";
    ...
    public function uploadExist($config_gid, $postfix, $file_path)
    {
        $return["errors"] = array();
        $return["file"] = "";
        ...
        if ($image_return["error"] != '') {
            ...
            switch ($config_data["name_format"]) {
                case "generate": $new_file_name = $this->generateFilename($file_name_ext); break;
                case "format": $new_file_name = $this->formatFilename($file_name); break;
            }
     ...
     */
    public function uploadAnim($config_gid, $postfix, $file_path, $sources)
    {
        $return["errors"] = array();
        $return["file"] = "";
    ...
    public function validateUpload($config_gid, $upload_gid)
    {
        $return["errors"] = array();
        $return["file"] = "";
    ...
    public function formatUpload($config_gid, $postfix, $file_name = '', $original = '')
    {
        if (empty($file_name)) {
            return $this->formatDefaultUpload($config_gid);
        } else {
            $config_data = $this->getConfig($config_gid);
            if (substr($file_name, 0, 2) == '//' || preg_match('#https?:#', $file_name)) {
                $path = $url = dirname($file_name);
                $file_name = str_replace($url . "/", '', $file_name);
                $path .= DIRECTORY_SEPARATOR;
                $url .= "/";
            } else {
                $path = $this->getMediaPath($config_gid, $postfix) . $original;
                $url = $this->getMediaUrl($config_gid, $postfix) . $original;
            }
            $upload = array(
                "path"      => $path,
                "url"       => $url,
    ...
    public function formatDefaultUpload($config_gid)
    {
        $config_data = $this->get_config($config_gid);
    ...
    public function moveUpload($config_gid, $cur_path, $file_name, $path_destination) {
        @copy($cur_path . $file_name, $path_destination . $file_name);
        $config_data = $this->getConfig($config_gid);
        ...
        if (isset($config_data["thumbs"]) && !empty($config_data["thumbs"])) {
            foreach ($config_data["thumbs"] as $thumb_data) {
                if ($thumb_data["animation"]) {
                    ...
                    $thumb = $cur_path . $thumb_data["prefix"] . '-' . $basename . ".gif";
                    $thumb_destination = $path_destination . $thumb_data["prefix"] . '-' . $basename . ".gif";
                } else {
                    $thumb = $cur_path . $thumb_data["prefix"] . '-' . $file_name;
                    $thumb_destination = $path_destination . $thumb_data["prefix"] . '-' . $file_name;
                }
     
                if (file_exists($thumb)) {
                    @copy($thumb, $thumb_destination);
                }
            }
        }
    }

    public function deleteUpload($config_gid, $postfix, $file_name, $original = '')
    {
        $config_data = $this->getConfig($config_gid);
        $path = $this->getMediaPath($config_gid, $postfix) . $original;
        $file = $path . $file_name;
    ...
    public function recropUpload($config_gid, $postfix, $file_name, $recrop_data, $thumb_prefix = "", $is_admin_access = false)
    {
        $config_data = $this->get_config($config_gid);
        ...
        $path = $this->get_media_path($config_gid, $postfix, $is_admin_access);
        $src = $path . $file_name;
     ...
     */
    public function rotateUpload($config_gid, $postfix, $file_name, $angle, $is_admin_access = false)
    {
        $config_data = $this->get_config($config_gid);
        ...
        $rotate_config['rotation_angle'] = $angle;
    ...
    public function blurImage($config_gid, $postfix, $file_name, $power)
    {
        $config_data = $this->getConfig($config_gid);
        $this->ci->load->library('image_lib');
        $path = $this->getMediaPath($config_gid, $postfix);
        $src = $path . $file_name;
        $temp_src = $path . 'blur_temp_' . $file_name;
        @copy($src, $temp_src);
        $this->ci->image_lib->initialize([
            'source_image' => $temp_src,
            'create_thumb' => false
        ]);
        $this->ci->image_lib->setBlurImageGD($power);
        if (isset($config_data["thumbs"]) && !empty($config_data["thumbs"])) {
            $this->createThumbs('blur_temp_' . $file_name, $path, $config_data["thumbs"]);
            foreach ($config_data["thumbs"] as $i => $thumb_config) {
                @copy($path . $thumb_config["prefix"] . "-blur_temp_" . $file_name, $path . $thumb_config["prefix"] . "-" . $file_name);
                @unlink($path . $thumb_config["prefix"] . "-blur_temp_" . $file_name);
            }
        }
    ...
    public function generateFilename($file_type)
    {
        srand();
    ...
    public function createThumbs($file_name, $file_path, $thumbs_data, $animation = false)
    {
    ...
    public function createDefault($file_name, $file_path, $thumbs_data)
    {
        if (empty($thumbs_data)) {
            return false;
        }
    ...
    public function resize($file, $width, $height, $type = 'in', $maintain = false, $color = false, $effect = false, $dynamic_output = false)
    {
        ...
        if ($effect !== false) {
            switch ($effect) {
                ...
                case "blurimage": $resize_config['blurimage'] = true; break;
            }
        }
     ...
     * @retrun array
     */
    private function convert2gif($file_name, $file_path, $sources)
    {
     ...
     * @param array  $sources     source images
     */
    public function createAnim($file_name, $file_path, $thumbs_data, $sources)
    {
        if (empty($thumbs_data)) {
            return false;
        }
        $sources = $this->convert2gif($file_name, $file_path, $sources);
     ...
     */
    public function preCheckFileType($file_name, $file, $post_file_type, $allowed_extensions)
    {
        ...
        $use_fileinfo = $this->ci->pg_module->get_module_config('file_uploads', 'use_fileinfo');
     ...
     */
    public function deletePath($config_gid, $postfix)
    {
        $path = $this->get_media_path($config_gid, $postfix);
        if (file_exists($path)) {
            @rmdir($path);
    ...
    {
        $methods = [
            'create_anim' => 'createAnim',
            'create_default' => 'createDefault',
            'create_thumbs' => 'createThumbs',
            'delete_path' => 'deletePath',
            'delete_upload' => 'deleteUpload',
            'format_default_upload' => 'formatDefaultUpload',
            'format_filename' => 'formatFilename',
            'format_upload' => 'formatUpload',
            'generate_filename' => 'generateFilename',
            'get_config' => 'getConfig',
            'get_media_path' => 'getMediaPath',
            'get_media_url' => 'getMediaUrl',
            'pre_check_file_type' => 'preCheckFileType',
            'recrop_upload' => 'recropUpload',
            'rotate_upload' => 'rotateUpload',
            'upload_anim' => 'uploadAnim',
