 ...
 * @author      Pilot Group Ltd <http://www.pilotgroup.net/>
 */
class AdminModerators extends \Controller
{
    ...
    {
        if ($user_id) {
            $data = $this->Moderators_model->getUserById($user_id);
        } else {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $post_data['permission_data'] = $this->Moderators_model->setModuleDependences($post_data['permission_data']);
            $validate_data = $this->Moderators_model->validateUser($user_id, $post_data);
            if (!empty($validate_data["errors"])) {
                ...
                $new_user_id = $this->Moderators_model->saveUser($user_id, $data);
        ...
        $methods = $this->Moderators_model->getMethodsByGroups(array('where' => array('is_hidden' => 0)));
        if (!empty($data["permission_data"])) {
            foreach ($methods as $module_group => $module_data) {
                if (!empty($module_data["methods"])) {
                   foreach ($module_data["methods"] as $key => $method) {
                        ...
                        if (isset($data["permission_data"][$module][$current_method])) {
                            $methods[$module_group]["methods"][$key]["checked"] = 1;
                        }
                    }
                }
            }
        } else {
            if (!$user_id && empty($validate_data["errors"])) {
                /*
                 * Set default methods, which could be available for the new moderator
                 */
                $methods_default = $this->Moderators_model->getMethods(array('where' => array('module' => 'moderation')));
                if (empty($methods_default)) {
                    //get the first method from the list
                    $methods_default = $this->Moderators_model->getMethods(array('limit' => 1));     
                }                
                foreach ($methods as $module_group => $module_data) {
                    if (!empty($module_data["methods"])) {
                       foreach ($module_data["methods"] as $key => $method) {
                            $current_method = $method["method"];
                            $module = $method["module"];
                            if (isset($methods_default[$module])){
                                $methods[$module_group]["methods"][$key]["checked"] = 1;
                            }
                        }
                    }
