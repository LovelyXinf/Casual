...
class UsersConnectionsModel extends \Model
{
    ...
    public function getConnectionByUserId($service_id = false, $user_id = false)
    {
        ...
        $this->ci->db->select(implode(", ", $select_attrs))->from(USER_CONNECTIONS_TABLE)->where(array('service_id' => $service_id, 'user_id' => $user_id))->order_by('id ASC');
    ...
    public function getConnectionById($connection_id = false)
    {
        $data = array();
        $select_attrs = $this->fields_all;
        $this->ci->db->select(implode(", ", $select_attrs))->from(USER_CONNECTIONS_TABLE)->where(array('id' => $connection_id))->order_by('id ASC');
        $result = $this->ci->db->get()->result_array();
        if (!empty($result)) {
            $data = $result[0];
        }
    ...
    public function getConnectionByData($service_id = false, $odata = false)
    {
        $data = array();
        $select_attrs = $this->fields_all;
        $this->ci->db->select(implode(", ", $select_attrs))->from(USER_CONNECTIONS_TABLE)->where(array('service_id' => $service_id, 'data' => $odata))->order_by('id ASC');
        $result = $this->ci->db->get()->result_array();
        if (!empty($result)) {
            $data = $result[0];
        }
