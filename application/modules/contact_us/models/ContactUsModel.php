...
class ContactUsModel extends \Model
{
    ...
    {
        $result = $this->ci->db->select(implode(", ", $this->fields))->from(CONTACT_REASONS_TABLE)->where("id", $id)->get()->result_array();
        if (!empty($result)) {
    ...
    public function getReasonList($params = array(), $filter_object_ids = null, $order_by = null)
    {
    ...
    public function getReasonCount($params = array(), $filter_object_ids = null)
    {
            ...
            if ($generate === true) {
                ...
                foreach ($languages as $lang_id => $lang_data) {
                    if ($this->ci->pg_language->is_active($lang_id) === true) {
                        ...
                        $return[] = array(
                            "url"      => rewrite_link('contact_us', 'index', array(), false, $lang_code),
