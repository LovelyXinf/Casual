...
use Pg\modules\field_editor\models\FieldTypesLoaderModel;
...
class FieldEditorFormsModel extends \Model
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('field_editor_forms', CacheManager::PROVIDER_REDIS);
    }
    ...
    public function formatOutputForm($data, $content = array(), $defaults = false)
    {
    ...
    public function getFormsList($params = array(), $order_by = array())
    {
    ...
    public function getFormsCount($params = array())
    {
        if (isset($params["where"]) && is_array($params["where"]) && count($params["where"])) {
            foreach ($params["where"] as $field => $value) {
                $this->ci->db->where($field, $value);
    ...
    public function saveForm($id, $data)
    {
        ...
        $this->ci->cache->flash('field_editor_forms');
