 ...
 * */
class AdminFieldEditor extends \Controller
{
    /**
     * Controller
     */
    ...
    {
        parent::__construct();
        $this->load->model(['Field_editor_model', 'Menu_model']);
     ...
     *
     */
    public function sectionEdit($type, $id = null)
    {
        if (empty($type)) {
            $type = $this->Field_editor_model->getDefaultEditorType(true);
        }
        $this->Field_editor_model->initialize($type);
        $this->view->assign('type', $type);
        $data = [];
        if (!empty($id)) {
            $data = $this->Field_editor_model->getSectionById($id);
        ...
        if ($this->input->post('btn_save')) {
            ...
            $validate_data = $this->Field_editor_model->validateSection($id, $post_data, $langs);
            ...
            if (!empty($validate_data["errors"])) {
                ...
                if (!$id) {
                    $validate_data["data"]["sorter"] = $this->Field_editor_model->getSectionCount() + 1;
                }

                $this->Field_editor_model->saveSection($id, $validate_data["data"], $validate_data["lang"]);
     ...
     *
     */
    public function sectionDelete($type, $id)
    {
        $this->Field_editor_model->initialize($type);
        if (!empty($id)) {
     ...
     *
     */
    public function ajaxSectionSort()
    {
     ...
     * @return mixed
     */
    public function fieldEdit($type, $section, $id = null)
    {
        if (empty($type)) {
        ...
        if ($this->input->post('btn_save')) {
            ...
            $langs = $this->input->post("langs", true);
            $validate_data = $this->Field_editor_model->validateField($id, $field_type, $post_data, $langs);
            ...
            if (!empty($validate_data["errors"])) {
                ...
                $id = $this->Field_editor_model->saveField($id, $type, $section, $validate_data["data"], $validate_data['lang']);
                $this->system_messages->addMessage(View::MSG_SUCCESS, l('success_update_section_data', 'field_editor'));
                if ($flag == "add") {
    ...
    private function getFieldTypeBlock($field_type, $data)
    {
        ...
        $this->view->assign('data', $data);
        $this->view->assign('initial', $this->Field_editor_model->getFieldSettings($field_type));
     ...
     *
     */
    public function ajaxGetFieldSelectOptions($id_field)
    {
     ...
     *
     */
    public function ajaxGetSelectOptionForm($id_field, $option_gid = '')
    {
        $field_data = $this->Field_editor_model->get_field_by_id($id_field);
     ...
     *
     */
    public function ajaxSetSelectOption($id_field, $option_gid = '')
    {
     ...
     *
     */
    public function ajaxDeleteSelectOption($id_field, $option_gid)
    {
     ...
     *
     */
    public function ajaxSaveSelectOptionSorter($id_field)
    {
        $field_data = $this->Field_editor_model->get_field_by_id($id_field);
     ...
     *
     */
    public function fieldDelete($type, $section, $id)
    {
        $this->Field_editor_model->initialize($type);
        if (!empty($id)) {
     ...
     *
     */
    public function formEdit($type, $id = 0)
    {
        if (empty($type)) {
            $type = $this->Field_editor_model->get_default_editor_type(true);
        }
     ...
     *
     */
    public function formDelete($type, $id = 0)
    {
        if (empty($type)) {
            $type = $this->Field_editor_model->get_default_editor_type(true);
        }
     ...
     *
     */
    public function ajaxSaveFormFields($id)
    {
        $this->load->model('field_editor/models/Field_editor_forms_model');
        $post_data = array(
            'field_data' => $this->input->post("field_data", true),
        );
        $validate_data = $this->Field_editor_forms_model->validateForm($id, $post_data);
        $this->Field_editor_forms_model->saveForm($id, $validate_data["data"]);
     ...
     *
     */
    public function ajaxGetAddSectionForm($id, $section_gid = '')
    {
        $this->load->model('field_editor/models/Field_editor_forms_model');
        $form = $this->Field_editor_forms_model->get_form_by_id($id);
     ...
     *
     */
    public function ajaxGetAddFieldForm($id, $section_gid = '')
    {
        $this->load->model('field_editor/models/Field_editor_forms_model');
        $data = $this->Field_editor_forms_model->get_form_by_id($id);
     ...
     *
     */
    public function ajaxGetSectionData($id_form, $section_gid = '')
    {
        $this->load->model('field_editor/models/Field_editor_forms_model');
        ...
        $langs = $this->input->post("langs", true);
        if (!empty($langs)) {
            $section_gid = $this->Field_editor_forms_model->formSectionSave($data["editor_type_gid"], $section_gid, $langs);
        }
        if (empty($section_gid)) {
            return false;
        }
     ...
     *
     */
    public function ajaxGetFieldData($id_form, $field_gid)
    {
        $this->load->model('field_editor/models/Field_editor_forms_model');
        $data = $this->Field_editor_forms_model->get_form_by_id($id_form);
        list($disallowed_sections, $disallowed_fields) = $this->getDisallowedData($data["field_data"]);
     ...
     *
     */
    public function ajaxDeleteFormSection($id_form, $section_gid)
    {
        $this->load->model('field_editor/models/Field_editor_forms_model');
        $data = $this->Field_editor_forms_model->get_form_by_id($id_form);
     ...
     *
     */
    public function ajaxGetFieldSettingsForm($id_form, $field_gid)
    {
        $this->load->model('field_editor/models/Field_editor_forms_model');
        $data = $this->Field_editor_forms_model->getFormById($id_form);
