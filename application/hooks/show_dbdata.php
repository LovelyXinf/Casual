    ...
    function show_dbdata()
    {
        ...
        if ($queries) {
            foreach ($queries as $key => $query) {
                ...
                $table = preg_match("/(FROM|INSERT INTO|UPDATE) (.*)( AS .*)*( |$)/iU", $query, $matches);
                $matches[2] = trim($matches[2], '()'); 
                if (!empty($matches[2])) {
                    $result['tables']['table'][$matches[2]] = $matches[2];
                    $result['tables']['queries'][$matches[2]][] = round($query_times[$key], $precision) . "\t" . $query;
                }
