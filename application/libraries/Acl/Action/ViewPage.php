...
class ViewPage extends Action
{
    ...
    /* <custom_R> */
    protected function onDeniedOperator()
    {
        $ci             = get_instance();
        $res_login_page = new Resource\Page(
            ['module' => 'start', 'controller' => 'operator_start', 'action' => 'login',]
        );
        if ($ci->acl->checkSimple($this->gid, $res_login_page->getResourceType(), $res_login_page->getResourceId())) {
            $ci->view->setRedirect(site_url() . 'operator/start/login');
        } else {
            $ci->view->setRedirect();
        }
    }
