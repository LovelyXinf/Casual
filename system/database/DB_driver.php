...
class CI_DB_driver
{
    ...
    const MYSQL_LOST_CONNECTION = '2013';

    private $mysql_timeout_errors = [
        self::MYSQL_GONE_AWAY_CODE,
    ...
    public function __construct($params)
    {
        ...
        $this->save_queries = $_ENV['USE_PROFILING'] == 1 || $_ENV['DISPLAY_ERRORS'] == 1;
     ...
     * @return mixed
     */
    public function query($sql, $binds = false, $return_object = true, $is_repeat=false)
    {
        if ($sql == '') {
            if ($this->db_debug) {
        ...
        if (false === ($this->result_id = $this->simple_query($sql))) {
            ...
            if (!$is_repeat && in_array($this->_error_number(), $this->mysql_timeout_errors)) {
                $this->conn_id = ($this->pconnect == FALSE) ? $this->db_connect() : $this->db_pconnect();
                return $this->query($sql, $binds, $return_object, true);
            }
            ...
            } elseif ($this->_error_number()) {
                $this->error_number = $this->_error_number();
                exit(file_get_contents(SITE_PHYSICAL_PATH . UPLOAD_DIR . 'errors/db.html'));
            }
