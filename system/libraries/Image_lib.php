...
class CI_Image_lib
{
    ...
    {
        $props = ['source_folder', 'dest_folder', 'source_image',
            'full_src_path', 'full_dst_path', 'new_image', 'image_type',
            'size_str', 'quality', 'orig_width', 'orig_height', 'rotation_angle',
            'x_axis', 'y_axis', 'create_fnc', 'copy_fnc', 'wm_overlay_path',
            'wm_use_truetype', 'dynamic_output', 'wm_font_size', 'wm_text',
            'wm_vrt_alignment', 'wm_hor_alignment', 'wm_padding',
            'wm_hor_offset', 'wm_vrt_offset', 'wm_font_color', 'wm_use_drop_shadow',
            'wm_shadow_color', 'wm_shadow_distance', 'wm_opacity', 'grayscale',
            'background', 'bgcolor', 'bgwidth', 'bgheight','blurimage'];
    ...
    public function image_process_gd($action = 'resize')
    {
        ...
        if (isset($this->blurimage) && $this->blurimage) {
            $dst_img = $this->setBlurImageGD($dst_img, $this->width, $this->height);
        }
    ...
    public function setBlurImageGD($power = 50)
    {
        if (!$img = $this->image_create_gd()) {
            return false;
        }
        if ($power > 0) {
            imagefilter($img, IMG_FILTER_PIXELATE, $power, true);
            $this->image_save_gd($img);
        } else {
            $img = $this->image_create_gd($this->full_src_path);
        }
        $this->image_save_gd($img);
        imagedestroy($img);
    }
    ...
    public function overlay_watermark()
    {
        ...
        if ($this->wm_vrt_alignment == 'B') {
            $this->wm_vrt_offset = (is_numeric($this->wm_vrt_offset) ? $this->wm_vrt_offset : 0) * -1;
        }

        if ($this->wm_hor_alignment == 'R') {
            $this->wm_hor_offset = (is_numeric($this->wm_hor_offset) ? $this->wm_hor_offset : 0) * -1;
        }
        $this->wm_padding = is_numeric($this->wm_padding) ? $this->wm_padding : 0;
