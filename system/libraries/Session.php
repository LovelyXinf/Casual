...
class CI_Session
{
    ...
    public function __construct($params = array())
    {
        ...
        $this->cache->registerService('sessions', CacheManager::PROVIDER_REDIS);
    ...
    public function sess_read()
    {
        ...
        if ($this->sess_use_database === true) {
            if (!empty($session['session_id'])) {
                ...
                $sess_table_name = $this->sess_table_name;

                $result = $this->cache->get('sessions', $session_id, function () use ($session_id, $sess_table_name)
                {
                    $ci = &get_instance();
                    $query = $ci->db->where('session_id', $session_id)
                        ->get($sess_table_name);
            ...
            // No result?  Kill it!
            if (empty($result) ||
                $this->sess_match_ip == true && !$this->ci->router->is_api_class && $result['ip_address'] != $session['ip_address'] ||
                $this->sess_match_useragent == true && !$this->ci->router->is_api_class && $result['user_agent'] != $session['user_agent']) {
                $this->sess_destroy();
                return false;
            }
    ...
    public function sess_write()
    {
        ...
        $this->cache->set('sessions', $session_id, $session_data, function () use ($session_id, $sess_table_name, $session_data)
        {
            $ci = &get_instance();
            $ci->db->where('session_id', $session_id)
                ->update($sess_table_name, $session_data);
        });
    ...
    public function sess_create()
    {
        ...
        if ($this->sess_use_database === true) {
            ...
            $this->cache->set('sessions', $userdata['session_id'], $userdata, function () use ($sess_table_name, $userdata)
            {
                $ci = &get_instance();
    ...
    public function sess_update()
    {
        ...
        if ($this->sess_use_database === true) {
            ...
            $this->cache->delete('sessions', $old_sessid);
            ...
            $this->cache->set('sessions', $new_sessid, $userdata, function () use ($sess_table_name, $userdata, $old_sessid)
            {
                $ci = &get_instance();
                $ci->db->query($ci->db->update_string($sess_table_name, $userdata, ['session_id' => $old_sessid]));
            });
        }
    ...
    public function sess_destroy()
    {
        ...
        if ($this->sess_use_database === true and isset($this->userdata['session_id'])) {
            $session_id = $this->userdata['session_id'];
            $sess_table_name = $this->sess_table_name;
            $this->cache->delete('sessions', $session_id, function () use ($session_id, $sess_table_name)
            {
                $ci = &get_instance();
                $ci->db->where('session_id', $session_id)
                    ->delete($sess_table_name);
            });
        }
        
        // TODO: нужно переделать очистку токена
        if ($this->ci->router->is_api_class) {
            $this->token_destroy();
        }
