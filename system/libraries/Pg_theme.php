...
class CI_Pg_theme
{
    ...
    private function getAllThemes()
    {
        ...
        return $this->ci->cache->get('themes', 'all', function () use ($fields) {
            $ci = &get_instance();
    ...
    private function getAllColorsets()
    {
        ...
        return $this->ci->cache->get('themes_colorsets', 'all', function () use ($fields) {
            $ci = &get_instance();
    ...
    public function get_current_theme_type()
    {
        if ($this->ci->router->is_admin_class) {
            return 'admin';
        /* <custom_R> */
        } elseif ($this->ci->router->is_operator_class) {
            return 'operator';
        /* </custom_R> */
        } else {
            return 'user';
    ...
    public function format_theme_settings($module = '', $theme_type = '', $theme = '', $scheme = '')
    {
        ...
        if ($lang_id && !empty($active_settings['scheme_data']['id'])) {
            ...
            $res = $this->ci->Themes_model->getColoursetLogo($params);
    ...
    public function get_include_css_code($theme = '', $scheme = '')
    {
        ...
        if (INSTALL_MODULE_DONE && $this->ci->pg_module->is_module_installed('themes')) {
            $style_prefix = $this->ci->router->is_admin_class ? 'admin' : /* <custom_R */($this->ci->router->is_operator_class ? 'operator' : 'user')/* </custom_R> */;
