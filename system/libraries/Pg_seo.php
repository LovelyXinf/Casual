...
class CI_Pg_seo
{
    ...
    public function __construct()
    {
        ...
        // TODO: cache
        $this->ci->cache->registerService('seo_modules', CacheManager::PROVIDER_REDIS);
        $this->ci->cache->registerService('seo_settings', CacheManager::PROVIDER_REDIS);
    ...
    public function get_settings_full()
    {
        ...
        $languages = array_keys($ci->pg_language->languages);

        foreach ($results_raw as $result_raw) {
            foreach ($languages as $lang_id) {
                if ($result_raw['meta_' . $lang_id]) {
                    $result_raw['meta_' . $lang_id] = (array) unserialize($result_raw['meta_' . $lang_id]);
                } else {
                    $result_raw['meta_' . $lang_id] = [];
                }

                if ($result_raw['og_' . $lang_id]) {
                    $result_raw['og_' . $lang_id] = (array) unserialize($result_raw['og_' . $lang_id]);
                } else {
                    $result_raw['og_' . $lang_id] = [];
                }
            }
    ...
    public function preload_settings_cache()
    {
        ...
        // TODO: cache
        $this->settings_cache = $this->ci->cache->get('seo_settings', 'all', function () use ($fields) {
            return $this->get_settings_full();
        });
    }
    ...
    public function clear_settings_cache()
    {
        ...
        // TODO: cache
        $this->ci->cache->flush('seo_settings');
    }
    ...
    public function get_default_settings($controller, $module_gid, $method = '')
    {
        ...
        if ($controller == 'admin' || $controller == 'operator') {
            return false;
        }
        ...
        $model_name  = $module_data['model_name'];
        $method_name = $module_data['get_settings_method'];

        $this->ci->load->model($module_data['module_gid'] . '/models/' . $model_name);
        ...
        $settings = $this->ci->$model_name->$method_name($method);
        if (empty($method)) {
    ...
    {
        $fields = $this->modules_fields;

        // TODO: cache
        $this->seo_module_cache = $this->ci->cache->get('seo_modules', 'all', function () use ($fields) {
            $ci = &get_instance();

            $results_raw = $ci->db->select(implode(',', $fields))
                ->from(SEO_MODULES_TABLE)
                ->get()->result_array();

            $results = [];

            foreach ($results_raw as $result_raw) {
                $results[$result_raw['module_gid']] = $result_raw;
            }
    ...
    {
        $this->seo_module_cache = [];

        // TODO: cache
        $this->ci->cache->flush('seo_modules');
    }
     ...
     * @return void
     */
    public function set_seo_module($module_gid, $data = [])
    {
        $module_data = $this->get_seo_module_by_gid($module_gid);
        if (empty($module_data)) {
    ...
    public function parse_seo_data($settings)
    {
        ...
        if (isset($this->seo_dynamic_data['og_image_width'])) {
            $settings['og_image_width'] = $this->seo_dynamic_data['og_image_width'];
        }
        if (isset($this->seo_dynamic_data['og_image_height'])) {
            $settings['og_image_height'] = $this->seo_dynamic_data['og_image_height'];
        }
        ...
        if (empty($this->seo_tags_html_cache[$controller][$module_gid][$method])) {
            ...
            if (!empty($this->seo_page_tags)) {
                ...
                if (!empty($user_settings) && !empty($user_settings['meta_' . $this->ci->pg_language->current_lang_id])) {
                    ...
                    $default_data['og_image']       = isset($user_settings['og_' . $this->ci->pg_language->current_lang_id]['og_image']) ? $user_settings['og_' . $this->ci->pg_language->current_lang_id]['og_image'] : '';
            ...
            if (!empty($default_data['og_image'])) {
                ...
                $html['og_image'] = '   <meta property="og:image" content="' . APPPATH_VIRTUAL . 'views/flatty/logo/logo.png">' . "\n";
            }
    ...
    public function url_template_transform($module, $method, $data, $from, $to)
    {
        ...
        if ($from === 'base' && $to === 'scheme') {
            ...
            $auth_prefix   = ($parsed[0]['value'] == 'admin/') ? 'admin/' : /* <custom_R> */(($parsed[0]['value'] == 'operator/') ? 'operator/' : '')/* </custom_R> */;
            $redirect      = $auth_prefix . $module . '/' . $method;
            $exp_index     = [1];
            $postfix_index = 0;
            $rules         = [''];
            $vars_order    = [];
            ...
            foreach ($parsed as $match) {
                ...
                switch ($match['type']) {
                    ...
                    case 'postfix':
                        ...
                        $vars_order[$postfix_index]                    = isset($vars_order[$postfix_index - 1]) ? $vars_order[$postfix_index - 1] : [];
                        $vars_order[$postfix_index][$match['var_num']] = ['num' => $exp_index[$postfix_index - 1], 'name' => $match['var_name']];
                    ...
                    case 'tpl':
                        ...
                        $vars_order[$postfix_index][$match['var_num']] = ['num' => $exp_index[$postfix_index], 'name' => $match['var_name']];
     ...
     * @return string
     */
    public function create_url($module, $method, $str = [], $is_admin = false, $lang_code = null, $no_lang_in_url = false)
    {
        $link     = '';
    ...
    public function lang_dedicate_module_callback_add($lang_id)
    {
        ...
        if ($this->ci->db->field_exists('meta_' . $lang_id, SEO_SETTINGS_TABLE) === false) {
            $this->ci->dbforge->add_column(SEO_SETTINGS_TABLE, [
                'meta_' . $lang_id => [
                    'type' => 'text',
                    'null' => false,
                ]
            ...
            if ($lang_id != $default_lang_id) {
                $this->ci->db->set('meta_' . $lang_id, 'meta_' . $default_lang_id, false);
                $this->ci->db->update(SEO_SETTINGS_TABLE);
            }
        }
        if ($this->ci->db->field_exists('og_' . $lang_id, SEO_SETTINGS_TABLE) === false) {
            ...
            if ($lang_id != $default_lang_id) {
                $this->ci->db->set('og_' . $lang_id, 'og_' . $default_lang_id, false);
                $this->ci->db->update(SEO_SETTINGS_TABLE);
            }
        }
        return;
